...
backup_database() {
    ...
    backup_file="$backup_dir/backup_staging_$timestamp.sql"
    
    mkdir -p "$backup_dir"
    
    if pg_dump -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "${POSTGRES_DB:-cardstrategy_staging}" > "$backup_file"; then
        log_success "測試數據庫備份完成: $backup_file"
    else
        log_warning "測試數據庫備份失敗，繼續部署..."
    fi
}
...
build_and_start() {
    ...
    export POSTGRES_DB=${POSTGRES_DB:-cardstrategy_staging}
...
wait_for_services() {
    ...
    timeout=60
    while ! docker exec cardstrategy_postgres pg_isready -U "$POSTGRES_USER" &> /dev/null; do
    ...
    timeout=30
    while ! docker exec cardstrategy_redis redis-cli --raw incr ping &> /dev/null; do
        if [ $timeout -le 0 ]; then
...
run_migrations() {
    ...
    if docker exec cardstrategy_backend npm run migrate; then
        log_success "測試數據庫遷移完成"
    else
        log_error "測試數據庫遷移失敗"
        exit 1
    fi
...
run_tests() {
    ...
    # 運行單元測試
    log_info "運行單元測試..."
    if docker exec cardstrategy_backend npm run test:unit; then
        log_success "單元測試通過"
    else
        log_error "單元測試失敗"
        exit 1
    ...
    # 運行集成測試
    log_info "運行集成測試..."
    if docker exec cardstrategy_backend npm run test:integration; then
        log_success "集成測試通過"
    else
        log_error "集成測試失敗"
        exit 1
    ...
    # 運行 API 測試
    log_info "運行 API 測試..."
    if docker exec cardstrategy_backend npm run test:api; then
...
show_status() {
    ...
    echo ""
    echo "=== 測試環境特點 ==="
    echo "- 使用測試支付 API (Stripe Test, PayPal Sandbox)"
    echo "- 數據庫: cardstrategy_staging"
    echo "- 日誌級別: info"
    echo "- 錯誤追蹤: 啟用"
    echo "- 用戶分析: 禁用"
}
...
rollback() {
    ...
    if [ -f "$backup_file" ]; then
        log_info "恢復測試數據庫備份..."
        psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "${POSTGRES_DB:-cardstrategy_staging}" < "$backup_file"
    fi
