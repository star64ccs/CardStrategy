...
// 錯誤處理包裝模板函數
function createErrorHandlerWrapper(methodName, params, originalMethodBody, serviceName) {
  return `
  // 使用錯誤處理包裝器
...
function hasErrorHandlerImport(content) {
  return content.includes('errorHandler') || content.includes('withErrorHandling');
}
...
function addErrorHandlerImport(content) {
  ...
  return content.slice(0, insertIndex) + '\n' + errorHandlerImport + content.slice(insertIndex);
}
