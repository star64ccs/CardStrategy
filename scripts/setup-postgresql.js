...
function checkPostgreSQLService() {
  try {
    if (process.platform === 'win32') {
      ...
      // Linux/macOS
      const result = execSync('systemctl is-active postgresql', { encoding: 'utf8' }).trim();
      if (result === 'active') {
        console.log('✅ PostgreSQL 服務正在運行');
        return true;
      }
      console.log('❌ PostgreSQL 服務未運行');
      return false;
...
function setupDatabase() {
  ...
  try {
    // 檢查數據庫是否存在
    const dbExists = execSync('psql -U postgres -lqt | cut -d \| -f 1 | grep -qw cardstrategy', { encoding: 'utf8' });
    ...
    // 檢查用戶是否存在
    const userExists = execSync('psql -U postgres -t -c "SELECT 1 FROM pg_roles WHERE rolname=\'cardstrategy\'"', { encoding: 'utf8' });
