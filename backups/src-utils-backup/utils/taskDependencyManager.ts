...
export class TaskDependencyManager extends EventEmitter {
  ...
  isTaskReady(task: Task): boolean {
    ...
    return task.dependencies.every(dep => {
      ...
      // 根據依賴類型檢查狀態
      switch (
        dependentTask.dependencies.find(d => d.taskId === dep.taskId)?.type
      ) {
  ...
  private registerSyncOperations(): void {
    ...
    this.on('taskProgressUpdate', (data: any) => {
  ...
  private async updateTaskFromSync(syncData: TaskSyncData): Promise<void> {
    ...
    task.lastSyncTime = new Date(syncData.timestamp);
    task.isDirty = false;
  ...
  private async mergeTaskVersions(conflict: SyncConflict): Promise<void> {
    ...
    mergedTask.lastSyncTime = new Date();
   ...
   * 發送事件
   */
  private handleProgressUpdate(taskId: string, progress: ProgressUpdate): void {
    const task = this.tasks.get(taskId);
    if (task) {
