  ...
  private handleMessage(event: MessageEvent): void {
    ...
    switch (data.type) {
      case 'PERFORMANCE_METRICS':
        ...
        break;
      case 'CACHE_UPDATED':
        logger.info('[SW Manager] 緩存已更新:', {
          cacheName: data.cacheName as string,
        });
        break;
      case 'SYNC_COMPLETED':
...
export class ServiceWorkerManager {
  ...
  async prefetchResources(config: PrefetchConfig): Promise<void> {
    ...
    try {
      ...
      logger.info('[SW Manager] 預取請求已發送:', { urls: config.urls });
    } catch (error) {
  ...
  async clearCache(cacheName?: string): Promise<void> {
    ...
    try {
      ...
      logger.info('[SW Manager] 緩存清理請求已發送:', {
  ...
  addToSyncQueue(data: SyncData): void {
    ...
    logger.info('[SW Manager] 添加到同步隊列:', { id: data.id });
  }
    ...
    switch (feature) {
      ...
      case 'backgroundSync':
        return (
          ...
          'sync' in ((navigator.serviceWorker as any).registration || {})
        );
