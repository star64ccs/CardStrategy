...
class PublicDatasetService {
  ...
  // 整合數據集到本地數據庫
  async integrateDataset(
    datasetId: string
  ): Promise<ApiResponse<DatasetIntegrationResult>> {
    try {
      const response = await apiService.post<DatasetIntegrationResult>(
        `${API_ENDPOINTS.DATASET.INTEGRATE}/${datasetId}` ||
          `/dataset/integrate/${datasetId}`
      );
  ...
  async syncDataset(
    ...
    try {
      const response = await apiService.post<DatasetIntegrationResult>(
        `${API_ENDPOINTS.DATASET.SYNC}/${datasetId}` ||
          `/dataset/sync/${datasetId}`
      );
  ...
  > {
    try {
      const response = await apiService.get<{
        ...
        `${API_ENDPOINTS.DATASET.VALIDATE}/${datasetId}` ||
          `/dataset/validate/${datasetId}`
      );
