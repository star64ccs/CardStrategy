...
class AntiCounterfeitService {
  ...
  private async enhancedColorAnalysis(imageData: string): Promise<ColorScore> {
    try {
      ...
      return {
        ...
        brightness,
        overallScore,
        issues: this.generateColorIssues(
          colorHistogram,
          colorConsistency,
          colorGradient
        ),
  ...
  private calculateWeightedScore(apiResult: any, enhancedResult: any): number {
    ...
    return (
      apiResult.overallScore * apiWeight +
      enhancedResult.overallScore * enhancedWeight
    );
  }
      ...
      const report: AntiCounterfeitReport = {
        ...
        recommendations: {
          ...
          longTerm: analysis.recommendations.filter(
            r => r.includes('定期') || r.includes('監控') || r.includes('檢查')
          ),
          prevention: analysis.recommendations.filter(
            r => r.includes('建議') || r.includes('保持') || r.includes('正常')
          ),
        },
