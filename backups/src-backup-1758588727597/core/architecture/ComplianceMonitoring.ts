...
export class ComplianceRuleEngine {
  ...
  evaluateEvent(event: ComplianceEvent): ComplianceEvent[] {
    ...
    const applicableRules = this.getAllRules().filter(rule => rule.enabled);

    for (const rule of applicableRules) {
      if (this.matchesRule(event, rule)) {
        const triggeredEvent = this.createTriggeredEvent(event, rule);
  ...
  private determineSeverity(
    ...
    const baseSeverity = event.severity;
...
export class ComplianceEventTracker {
  ...
  addEvent(event: ComplianceEvent): void {
    ...
    // 評估規則並創建觸發的事件
    const triggeredEvents = this.ruleEngine.evaluateEvent(event);
  ...
  updateEvent(eventId: string, updates: Partial<ComplianceEvent>): boolean {
    ...
    const updatedEvent = { ...event, ...updates, timestamp: new Date() };
...
export class ComplianceMonitoring {
  ...
  getComplianceReport(): ComplianceReport {
    const events = this.eventTracker.getAllEvents();
    const summary = {
  ...
  private generateRecommendations(
    ...
    // 檢查隱私違規
    const privacyViolations = events.filter(
  ...
  private calculateComplianceScore(events: ComplianceEvent[]): number {
    ...
    const maxPossibleWeight = events.length * 4; // 假設所有事件都是嚴重違規
