...
describe('ComplianceMonitoring', () => {
  ...
  describe('單例模式', () => {
    it('應該返回相同的實例', () => {
      const instance1 = ComplianceMonitoring.getInstance();
      const instance2 = ComplianceMonitoring.getInstance();
  ...
  describe('初始化', () => {
    ...
    it('應該處理初始化失敗', async () => {
      // 創建一個新的實例來測試初始化失敗
      (ComplianceMonitoring as any).instance = undefined;
      const newMonitoring = ComplianceMonitoring.getInstance();

      // Mock ruleEngine 的方法
      const mockRuleEngine = newMonitoring as any;
      ...
      const result = await newMonitoring.initialize();
  ...
  describe('報告生成', () => {
    ...
    it('應該生成合規報告', () => {
      const report = monitoring.getComplianceReport();
  ...
  describe('警報管理', () => {
    ...
    it('應該獲取未確認警報', () => {
      const unacknowledgedAlerts = monitoring.getUnacknowledgedAlerts();
    ...
    it('應該確認警報', () => {
      ...
      if (alerts.length > 0) {
        ...
        const result = monitoring.acknowledgeAlert(alertId, 'test_user');
        expect(result).toBe(true);
      }
    });
  });
  ...
  describe('審計日誌', () => {
    ...
    it('應該獲取審計日誌', () => {
      const auditLog = monitoring.getAuditLog();
      expect(Array.isArray(auditLog)).toBe(true);
    });

    it('應該導出審計日誌為 JSON', () => {
      const jsonExport = monitoring.exportAuditLog('json');
    ...
    it('應該導出審計日誌為 CSV', () => {
      ...
      const csvExport = monitoring.exportAuditLog('csv');
  ...
  describe('規則管理', () => {
    ...
    it('應該添加規則', () => {
      ...
      const rules = monitoring.getRules();
    ...
    it('應該刪除規則', () => {
      ...
      monitoring.addRule(rule);
      const result = monitoring.removeRule('test_rule_to_delete');
      expect(result).toBe(true);
    });
  });
});
...
describe('ComplianceRuleEngine', () => {
  ...
  describe('規則管理', () => {
    it('應該添加規則', () => {
      ...
      const retrievedRule = ruleEngine.getRule('test_rule_2');
    ...
    it('應該刪除規則', () => {
      ...
      ruleEngine.addRule(rule);
      const result = ruleEngine.removeRule('test_rule_3');
      expect(result).toBe(true);

      const deletedRule = ruleEngine.getRule('test_rule_3');
    ...
    it('應該按類別查詢規則', () => {
      ...
      const privacyRules = ruleEngine.getRulesByCategory(
        ComplianceCategory.PRIVACY
      );
      const securityRules = ruleEngine.getRulesByCategory(
    ...
    it('應該按司法管轄區查詢規則', () => {
      ...
      const euRules = ruleEngine.getRulesByJurisdiction('EU');
      const globalRules = ruleEngine.getRulesByJurisdiction('Global');
  ...
  describe('事件評估', () => {
    it('應該評估事件並觸發規則', () => {
      ...
      const triggeredEvents = ruleEngine.evaluateEvent(event);
...
describe('ComplianceEventTracker', () => {
  ...
  describe('事件管理', () => {
    it('應該添加事件', () => {
      ...
      const retrievedEvent = eventTracker.getEvent('test_event_5');
    ...
    it('應該更新事件', () => {
      ...
      eventTracker.addEvent(event);
      const result = eventTracker.updateEvent('test_event_6', {
        status: ComplianceEventStatus.INVESTIGATING,
      });
      expect(result).toBe(true);

      const updatedEvent = eventTracker.getEvent('test_event_6');
  ...
  describe('事件查詢', () => {
    ...
    it('應該按狀態查詢事件', () => {
      const openEvents = eventTracker.getEventsByStatus(
        ComplianceEventStatus.OPEN
      );
      const resolvedEvents = eventTracker.getEventsByStatus(
    ...
    it('應該按嚴重性查詢事件', () => {
      const highEvents = eventTracker.getEventsBySeverity(
        ComplianceSeverity.HIGH
      );
      const mediumEvents = eventTracker.getEventsBySeverity(
    ...
    it('應該按類型查詢事件', () => {
      const accessEvents = eventTracker.getEventsByType(
        ComplianceEventType.ACCESS_CONTROL
      );
      const dataEvents = eventTracker.getEventsByType(
    ...
    it('應該按司法管轄區查詢事件', () => {
      const globalEvents = eventTracker.getEventsByJurisdiction('Global');
      const euEvents = eventTracker.getEventsByJurisdiction('EU');
    ...
    it('應該查詢開放違規', () => {
      const openViolations = eventTracker.getOpenViolations();
  ...
  describe('警報管理', () => {
    it('應該獲取警報', () => {
      const alerts = eventTracker.getAlerts();
      expect(Array.isArray(alerts)).toBe(true);
    });

    it('應該獲取未確認警報', () => {
      const unacknowledgedAlerts = eventTracker.getUnacknowledgedAlerts();
      expect(Array.isArray(unacknowledgedAlerts)).toBe(true);
    });
    ...
    it('應該確認警報', () => {
      ...
      if (alerts.length > 0) {
        const alertId = alerts[0].id;
        const result = eventTracker.acknowledgeAlert(alertId, 'test_user');
        expect(result).toBe(true);
      }
    });
  });
...
describe('ComplianceAuditTrail', () => {
  ...
  describe('審計日誌管理', () => {
    it('應該添加審計事件', () => {
      ...
      const auditLog = auditTrail.getAuditLog();
    ...
    it('應該按日期範圍查詢審計日誌', () => {
      ...
      const startDate = new Date(Date.now() - 24 * 60 * 60 * 1000); // 1天前
      const endDate = new Date(Date.now() + 24 * 60 * 60 * 1000); // 1天後

      const filteredLog = auditTrail.getAuditLogByDateRange(startDate, endDate);
    ...
    it('應該按用戶查詢審計日誌', () => {
      ...
      const userLog = auditTrail.getAuditLogByUser('user123');
    ...
    it('應該按事件類型查詢審計日誌', () => {
      ...
      const auditEvents = auditTrail.getAuditLogByEventType(
