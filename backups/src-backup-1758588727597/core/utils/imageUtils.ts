...
export async function convertImagesToBase64(
  ...
  for (const file of files) {
    ...
    try {
      const base64 = await convertImageToBase64(file, options);
      results.push({
        success: true,
        data: base64,
...
export function base64ToBlob(base64: string, _mimeType = 'image/jpeg'): Blob {
  try {
    ...
    // byteCharacters 應該總是有效的，因為 atob 成功執行了
    ...
    try {
      ...
      // 在測試環境中，如果 Uint8Array 或 Blob 構造失敗，返回一個簡單的模擬 Blob
      if (process.env.NODE_ENV === 'test' || !process.env.NODE_ENV) {
