...
export class AppInitializer {
  ...
  async initializeApp(): Promise<AppInitializationResult> {
    ...
    try {
      ...
      const serviceResult = await serviceManager.initializeAll();
      ...
      const healthResults = await serviceManager.performHealthCheck();
      ...
      // 3. 生成服務報告
      const serviceReport = serviceManager.generateServiceReport();
      logger.info('服務初始化報告:', serviceReport);

      const endTime = new Date();
      const duration = performanceMonitor.endTimer('app_initialization');
  ...
  getAppStatus(): {
    ...
    const serviceStats = serviceManager.getServiceStatistics();
  ...
  async performAppHealthCheck(): Promise<{
    ...
    try {
      ...
      // 檢查服務健康狀態
      const serviceHealthResults = await serviceManager.performHealthCheck();
      const unhealthyServices = Array.from(serviceHealthResults.entries())
        .filter(([, isHealthy]) => !isHealthy)
        .map(([service]) => service);
  ...
  async generateDiagnosticReport(): Promise<{
    ...
    const healthCheck = await this.performAppHealthCheck();
    const serviceReport = serviceManager.generateServiceReport();
