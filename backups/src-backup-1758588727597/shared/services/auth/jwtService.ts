...
export class JWTService {
  ...
  generateToken(
    ...
    try {
      ...
      const accessTokenExp = now + this.parseExpiresIn(this.config.expiresIn);
      ...
      const accessToken = this.encodeToken(accessTokenPayload);
      const refreshToken = this.encodeToken(refreshTokenPayload);
  ...
  refreshToken(refreshToken: string): ApiResponse<JWTToken> {
    try {
      ...
      // 生成新的令牌對
      const newTokenResult = this.generateToken({
        userId: payload.userId,
        email: payload.email,
