...
export class AuthService {
  ...
  async biometricLogin(request?: BiometricAuthRequest): Promise<AuthResponse> {
    try {
      ...
      // 執行生物識別認證
      const biometricResult = await biometricAuthService.authenticate(request);
  ...
  async disableBiometricAuth(): Promise<boolean> {
    try {
      ...
      // 向服務器註銷生物識別
      const response = await api.delete('/auth/biometric/unregister');
  ...
  async checkBiometricAuthStatus(): Promise<{
    ...
    try {
      ...
      if (this.isAuthenticated) {
        try {
          const response = await api.get('/auth/biometric/status');
          isRegistered =
  ...
  async quickLogin(): Promise<AuthResponse> {
    try {
      ...
      // 首先嘗試生物識別登錄
      const biometricStatus = await this.checkBiometricAuthStatus();
