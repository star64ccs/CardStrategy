...
export class GeminiService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const apiKey = serviceConfig.get('GOOGLE_GEMINI_API_KEY');
      if (!apiKey) {
  ...
  async authenticateCard(
    ...
    try {
      ...
      const result = await this.multiModalChat(messages);

      // 解析結果（這裡可以添加更複雜的解析邏輯）
      const isAuthentic = result.includes('真品') || result.includes('正品');
      const confidence = this.extractConfidence(result);
      const reasons = this.extractReasons(result);
      const recommendation = this.extractRecommendation(result);
