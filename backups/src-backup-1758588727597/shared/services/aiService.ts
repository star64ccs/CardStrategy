...
export class AIService {
  ...
  async analyzeCardImage(
    ...
    try {
      ...
      if (
        ...
        const response = await cohereService.generateText(
        ...
        if (response.success && response.data) {
          // 等待預測完成
          const finalResult = await replicateService.waitForPrediction(
            response.data.id
          );
  ...
  async generateInvestmentAdvice(
    ...
    try {
      ...
      if (
        ...
        const prompt = this.buildInvestmentPrompt(cardData);
        ...
        const prompt = this.buildInvestmentPrompt(cardData);
        const response = await cohereService.generateText(prompt, 300, 0.7);
  ...
  async chat(
    ...
    try {
      ...
      if (
        ...
        const response = await cohereService.generateText(prompt, 200, 0.7);
        return response.success && response.data
          ? response.data
          : '無法生成回應';
      } else if (provider === 'replicate' && replicateService.isAvailable()) {
  ...
  async authenticateCard(
    ...
    try {
      ...
      if (serviceManager.isServiceAvailable('gemini')) {
        const frontImageData = await this.fetchImageAsBase64(frontImageUrl);
        const backImageData = await this.fetchImageAsBase64(backImageUrl);
  ...
  private calculateConfidence(analysisText: string): number {
    ...
    // 關鍵詞因子
    const keywords = ['卡牌', '稀有', '價格', '狀況', '投資', '市場'];
