...
export class MixelService {
  ...
  async trackEvent(event: MixelEvent): Promise<ApiResponse> {
    try {
      ...
      const response = await api.post(`${this.baseUrl}/events`, {
  ...
  async identifyUser(user: MixelUser): Promise<ApiResponse> {
    try {
      ...
      const response = await api.post(`${this.baseUrl}/users`, {
        project_token: this.projectToken,
        api_secret: this.apiSecret,
        ...user,
      });
  ...
  async trackUserSignUp(
    ...
    const identifyResult = await this.identifyUser({
      userId,
      properties: {
...
export const mixelService = new MixelService();
