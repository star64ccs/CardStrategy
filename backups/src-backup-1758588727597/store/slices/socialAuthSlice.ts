...
import { _socialAuthService as socialAuthService } from '../../features/auth/services/socialAuthService';
...
export const socialLogin = createAsyncThunk(
  ...
  async (credentials: SocialLoginCredentials, { rejectWithValue }) => {
    try {
      const response = await socialAuthService.socialLogin(credentials);
      return response;
    } catch (error: unknown) {
      return rejectWithValue((error as any).message || '社交登錄失敗');
    }
  }
);
 ...
 * 獲取社交登錄 URL
 */
export const getSocialLoginUrl = createAsyncThunk(
  ...
  async (
    ...
    try {
      const url = await socialAuthService.getSocialLoginUrl(
        provider,
        redirectUri
      );
      return { provider, url };
    } catch (error: unknown) {
      return rejectWithValue((error as any).message || '獲取社交登錄 URL 失敗');
    }
  }
);
...
export const handleSocialCallback = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await socialAuthService.handleSocialCallback(
        provider,
        code,
        state
      );
      ...
      return rejectWithValue((error as any).message || '處理社交登錄回調失敗');
    }
  }
);
...
export const linkSocialAccount = createAsyncThunk(
  ...
  async (credentials: SocialLoginCredentials, { rejectWithValue }) => {
    try {
      const account = await socialAuthService.linkSocialAccount(credentials);
      return account;
    } catch (error: unknown) {
      return rejectWithValue((error as any).message || '鏈接社交帳戶失敗');
    }
  }
);
...
export const unlinkSocialAccount = createAsyncThunk(
  ...
  async (provider: SocialProvider, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue((error as any).message || '解除鏈接社交帳戶失敗');
    }
  }
);
 ...
 * 獲取已鏈接的社交帳戶
 */
export const getLinkedSocialAccounts = createAsyncThunk(
  'socialAuth/getLinkedSocialAccounts',
  async (_, { rejectWithValue }) => {
    try {
      const accounts = await socialAuthService.getLinkedSocialAccounts();
      return accounts;
    } catch (error: unknown) {
      return rejectWithValue(
        (error as any).message || '獲取已鏈接社交帳戶失敗'
      );
    }
  }
);
 ...
 * 檢查社交帳戶是否已鏈接
 */
export const checkSocialAccountLinked = createAsyncThunk(
  'socialAuth/checkSocialAccountLinked',
  async (provider: SocialProvider, { rejectWithValue }) => {
    try {
      const isLinked = await socialAuthService.isSocialAccountLinked(provider);
      return { provider, isLinked };
    } catch (error: unknown) {
      return rejectWithValue(
        (error as any).message || '檢查社交帳戶鏈接狀態失敗'
      );
    }
  }
);
 ...
 * 獲取社交用戶信息
 */
export const getSocialUserInfo = createAsyncThunk(
  ...
  async (
    ...
    try {
      const userInfo = await socialAuthService.getSocialUserInfo(
        provider,
        accessToken
      );
      return { provider, userInfo };
    } catch (error: unknown) {
      return rejectWithValue((error as any).message || '獲取社交用戶信息失敗');
    }
  }
);
 ...
 * 檢查社交登錄配置
 */
export const checkSocialLoginConfig = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue((error as any).message || '檢查社交登錄配置失敗');
    }
  }
);
...
const socialAuthSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(linkSocialAccount.fulfilled, (state, action) => {
      // 將新鏈接的帳戶添加到列表中
      const existingIndex = state.linkedAccounts.findIndex(
...
// 導出選擇器
export const selectSocialAuth = (state: { socialAuth: SocialAuthState }) =>
  state.socialAuth;
export const selectIsSocialLoggingIn = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.isSocialLoggingIn;
export const selectSocialLoginError = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.socialLoginError;
export const selectSocialAuthResponse = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.socialAuthResponse;
export const selectLinkedAccounts = (state: { socialAuth: SocialAuthState }) =>
  state.socialAuth.linkedAccounts;
export const selectIsLoadingAccounts = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.isLoadingAccounts;
export const selectAccountsError = (state: { socialAuth: SocialAuthState }) =>
  state.socialAuth.accountsError;
export const selectSocialLoginUrls = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.socialLoginUrls;
export const selectIsLoadingUrls = (state: { socialAuth: SocialAuthState }) =>
  state.socialAuth.isLoadingUrls;
export const selectUrlsError = (state: { socialAuth: SocialAuthState }) =>
  state.socialAuth.urlsError;
export const selectConfiguredProviders = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.configuredProviders;
export const selectIsCheckingConfig = (state: {
  socialAuth: SocialAuthState;
}) => state.socialAuth.isCheckingConfig;
export const selectConfigError = (state: { socialAuth: SocialAuthState }) =>
