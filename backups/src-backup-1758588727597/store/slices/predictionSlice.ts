...
import { _predictionService as predictionService } from '../../features/ai/services/predictionService';
import type {
  PredictionError,
  PredictionOptions,
  PredictionRequest,
...
// 異步 Action
export const performPrediction = createAsyncThunk(
  'prediction/performPrediction',
  async (request: PredictionRequest, { rejectWithValue }) => {
    try {
      const result = await predictionService.performPrediction(request);
      return result;
    } catch (error) {
...
export const getPredictionHistory = createAsyncThunk(
  ...
  async (cardId: string, { rejectWithValue }) => {
    try {
      const history = await predictionService.getPredictionHistory(cardId);
      return history;
    } catch (error) {
      return rejectWithValue({
...
export const getPredictionStats = createAsyncThunk(
...
export const getPredictionOptions = createAsyncThunk(
  'prediction/getPredictionOptions',
  async (_, { rejectWithValue }) => {
    try {
      const options = await predictionService.getPredictionOptions();
      return options;
    } catch (error) {
      return rejectWithValue({
...
const predictionSlice = createSlice({
  ...
  reducers: {
    ...
    addPredictionToHistory: (
      ...
      const existingHistory = state.predictionHistory.find(
  ...
  extraReducers: builder => {
    ...
    builder
      ...
      .addCase(getPredictionHistory.fulfilled, (state, action) => {
        state.loading = false;
        if (action.payload) {
          const existingIndex = state.predictionHistory.findIndex(
            history => history.cardId === action.payload.cardId
          );
          if (existingIndex >= 0) {
...
export const selectPredictionHistory = (state: {
  prediction: PredictionState;
...
export const selectPredictionLoading = (state: {
  prediction: PredictionState;
...
export const selectPredictionOptions = (state: {
  prediction: PredictionState;
}) => state.prediction.options;

export const selectPredictionHistoryByCardId = (
...
export const selectTotalPredictions = (state: {
  prediction: PredictionState;
}) => state.prediction.predictionStats?.totalPredictions || 0;

export const selectTopPerformingCards = (state: {
  prediction: PredictionState;
}) => state.prediction.predictionStats?.topPerformingCards || [];

export const selectRecentPredictions = (state: {
  prediction: PredictionState;
}) => state.prediction.predictionStats?.recentPredictions || [];

export const selectModelPerformance = (state: {
  prediction: PredictionState;
