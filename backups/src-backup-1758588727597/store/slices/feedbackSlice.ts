...
export const initializeFeedbackService = createAsyncThunk(
  ...
  async (config?: Partial<FeedbackServiceConfig>) => {
    const service = FeedbackService.getInstance(config);
...
export const updateFeedback = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: Partial<FeedbackData> }) => {
    const service = FeedbackService.getInstance();
    const feedback = await service.updateFeedback(id, data);
    return feedback;
  }
);
...
export const fetchFeedbacks = createAsyncThunk(
  ...
  async ({
    ...
    const service = FeedbackService.getInstance();
    const result = await service.getFeedbacks(filters, sort, pagination);
    return result;
  }
);
...
export const createReport = createAsyncThunk(
  ...
  async (report: Omit<FeedbackReport, 'id' | 'generatedAt'>) => {
    const service = FeedbackService.getInstance();
    const newReport = await service.createReport(report);
    return newReport;
  }
);
...
// 同步數據
export const syncFeedbackData = createAsyncThunk(
  'feedback/syncData',
  async () => {
    const service = FeedbackService.getInstance();
    await service.sync();
...
// 清除緩存
export const clearFeedbackCache = createAsyncThunk(
  'feedback/clearCache',
  async () => {
    const service = FeedbackService.getInstance();
...
export const selectAnalytics = (state: { feedback: FeedbackState }) =>
  state.feedback.analytics;
export const selectNotifications = (state: { feedback: FeedbackState }) =>
  state.feedback.notifications;
export const selectReports = (state: { feedback: FeedbackState }) =>
  state.feedback.reports;
export const selectFilters = (state: { feedback: FeedbackState }) =>
  state.feedback.filters;
export const selectSort = (state: { feedback: FeedbackState }) =>
  state.feedback.sort;
export const selectPagination = (state: { feedback: FeedbackState }) =>
  state.feedback.pagination;
export const selectLoading = (state: { feedback: FeedbackState }) =>
  state.feedback.loading;
export const selectSubmitting = (state: { feedback: FeedbackState }) =>
  state.feedback.submitting;
export const selectSyncing = (state: { feedback: FeedbackState }) =>
  state.feedback.syncing;
export const selectError = (state: { feedback: FeedbackState }) =>
  state.feedback.error;
export const selectServiceConfig = (state: { feedback: FeedbackState }) =>
  state.feedback.serviceConfig;
export const selectIsInitialized = (state: { feedback: FeedbackState }) =>
  state.feedback.isInitialized;
export const selectIsOnline = (state: { feedback: FeedbackState }) =>
  state.feedback.isOnline;
export const selectSyncStatus = (state: { feedback: FeedbackState }) =>
  state.feedback.syncStatus;

// 派生選擇器
export const selectFeedbackById = (
  state: { feedback: FeedbackState },
  id: string
) => state.feedback.feedbacks.find(f => f.id === id);

export const selectFeedbacksByType = (
  state: { feedback: FeedbackState },
  type: string
) => state.feedback.feedbacks.filter(f => f.type === type);

export const selectFeedbacksByCategory = (
  state: { feedback: FeedbackState },
  category: string
) => state.feedback.feedbacks.filter(f => f.category === category);

export const selectFeedbacksByStatus = (
  state: { feedback: FeedbackState },
  status: string
...
export const selectTotalFeedbacks = (state: { feedback: FeedbackState }) =>
  state.feedback.feedbacks.length;
export const selectTotalNotifications = (state: { feedback: FeedbackState }) =>
  state.feedback.notifications.length;
export const selectTotalReports = (state: { feedback: FeedbackState }) =>
