...
describe('CommunicationService', () => {
  ...
  describe('服務可用性檢查', () => {
    test('應該正確檢查可用渠道', () => {
      const channels = communicationService.getAvailableChannels();
  ...
  describe('用戶偏好設置', () => {
    test('應該正確設置用戶偏好', () => {
      ...
      const retrieved = communicationService.getUserPreferences('test-user-1');
    ...
    test('應該正確獲取用戶偏好', () => {
      ...
      communicationService.setUserPreferences(preferences);
      const retrieved = communicationService.getUserPreferences('test-user-2');
  ...
  describe('批量發送通知', () => {
    test('空通知列表應該返回成功', async () => {
      const result = await communicationService.sendBulkNotifications([]);
  ...
  describe('獲取服務統計', () => {
    test('應該返回正確的統計格式', async () => {
      const result = await communicationService.getServiceStats();
