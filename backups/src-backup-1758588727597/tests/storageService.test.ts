...
describe('Storage Services Tests', () => {
  ...
  describe('MockS3Service', () => {
    ...
    test('上傳文件應該成功', async () => {
      const result = await mockS3Service.uploadFile(
        'test-file.txt',
        Buffer.from('test')
      );
    ...
    test('下載文件應該成功', async () => {
      const result = await mockS3Service.downloadFile('test-file.txt');
      expect(result.success).toBe(true);
    ...
    test('刪除文件應該成功', async () => {
      const result = await mockS3Service.deleteFile('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.message).toBe('File deleted successfully');
    });

    test('列出文件應該成功', async () => {
      const result = await mockS3Service.listFiles();
      expect(result.success).toBe(true);
    ...
    test('獲取文件URL應該成功', async () => {
      const result = await mockS3Service.getFileUrl('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.data).toContain('s3.amazonaws.com');
    });

    test('複製文件應該成功', async () => {
      const result = await mockS3Service.copyFile('source.txt', 'dest.txt');
      expect(result.success).toBe(true);
      expect(result.data?.key).toBe('copied-file.txt');
    });

    test('獲取文件元數據應該成功', async () => {
      const result = await mockS3Service.getFileMetadata('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.data?.key).toBe('test-file.txt');
    });

    test('獲取服務統計應該成功', async () => {
      const result = await mockS3Service.getServiceStats();
      expect(result.success).toBe(true);
  ...
  describe('MockCloudflareStorageService', () => {
    ...
    test('上傳文件應該成功', async () => {
      const result = await mockCloudflareService.uploadFile(
        'test-file.txt',
        Buffer.from('test')
      );
      expect(result.success).toBe(true);
    ...
    test('下載文件應該成功', async () => {
      const result = await mockCloudflareService.downloadFile('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.data).toBeInstanceOf(Buffer);
      expect(result.data?.toString()).toBe('test content');
    });
    ...
    test('刪除文件應該成功', async () => {
      const result = await mockCloudflareService.deleteFile('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.message).toBe('File deleted successfully');
    });

    test('列出文件應該成功', async () => {
      const result = await mockCloudflareService.listFiles();
      expect(result.success).toBe(true);
      expect(result.data).toHaveLength(1);
      expect(result.data?.[0].key).toBe('test-file.txt');
    });
    ...
    test('獲取文件URL應該成功', async () => {
      const result = await mockCloudflareService.getFileUrl('test-file.txt');
      expect(result.success).toBe(true);
      expect(result.data).toContain('cloudflare.com');
    });

    test('複製文件應該成功', async () => {
      const result = await mockCloudflareService.copyFile(
        'source.txt',
        'dest.txt'
      );
      expect(result.success).toBe(true);
    ...
    test('獲取服務統計應該成功', async () => {
      const result = await mockCloudflareService.getServiceStats();
      expect(result.success).toBe(true);
