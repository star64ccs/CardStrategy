...
describe('CohereService', () => {
  ...
  describe('文本嵌入測試', () => {
    test('應該能夠生成文本嵌入向量', async () => {
      const texts = ['這是一張遊戲卡牌', '卡牌收藏很有價值'];

      const result = await cohereService.embedTexts(texts);
    ...
    test('應該處理單個文本', async () => {
      const text = '單張卡牌測試';
      const result = await cohereService.embedTexts([text]);
  ...
  describe('語義搜索測試', () => {
    test('應該能夠進行語義搜索', async () => {
      ...
      const result = await cohereService.semanticSearch(query, documents, 3);
  ...
  describe('文本生成測試', () => {
    test('應該能夠生成文本', async () => {
      const prompt = '請介紹一下卡牌收藏的樂趣';

      const result = await cohereService.generateText(prompt, 100, 0.7);
    ...
    test('應該處理不同的生成參數', async () => {
      ...
      const result1 = await cohereService.generateText(prompt, 50, 0.3);
      const result2 = await cohereService.generateText(prompt, 100, 0.8);
  ...
  describe('文本分類測試', () => {
    test('應該能夠進行文本分類', async () => {
      ...
      const result = await cohereService.classifyText(text, examples);
  ...
  describe('文本摘要測試', () => {
    test('應該能夠生成文本摘要', async () => {
      ...
      const result = await cohereService.summarizeText(
        text,
        'short',
        'paragraph'
      );
    ...
    test('應該支持不同的摘要格式', async () => {
      const text = '卡牌收藏很有趣。稀有卡牌有價值。投資潛力很大。';

      const paragraphResult = await cohereService.summarizeText(
        text,
        'medium',
        'paragraph'
      );
      const bulletsResult = await cohereService.summarizeText(
        text,
        'medium',
        'bullets'
      );
  ...
  describe('批量處理測試', () => {
    test('應該能夠批量處理文本嵌入', async () => {
      ...
      const result = await cohereService.batchProcess(texts, 'embed', {
    ...
    test('應該處理批量文本生成', async () => {
      const texts = ['卡牌收藏', '投資價值', '稀有度'];

      const result = await cohereService.batchProcess(texts, 'summarize', {
        length: 'short',
      });
  ...
  describe('錯誤處理測試', () => {
    test('應該處理無效的 API 密鑰', async () => {
      // 暫時修改 API 密鑰來測試錯誤處理
      const originalKey = serviceConfig.get('COHERE_API_KEY');
      serviceConfig.set('COHERE_API_KEY', 'invalid_key');

      const result = await cohereService.embedTexts(['測試文本']);
  ...
  describe('性能測試', () => {
    ...
    test('應該能夠處理大量文本', async () => {
      const texts = Array.from({ length: 20 }, (_, i) => `測試文本 ${i + 1}`);
