...
class MockRBACService {
  ...
  async createPermission(permission: unknown) {
    if (this.isInitialized) {
      ...
      const newPermission = { ...permission, id, createdAt: new Date() };
  ...
  async assignRole(
    ...
    if (this.isInitialized && this.roles.has(roleId)) {
      ...
      // 檢查角色數量限制（假設最大5個）
      const activeRoles = userRoles.filter((ur: unknown) => ur.isActive);
  ...
  async checkAccess(request: unknown) {
    if (this.isInitialized) {
      ...
      const roleNames = userRoles.map((ur: unknown) => {
      ...
      // 檢查權限
      const hasPermission = userPermissions.some((permission: unknown) => {
...
describe('RBAC Service Tests', () => {
  ...
  describe('MockRBACService', () => {
    test('初始化應該成功', async () => {
      const result = await mockRBACService.initialize();
    ...
    test('創建權限應該成功', async () => {
      ...
      const result = await mockRBACService.createPermission(permission);
      expect(result.success).toBe(true);
    ...
    test('更新權限應該成功', async () => {
      ...
      const createResult = await mockRBACService.createPermission(permission);
      const permissionId = createResult.data?.id;

      const updateResult = await mockRBACService.updatePermission(
        permissionId,
        {
          description: '更新的描述',
        }
    ...
    test('刪除權限應該成功', async () => {
      ...
      const deleteResult = await mockRBACService.deletePermission(permissionId);
    ...
    test('創建角色應該成功', async () => {
      ...
      const result = await mockRBACService.createRole(role);
      expect(result.success).toBe(true);
    ...
    test('更新角色應該成功', async () => {
      ...
      const createResult = await mockRBACService.createRole(role);
      const roleId = createResult.data?.id;

      const updateResult = await mockRBACService.updateRole(roleId, {
        description: '更新的角色描述',
      });
    ...
    test('更新系統角色權限應該失敗', async () => {
      const updateResult = await mockRBACService.updateRole('role1', {
        permissions: ['perm1', 'perm2'],
      });
    ...
    test('刪除系統角色應該失敗', async () => {
      const deleteResult = await mockRBACService.deleteRole('role1');
      expect(deleteResult.success).toBe(false);
      expect(deleteResult.error).toBe('Cannot delete system role');
    });

    test('分配角色應該成功', async () => {
      const result = await mockRBACService.assignRole(
        'user1',
        'role2',
        'admin1'
      );
    ...
    test('撤銷角色應該成功', async () => {
      ...
      const result = await mockRBACService.revokeRole(
        'user1',
        'role3',
        'admin1'
      );
    ...
    test('權限檢查應該正確', async () => {
      ...
      // 檢查用戶權限
      const result = await mockRBACService.checkAccess({
        userId: 'user1',
        resource: 'user',
        action: 'read',
      });
    ...
    test('獲取用戶角色應該正確', async () => {
      ...
      const roles = mockRBACService.getUserRoles('user1');
    ...
    test('獲取用戶活躍角色應該正確', async () => {
      ...
      const activeRoles = mockRBACService.getUserActiveRoles('user1');
    ...
    test('獲取用戶權限應該正確', async () => {
      ...
      const permissions = mockRBACService.getUserPermissions('user1');
    ...
    test('獲取所有角色應該正確', () => {
      const roles = mockRBACService.getAllRoles();
    ...
    test('獲取所有權限應該正確', () => {
      const permissions = mockRBACService.getAllPermissions();
  ...
  describe('錯誤處理測試', () => {
    test('未初始化服務應該返回錯誤', async () => {
      const uninitializedService = new MockRBACService();
      const result = await uninitializedService.createPermission({
        name: 'test:read',
        description: '測試權限',
        resource: 'test',
        action: 'read',
    ...
    test('刪除被使用的權限應該失敗', async () => {
      // perm3 被 guest 角色使用
      const result = await mockRBACService.deletePermission('perm3');
      expect(result.success).toBe(false);
    ...
    test('RBAC 基本功能測試', async () => {
      ...
      // 測試角色分配
      const assignResult = await mockRBACService.assignRole(
        'test-user',
        'role2',
        'admin1'
      );
      ...
      // 測試權限檢查
      const accessResult = await mockRBACService.checkAccess({
        userId: 'test-user',
        resource: 'user',
        action: 'read',
      });
