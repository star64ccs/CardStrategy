...
class MockTwoFactorService {
  ...
  async setupTOTP(userId: string, userEmail: string) {
    if (this.isInitialized) {
      const secret = 'TESTTOTPSECRET12345678901234567890';
      const qrCode = `otpauth://totp/CardStrategy:${userEmail}?secret=${secret}`;
  ...
  async verifyCode(verification: unknown) {
    if (this.isInitialized) {
      ...
      if (method === 'totp' && /^\d{6}$/.test(code)) {
        const userMethods = this.userMethods.get(userId);
        const totpMethod = userMethods?.find((m: unknown) => m.type === 'totp');
        if (totpMethod) {
      ...
      if (method === 'sms' && /^\d{6}$/.test(code)) {
        const userMethods = this.userMethods.get(userId);
        const smsMethod = userMethods?.find((m: unknown) => m.type === 'sms');
        if (smsMethod) {
      ...
      if (method === 'email' && /^\d{6}$/.test(code)) {
        const userMethods = this.userMethods.get(userId);
        const emailMethod = userMethods?.find((m: unknown) => m.type === 'email');
        if (emailMethod) {
  ...
  async enable2FA(userId: string, methodType: string) {
    if (this.isInitialized) {
      const userMethods = this.userMethods.get(userId);
      const method = userMethods?.find((m: unknown) => m.type === methodType);
  ...
  async regenerateBackupCodes(userId: string) {
    if (this.isInitialized) {
      const newCodes = ['11111111', '22222222', '33333333'];
...
describe('Two-Factor Authentication Service Tests', () => {
  ...
  describe('MockTwoFactorService', () => {
    test('初始化應該成功', async () => {
      const result = await mockTwoFactorService.initialize();
    ...
    test('設置 TOTP 應該成功', async () => {
      const result = await mockTwoFactorService.setupTOTP(
        'test-user',
        'test@example.com'
      );
      expect(result.success).toBe(true);
    ...
    test('設置 SMS 應該成功', async () => {
      const result = await mockTwoFactorService.setupSMS(
        'test-user',
        '+1234567890'
      );
      expect(result.success).toBe(true);
    ...
    test('設置郵件應該成功', async () => {
      const result = await mockTwoFactorService.setupEmail(
        'test-user',
        'test@example.com'
      );
      expect(result.success).toBe(true);
    ...
    test('驗證 TOTP 代碼應該成功', async () => {
      ...
      // 驗證代碼
      const result = await mockTwoFactorService.verifyCode({
        method: 'totp',
        code: '123456',
        userId: 'test-user',
      });
    ...
    test('啟用雙因素認證應該成功', async () => {
      ...
      // 啟用 2FA
      const result = await mockTwoFactorService.enable2FA('test-user', 'totp');
      expect(result.success).toBe(true);
    ...
    test('禁用雙因素認證應該成功', async () => {
      ...
      // 禁用 2FA
      const result = await mockTwoFactorService.disable2FA('test-user', 'totp');
      expect(result.success).toBe(true);
    ...
    test('獲取備用碼應該成功', async () => {
      ...
      // 獲取備用碼
      const result = await mockTwoFactorService.getBackupCodes('test-user');
      expect(result.success).toBe(true);
    ...
    test('獲取用戶方法應該正確', () => {
      ...
      const methods = mockTwoFactorService.getUserMethods('test-user');
  ...
  describe('錯誤處理測試', () => {
    test('未初始化服務應該返回錯誤', async () => {
      const uninitializedService = new MockTwoFactorService();
      const result = await uninitializedService.setupTOTP(
        'test-user',
        'test@example.com'
      );
