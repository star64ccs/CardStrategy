...
class MockRateLimitService {
  ...
  async checkLimit(request: unknown) {
    ...
    const remaining = Math.max(0, matchingRule.maxRequests - clientData.count);
    const allowed = clientData.count < matchingRule.maxRequests;
  ...
  async getLimitStatus(clientId: string, endpoint?: string, method?: string) {
    ...
    // 全局狀態
    if (this.config.enableGlobalLimit) {
      const globalData = this.globalCounts.get(clientId);
      if (!globalData || globalData.resetTime <= now) {
        statuses.push({
          limit: this.config.globalMaxRequests,
    ...
    if (endpoint && method) {
      const matchingRule = this.findMatchingRule(endpoint, method);
      if (matchingRule) {
        const ruleCounts = this.requestCounts.get(matchingRule.id);
...
describe('Rate Limit Service Tests', () => {
  ...
  describe('MockRateLimitService', () => {
    test('初始化應該成功', async () => {
      const result = await mockRateLimitService.initialize();
    ...
    test('創建規則應該成功', async () => {
      ...
      const result = await mockRateLimitService.createRule(rule);
      expect(result.success).toBe(true);
    ...
    test('更新規則應該成功', async () => {
      ...
      const createResult = await mockRateLimitService.createRule(rule);
      const ruleId = createResult.data?.id;

      const updateResult = await mockRateLimitService.updateRule(ruleId, {
        maxRequests: 20,
      });
    ...
    test('刪除規則應該成功', async () => {
      ...
      const deleteResult = await mockRateLimitService.deleteRule(ruleId);
    ...
    test('速率限制檢查應該正確', async () => {
      ...
      const result = await mockRateLimitService.checkLimit(request);
      expect(result.success).toBe(true);
    ...
    test('記錄請求應該成功', async () => {
      ...
      const result = await mockRateLimitService.recordRequest(request);
      expect(result.success).toBe(true);
    });
    ...
    test('獲取限制狀態應該正確', async () => {
      ...
      const result = await mockRateLimitService.getLimitStatus(
        clientId,
        endpoint,
        method
      );
    ...
    test('重置限制應該成功', async () => {
      ...
      // 重置限制
      const result = await mockRateLimitService.resetLimit(clientId);
      expect(result.success).toBe(true);
      expect(result.message).toBe('Limit reset');
    });

    test('獲取所有規則應該正確', () => {
      const rules = mockRateLimitService.getAllRules();
    ...
    test('全局限制應該生效', async () => {
      ...
      const result = await mockRateLimitService.checkLimit({
        endpoint: '/api/test',
        method: 'GET',
        clientId,
      });
    ...
    test('不同客戶端應該有獨立的限制', async () => {
      ...
      // 客戶端1應該被限制
      const result1 = await mockRateLimitService.checkLimit(request1);
      expect(result1.data?.allowed).toBe(false);

      // 客戶端2應該仍然可以訪問
      const result2 = await mockRateLimitService.checkLimit(request2);
  ...
  describe('錯誤處理測試', () => {
    test('未初始化服務應該返回錯誤', async () => {
      const uninitializedService = new MockRateLimitService();
      const result = await uninitializedService.createRule({
        name: '測試規則',
        endpoint: '/api/test',
        method: 'GET',
        windowMs: 60000,
    ...
    test('更新不存在的規則應該失敗', async () => {
      const result = await mockRateLimitService.updateRule('nonexistent-rule', {
        maxRequests: 20,
      });
    ...
    test('刪除不存在的規則應該失敗', async () => {
      const result = await mockRateLimitService.deleteRule('nonexistent-rule');
      expect(result.success).toBe(false);
      expect(result.error).toBe('Rule not found');
    });
  });
