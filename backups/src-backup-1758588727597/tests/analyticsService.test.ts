...
describe('Analytics Services', () => {
  ...
  describe('SegmentService', () => {
    ...
    test('空事件列表應該返回成功', async () => {
      const result = await segmentService.batchTrackEvents([]);
    ...
    test('追蹤頁面瀏覽應該返回適當響應', async () => {
      const result = await segmentService.trackPageView('test-page', {
        test: 'property',
      });
    ...
    test('追蹤用戶註冊應該返回適當響應', async () => {
      const result = await segmentService.trackUserSignUp('test-user', {
        plan: 'premium',
      });
      expect(result.success).toBe(false);
      expect(result.message).toContain('Segment service not available');
    ...
    test('追蹤卡牌事件應該返回適當響應', async () => {
      const result = await segmentService.trackCardEvent(
        'card_viewed',
        'card-123',
        { rarity: 'rare' }
      );
    ...
    test('獲取服務統計應該返回正確信息', async () => {
      const result = await segmentService.getServiceStats();
      expect(result.success).toBe(true);
  ...
  describe('MixelService', () => {
    ...
    test('空事件列表應該返回成功', async () => {
      const result = await mixelService.batchTrackEvents([]);
      expect(result.success).toBe(true);
      expect(result.data).toEqual([]);
      expect(result.message).toContain('No events to track');
      expect(result.timestamp).toBeInstanceOf(Date);
    ...
    test('追蹤頁面瀏覽應該返回適當響應', async () => {
      const result = await mixelService.trackPageView({
        page: 'test-page',
        userId: 'test-user',
    ...
    test('追蹤轉換應該返回適當響應', async () => {
      const result = await mixelService.trackConversion({
        event: 'purchase',
        userId: 'test-user',
        value: 100,
        currency: 'USD',
    ...
    test('獲取分析報告應該返回適當響應', async () => {
      const result = await mixelService.getAnalyticsReport(
    ...
    test('追蹤投資事件應該返回適當響應', async () => {
      const result = await mixelService.trackInvestmentEvent(
        'investment_made',
        500,
        { card_id: 'card-123' }
      );
    ...
    test('獲取服務統計應該返回正確信息', async () => {
      const result = await mixelService.getServiceStats();
      expect(result.success).toBe(true);
  ...
  describe('AnalyticsService', () => {
    test('應該正確獲取可用平台', () => {
      const platforms = analyticsService.getAvailablePlatforms();
    ...
    test('識別用戶應該返回適當響應', async () => {
      const result = await analyticsService.identifyUser({
        userId: 'test-user',
    ...
    test('追蹤用戶註冊應該返回適當響應', async () => {
      const result = await analyticsService.trackUserSignUp('test-user', {
        plan: 'premium',
      });
      expect(result.success).toBe(false);
      expect(result.message).toContain('No available analytics platforms');
    ...
    test('獲取服務統計應該返回正確信息', async () => {
      const result = await analyticsService.getServiceStats();
      expect(result.success).toBe(true);
