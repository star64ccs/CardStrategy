...
class ErrorHandlingService {
  ...
  async handleError(
    ...
    const appError = this.errorHandler.handleError(error, context);
  ...
  private recordErrorCount(): void {
    ...
    // 增加當前時間窗口的計數
    const currentCount = this.errorCounts.get(now) || 0;
  ...
  getErrorMetrics(): ErrorMetrics {
    ...
    const errorRate = totalCount / (this.errorRateWindow / 60000); // 每分鐘錯誤率
  ...
  exportErrorReports(format: 'json' | 'csv' = 'json'): string {
    ...
    // 確保所有屬性都可序列化
    const serializableReports = this.errorReports.map(report => ({
      id: report.id,
      context: report.context,
