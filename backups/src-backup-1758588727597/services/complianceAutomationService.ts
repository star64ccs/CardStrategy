...
export class ComplianceAutomationService {
  ...
  private async performRegulationMonitoring(): Promise<void> {
    try {
      ...
      // 檢查新的法規變化
      const newChanges = await this.checkForRegulationChanges();
  ...
  async performComplianceAssessment(): Promise<ComplianceAssessment> {
    ...
    // 收集當前合規狀態
    const currentResults = this.complianceResults.filter(
      result =>
    ...
    // 計算總體分數
    const totalScore = currentResults.reduce(
    ...
    // 統計問題
    const criticalIssues = currentResults.reduce(
      (count, result) =>
        count +
        result.issues.filter(issue => issue.severity === 'critical').length,
      0
    );
    const highPriorityIssues = currentResults.reduce(
      (count, result) =>
