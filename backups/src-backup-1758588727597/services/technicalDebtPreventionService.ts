...
export class TechnicalDebtPreventionService {
  ...
  private performCodeQualityAnalysis(): void {
    ...
    // 檢測新的技術債務
    const newIssues = this.detectTechnicalDebt(currentMetrics);
  ...
  private updateTrendAnalysis(): void {
    const recentMetrics = this.qualityMetrics.slice(-30); // 最近30天
    const previousMetrics = this.qualityMetrics.slice(-60, -30); // 前30天
  ...
  private estimateResolutionTime(): number {
    const unresolvedIssues = this.detectionResults.filter(r => !r.resolved);
    const totalEffort = unresolvedIssues.reduce(
  ...
  autoDetectTechnicalDebt(): TechnicalDebtDetectionResult[] {
    ...
    // 添加更多自動檢測邏輯
    const additionalIssues = this.performAdvancedDetection();
  ...
  analyzeTechnicalDebtTrend(period = '30天'): TechnicalDebtTrendAnalysis {
    const recentAnalysis = this.trendAnalyses[this.trendAnalyses.length - 1];
  ...
  resolveTechnicalDebt(issueId: string): void {
    const issue = this.detectionResults.find(r => r.id === issueId);
    if (issue) {
      issue.resolved = true;
    }
  }
...
// 導出單例實例
export const technicalDebtPreventionService =
