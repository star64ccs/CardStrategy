...
class ResponsiveComponentServiceClass implements ResponsiveComponentService {
  ...
  async testComponent(
    ...
    for (const device of testConfig.devices) {
      for (const orientation of testConfig.orientations) {
        const breakpoint = this.getBreakpointFromWidth(device.width);
        ...
        // 執行響應式測試
        const testIssues = await this.runResponsiveTests(
          component,
          device,
          breakpoint
        );
  ...
  generateTestReport(results: ResponsiveTestResult[]): string {
    ...
    // 按組件分組
    const componentGroups = this.groupResultsByComponent(results);
    ...
    for (const [componentName, componentResults] of componentGroups) {
      ...
      // 失敗的測試
      const failedResults = componentResults.filter(r => !r.passed);
      ...
      // 性能報告
      const avgRenderTime = this.calculateAveragePerformance(
        componentResults,
        'renderTime'
      );
      const avgMemoryUsage = this.calculateAveragePerformance(
        componentResults,
        'memoryUsage'
      );
      const avgInteractionTime = this.calculateAveragePerformance(
        componentResults,
        'interactionTime'
      );
  ...
  trackPerformance(
    ...
    const key = `${componentName}-${breakpoint}`;
  ...
  private getBreakpointFromWidth(width: number): Breakpoint {
    const breakpoints = layoutService.getBreakpointConfig();
  ...
  private measureInteractionTime(
    ...
    const interactionTime = Math.random() * 100 + 20; // 20-120ms
  ...
  private async runResponsiveTests(
    ...
    if (component.responsive) {
      const responsiveProps = Object.entries(component.props).filter(
        ([_, value]) =>
    ...
    // 檢查性能
    const performance = this.getPerformanceReport(component.name);
    if (performance.summary[component.name]?.[breakpoint]) {
      const metrics = performance.summary[component.name][breakpoint];
  ...
  private groupResultsByComponent(
    ...
    const groups = new Map<string, ResponsiveTestResult[]>();
  ...
  private calculateMetrics(data: unknown[]): unknown {
    const renderTimes = data.map(d => d.renderTime || 0);
    const memoryUsages = data.map(d => d.memoryUsage || 0);
    const interactionTimes = data.map(d => d.interactionTime || 0);
...
// 創建單例實例
export const responsiveComponentService =
