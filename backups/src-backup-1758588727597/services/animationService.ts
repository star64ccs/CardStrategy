...
export class AnimationService implements AnimationServiceInterface {
  ...
  public createAnimation(config: AnimationConfig): string {
    ...
    // 應用偏好設置
    const finalConfig = this.applyPreferences(config);
  ...
  public async playAnimation(id: string): Promise<void> {
    ...
    try {
      if (animation.element) {
        ...
        const keyframes = this.generateKeyframes(animation.config);
        const options = this.generateAnimationOptions(animation.config);
  ...
  public async playAll(): Promise<void> {
    const promises = Array.from(this.animations.keys()).map(id =>
  ...
  public updateConfig(id: string, config: Partial<AnimationConfig>): void {
    ...
    const updatedConfig = { ...animation.config, ...config };
  ...
  private stopOldestAnimation(): void {
    const oldestId = this.animations.keys().next().value;
    if (oldestId) {
