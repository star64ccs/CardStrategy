...
export class MultiLayerStorageService {
  ...
  public async set<T>(
    ...
    try {
      ...
      // 創建存儲項目
      const item = await this.createStorageItem(key, data, options);
      ...
      // 根據策略選擇存儲層
      const layers = this.selectStorageLayers(item.priority, options.layer);
      ...
      // 檢查寫入結果
      const successfulLayers = results.filter(
  ...
  public async get<T>(
    ...
    try {
      ...
      // 確定搜索層的順序
      const searchLayers = this.getSearchLayers(options.layer);

      for (const layer of searchLayers) {
        try {
          const item = await this.readFromLayer<T>(layer, key);
          if (item) {
            // 檢查是否過期
  ...
  public async delete(
    ...
    try {
      // 從所有層刪除
      const layers = Object.values(StorageLayer);
      const results = await Promise.allSettled(
    ...
    try {
      ...
      for (const key of keys) {
        if (key.startsWith('storage_item_')) {
          const item = await StorageManager.get<StorageItem>(key);
    ...
    try {
      switch (layer) {
        ...
        case StorageLayer.CACHE:
          const cacheExpiry = item.expiresAt
  ...
  private async readFromLayer<T>(
    ...
    const storageKey = `storage_item_${key}`;
  ...
  private async updateAccessTime(
    ...
    try {
      const item = await this.readFromLayer(layer, key);
      if (item) {
  ...
  private async updateStats(): Promise<void> {
    const totalReads = this.operationStats.reads;
    const totalHits = this.operationStats.hits;
  ...
  private async cleanupLRU(): Promise<number> {
    ...
    // 獲取所有項目並按訪問時間排序
    const items = Array.from(this.memoryStorage.entries()).sort(
  ...
  private async shouldRejectDueToMemoryLimit(
    ...
    const currentSize = Array.from(this.memoryStorage.values()).reduce(
      (total, item) => total + item.metadata.size,
      0
    );
