...
describe('MultiLayerStorageService', () => {
  ...
  describe('getInstance', () => {
    it('should return the same instance', () => {
      const instance1 = MultiLayerStorageService.getInstance();
      const instance2 = MultiLayerStorageService.getInstance();
  ...
  describe('initialize', () => {
    ...
    it('should handle initialization errors gracefully', async () => {
      // 創建一個新的服務實例來測試錯誤情況
      const testService = MultiLayerStorageService.getInstance();
      ...
      try {
        const result = await testService.initialize(invalidConfig as any);
  ...
  describe('set and get operations', () => {
    ...
    it('should store and retrieve data successfully', async () => {
      ...
      const options = { layer: 'memory' };

      const setResult = await service.set(key, data, options);
      if (!setResult.success) {
        throw new Error(`Set failed: ${JSON.stringify(setResult)}`);
      }
      expect(setResult.success).toBe(true);

      const retrievedData = await service.get(key, options);
      if (!retrievedData) {
    ...
    it('should handle different data types', async () => {
      ...
      for (const testCase of testCases) {
        ...
        const retrievedData = await service.get(testCase.key, options);
    ...
    it('should store data with options', async () => {
      ...
      const retrievedData = await service.get(key, { layer: 'memory' });
      expect(retrievedData).toEqual(data);
    });

    it('should handle storage with specific layer', async () => {
      const key = 'layer-test';
      const data = { layer: 'memory' };
      const options: StorageOptions = {
    ...
    it('should handle TTL expiration', async () => {
      ...
      // 立即獲取應該成功
      const immediateResult = await service.get(key, { layer: 'memory' });
      expect(immediateResult).toEqual(data);

      // 等待過期後應該返回 null
      await new Promise(resolve => setTimeout(resolve, 100));
      const expiredResult = await service.get(key, { layer: 'memory' });
  ...
  describe('delete operations', () => {
    ...
    it('should delete data successfully', async () => {
      ...
      const beforeDelete = await service.get(key, options);
      expect(beforeDelete).toEqual(data);

      const deleteResult = await service.delete(key);
      expect(deleteResult.success).toBe(true);

      const afterDelete = await service.get(key, options);
      expect(afterDelete).toBeNull();
    });

    it('should handle deletion of non-existent keys', async () => {
      const deleteResult = await service.delete('non-existent-key');
      expect(deleteResult.success).toBe(true);
    });
  });
  ...
  describe('query operations', () => {
    ...
    it('should query data by namespace', async () => {
      ...
      const queryResult = await service.query({ namespace });
    ...
    it('should query data by tags', async () => {
      ...
      const queryResult = await service.query({ tags: ['test'] });
      expect(queryResult.length).toBeGreaterThanOrEqual(0);
    });
    ...
    it('should query data by priority', async () => {
      ...
      const queryResult = await service.query({ priority: DataPriority.HIGH });
      expect(queryResult.length).toBeGreaterThanOrEqual(0);
    });
  ...
  describe('cleanup operations', () => {
    ...
    it('should preserve critical data during cleanup', async () => {
      const criticalData = { critical: true };
      const options = {
        layer: 'memory',
      ...
      // 關鍵數據應該仍然存在
      const retrievedData = await service.get('critical-data', {
        layer: 'memory',
      });
  ...
  describe('error handling', () => {
    ...
    it('should handle storage errors gracefully', async () => {
      // 測試無效的鍵值
      const setResult = await service.set('', null);
    ...
    it('should handle large data gracefully', async () => {
      ...
      const setResult = await service.set('large-data', largeData);
      // 應該成功或失敗，但不應拋出異常
    ...
    it('should handle malformed data', async () => {
      ...
      const setResult = await service.set('malformed-data', malformedData);
      // 應該處理循環引用錯誤
      expect(typeof setResult.success).toBe('boolean');
    });
  });
  ...
  describe('destroy', () => {
    it('should destroy service instance', async () => {
      ...
      // 銷毀後數據應該不存在
      const retrievedData = await service.get('destroy-test');
