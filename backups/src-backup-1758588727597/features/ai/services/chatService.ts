...
export class ChatService {
  ...
  public async sendMessage(request: ChatRequest): Promise<ChatResponse> {
    ...
    try {
      ...
      // 保存用戶消息
      const messages = this.messageHistory.get(request.sessionId) || [];
      ...
      // 分析用戶意圖
      const intent = request.intent || this.analyzeIntent(request.message);
  ...
  private async generateResponse(
    ...
    const response = responses[intent] || responses[UserIntent.UNKNOWN];
  ...
  public async getSessionHistory(
    ...
    const userSatisfaction = 4.2; // 模擬用戶滿意度
