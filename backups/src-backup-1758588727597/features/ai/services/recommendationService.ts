...
class RecommendationService {
  ...
  public async generateRecommendation(
    ...
    try {
      ...
      // 生成卡牌建議
      const cardRecommendations = await this.generateCardRecommendations(
        request,
        userAnalysis
      );

      // 生成投資組合建議
      const portfolioSuggestion = await this.generatePortfolioSuggestion(
        request,
        cardRecommendations
      );

      // 進行風險分析
      const riskAnalysis = await this.analyzeInvestmentRisk(
        request,
        cardRecommendations
      );

      // 計算預期回報
      const expectedReturn = await this.calculateExpectedReturn(
        cardRecommendations,
        request.timeHorizon
      );
      ...
      // 創建結果
      const result = await this.createRecommendationResult(
        request,
        cardRecommendations,
        portfolioSuggestion,
        riskAnalysis,
  ...
  public async getRecommendationStats(): Promise<RecommendationStats> {
    const histories = Array.from(this.recommendationHistory.values());

    const totalRecommendations = histories.reduce(
      (sum, history) => sum + history.recommendations.length,
      0
    );

    const successRate = this.calculateOverallSuccessRate(histories);
    const averageReturn = this.calculateAverageReturn(histories);
  ...
  public async analyzePortfolio(
    ...
    try {
      ...
      const analysis = this.analyzeDiversification(currentPortfolio);
      const recommendations = this.generateRebalanceRecommendations(
        currentPortfolio,
        userProfile
      );
      const riskAssessment = this.assessPortfolioRisk(currentPortfolio);
  ...
  private async generateCardRecommendations(
    ...
    for (let i = 0; i < maxRecommendations; i++) {
      ...
      const recommendation = await this.createCardRecommendation(
        cardId,
        request,
        userAnalysis,
        i
  ...
  private async createCardRecommendation(
    ...
    // 確保價格在預算範圍內
    const maxPrice = Math.min(request.budget * 0.8, 1000); // 最多使用80%預算或1000，取較小值
    const minPrice = Math.min(100, request.budget * 0.1); // 最少10%預算或100，取較小值
    const basePrice = minPrice + Math.random() * (maxPrice - minPrice);
    const expectedReturn = this.calculateCardExpectedReturn(
      request.riskTolerance,
      request.timeHorizon
    );
    const riskLevel = this.determineCardRiskLevel(
      request.riskTolerance,
      index
    );
  ...
  private async generatePortfolioSuggestion(
    ...
    const totalValue = request.budget;
    const emergencyFund =
      totalValue * RECOMMENDATION_CONSTANTS.EMERGENCY_FUND_PERCENTAGE;
    const cashReserve =
      totalValue * RECOMMENDATION_CONSTANTS.CASH_RESERVE_PERCENTAGE;
    const investmentAmount = totalValue - emergencyFund - cashReserve;

    const allocation = this.generatePortfolioAllocation(
      cardRecommendations,
      investmentAmount
    );
    const diversification = this.analyzeDiversification(cardRecommendations);
  ...
  private async analyzeInvestmentRisk(
    ...
    const portfolioRisk = this.calculatePortfolioRisk(cardRecommendations);
    const concentrationRisk =
      this.calculateConcentrationRisk(cardRecommendations);
    const marketRisk = this.assessMarketRisk(request.marketConditions);
    const liquidityRisk = this.assessLiquidityRisk(cardRecommendations);
    ...
    const riskScore = this.calculateRiskScore2(overallRisk);
  ...
  private async calculateExpectedReturn(
    ...
    const optimistic = weightedReturn * 1.5;
    const realistic = weightedReturn;
    const pessimistic = weightedReturn * 0.5;
  ...
  private async saveRecommendationResult(
    ...
    const history = this.recommendationHistory.get(result.userId);
  ...
  private calculateInvestmentCapacity(profile: UserProfile): number {
    const totalAssets = profile.totalInvestment;
  ...
  private determineCardRiskLevel(
    ...
    const baseIndex = Math.floor(index / 3) % riskLevels.length;
  ...
  private determinePriority(
    ...
    const score = expectedReturn * 100 - index;
  ...
  private generatePortfolioAllocation(
    ...
    const categories = ['高成長', '穩定收益', '價值投資', '投機性'];
    const allocations: PortfolioAllocation[] = [];

    categories.forEach((category, index) => {
      const percentage = [0.4, 0.3, 0.2, 0.1][index]; // 預設分配比例
      const value = investmentAmount * percentage;
      const cards = cardRecommendations
  ...
  private calculatePortfolioRisk(
    ...
    const avgRisk =
      cardRecommendations.reduce((sum, card) => {
        const riskValue = this.getRiskLevelValue(card.riskLevel);
  ...
  private calculateConcentrationRisk(
    ...
    // 簡化的集中度風險計算
    const maxWeight = 1 / cardRecommendations.length;
  ...
  private annualizeReturn(
    ...
    const periodYears = years[timeHorizon] || 1.0;
  ...
  private calculateRiskAdjustedReturn(
    ...
    const riskPenalty = avgRisk * 0.02; // 2% penalty per risk level
