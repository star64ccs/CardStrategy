...
describe('ChatService', () => {
  ...
  describe('createSession', () => {
    it('should create a new session successfully', async () => {
      ...
      const category = ChatCategory.INVESTMENT;

      const session = await chatService.createSession(userId, category);
    ...
    it('should generate unique session IDs', async () => {
      ...
      const session1 = await chatService.createSession(userId);
      const session2 = await chatService.createSession(userId);
  ...
  describe('sendMessage', () => {
    ...
    it('should update session statistics after sending message', async () => {
      ...
      const updatedSession = await chatService.getSession(sessionId);
  ...
  describe('getSession', () => {
    it('should return session by ID', async () => {
      ...
      const createdSession = await chatService.createSession(userId);
  ...
  describe('getSessionHistory', () => {
    ...
    it('should return null for non-existent session', async () => {
      const history = await chatService.getSessionHistory('non-existent');
  ...
  describe('addSessionTag', () => {
    ...
    it('should not add duplicate tags', async () => {
      const userId = 'test-user-tag-duplicate';
      const session = await chatService.createSession(userId);
      const tag = 'duplicate';
      ...
      const updatedSession = await chatService.getSession(session.id);
      const tagCount = updatedSession.tags.filter(t => t === tag).length;
  ...
  describe('removeSessionTag', () => {
    it('should remove tag from session successfully', async () => {
      const userId = 'test-user-remove-tag';
      const session = await chatService.createSession(userId);
      const tag = 'to-remove';
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle special characters in message', async () => {
      ...
      const session = await chatService.createSession(userId);
      const specialMessage = '測試中文 🎴 💰 📈 特殊符號!@#$%^&*()';
