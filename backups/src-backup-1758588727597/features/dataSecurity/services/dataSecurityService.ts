...
export class DataSecurityService {
  ...
  public async encryptData(
    ...
    try {
      ...
      // 執行加密
      const result = await this.encryptionService.encrypt(request);
  ...
  public async decryptData(
    ...
    try {
      ...
      // 執行解密
      const result = await this.encryptionService.decrypt(request);
  ...
  public async createBackup(config: BackupConfig): Promise<BackupTask> {
    try {
      // 執行備份
      const task = await this.backupService.createBackup(config);
  ...
  public async restoreBackup(request: RestoreRequest): Promise<RestoreResult> {
    try {
      // 執行恢復
      const result = await this.backupService.restoreBackup(request);
  ...
  public async generateKey(
    ...
    try {
      // 生成密鑰
      const key = await this.keyManager.generateKey(algorithm, metadata);
  ...
  public async rotateKey(keyId: string): Promise<EncryptionKey> {
    try {
      // 輪換密鑰
      const newKey = await this.keyManager.rotateKey(keyId);
  ...
  public async getSecurityState(): Promise<SecurityState> {
    try {
      const activeKeys = await this.keyManager.listKeys({
        status: 'active' as any,
      });
      const backupTasks = await this.backupService.listBackups();
      const activeTasks = backupTasks.filter(
        task => task.status === 'in_progress'
      );
      const completedTasks = backupTasks.filter(
        task => task.status === 'completed'
      );

      // 獲取統計信息
      const keyStats = await this.keyManager.getKeyStatistics();
  ...
  private async checkBackupHealth(): Promise<void> {
    try {
      const tasks = await this.backupService.listBackups();
