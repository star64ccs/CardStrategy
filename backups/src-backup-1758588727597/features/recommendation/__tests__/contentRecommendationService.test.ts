...
describe('ContentRecommendationService', () => {
  ...
  describe('Singleton Pattern', () => {
    it('應該返回相同的實例', () => {
      const instance1 = ContentRecommendationService.getInstance();
      const instance2 = ContentRecommendationService.getInstance();
  ...
  describe('getSimilarContent', () => {
    ...
    it('應該返回相似內容列表', async () => {
      ...
      const limit = 5;
      const similarityMethod = SimilarityMethod.COSINE;
  ...
  describe('updateUserPreference', () => {
    ...
    it('應該成功更新用戶偏好', async () => {
      ...
      const result = await service.updateUserPreference(userId, preference);
      expect(result).toBeUndefined();
    });
  ...
  describe('recordUserInteraction', () => {
    ...
    it('應該成功記錄用戶互動', async () => {
      ...
      const result = await service.recordUserInteraction(
        userId,
        contentId,
        interaction
      );
  ...
  describe('Data Validation', () => {
    ...
    it('相似內容結果應該有正確的結構', async () => {
      const contentId = 'content_1';
      const similarities = await service.getSimilarContent(contentId, 3);
