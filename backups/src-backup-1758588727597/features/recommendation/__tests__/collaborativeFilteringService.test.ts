...
describe('CollaborativeFilteringService', () => {
  ...
  describe('Singleton Pattern', () => {
    test('應該返回相同的實例', () => {
      const instance1 = CollaborativeFilteringService.getInstance();
      const instance2 = CollaborativeFilteringService.getInstance();
  ...
  describe('Error Handling', () => {
    ...
    test('應該處理無效項目ID', async () => {
      ...
      const response = await service.getSimilarItems({
        itemId: 'invalid_item',
        limit: 5,
      });
    ...
    test('應該處理服務未初始化', async () => {
      const newService = CollaborativeFilteringService.getInstance();
      (newService as any).isInitialized = false;

      const response = await newService.getRecommendations({
        userId: 'user_1',
        limit: 5,
      });
  ...
  describe('Data Validation', () => {
    ...
    test('相似用戶結果應該有正確的結構', async () => {
      const response = await service.getSimilarUsers({
        userId: 'user_1',
        limit: 3,
      });

      if (response.success && response.data.similarUsers.length > 0) {
        const similarUser = response.data.similarUsers[0];
    ...
    test('相似項目結果應該有正確的結構', async () => {
      ...
      if (response.success && response.data.similarItems.length > 0) {
        const similarItem = response.data.similarItems[0];
