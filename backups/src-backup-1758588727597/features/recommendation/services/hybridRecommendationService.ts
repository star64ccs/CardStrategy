...
export class HybridRecommendationService {
  ...
  async getRecommendations(
    ...
    try {
      ...
      if (this.config.weights.collaborative > 0) {
        try {
          const response = await this.collaborativeService.getRecommendations({
      ...
      const contentRecommendations = { recommendations: [] as any[] };
      if (this.config.weights.content > 0) {
        try {
          // 暫時註釋掉，等待 ContentRecommendationService 實現
          // const response = await this.contentService.getRecommendations({
          //   userId: request.userId,
      ...
      // 限制數量
      const finalRecommendations = sortedRecommendations.slice(
        0,
        request.limit
      );
  ...
  private sortRecommendations(
    ...
    return recommendations.sort((a, b) => {
      const scoreA = this.calculateHybridScore(a, userId);
      const scoreB = this.calculateHybridScore(b, userId);
  ...
  private calculateHybridScore(
    ...
    const popularityScore = recommendation.popularity || 0;
    const recencyScore = recommendation.recency || 0;
    const relevanceScore = recommendation.relevance || 0;
    const diversityScore = recommendation.diversity || 0;
  ...
  private calculateNovelty(recommendations: unknown[]): number {
    ...
    const noveltyScores = recommendations.map(rec => rec.novelty || 0);
    const totalNovelty = noveltyScores.reduce((sum, score) => sum + score, 0);
