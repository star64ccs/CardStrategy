...
class ContentRecommendationService {
  ...
  public async getContentRecommendations(
    ...
    try {
      ...
      const filteredItems = this.applyFilters(contentItems, request.filters);
  ...
  public async getSimilarContent(
    ...
    try {
      const cachedSimilarities = this.getCachedSimilarities(contentId);
      if (cachedSimilarities && this.config.cacheEnabled) {
        return cachedSimilarities.slice(0, limit);
      }

      const contentItems = await this.getContentItems();
      const targetItem = contentItems.find(item => item.id === contentId);
  ...
  public async updateUserPreference(
    ...
    try {
      const existingPreference = await this.getUserPreference(userId);
  ...
  private calculateHybridScore(
    ...
    const contentScore = this.calculateContentBasedScore(item, userPreference);
    const tagScore = this.calculateTagBasedScore(item, userPreference);
    const categoryScore = this.calculateCategoryBasedScore(
      item,
      userPreference
    );
    const attributeScore = this.calculateAttributeBasedScore(
      item,
      userPreference
    );
    const popularityScore = this.calculatePopularityBasedScore(item);
    const trendingScore = this.calculateTrendingBasedScore(item);
  ...
  private calculateConfidence(score: number, item: ContentItem): number {
    ...
    const ratingConfidence = item.attributes.rating / 5;
    const reviewConfidence = Math.min(item.attributes.reviewCount / 100, 1);
  ...
  private calculateJaccardSimilarity(
    ...
    const sourceSet = new Set([...source.tags, source.category, source.type]);
    const targetSet = new Set([...target.tags, target.category, target.type]);
  ...
  private calculateEuclideanSimilarity(
    ...
    const tagDiff = Math.abs(source.tags.length - target.tags.length);
  ...
  private getCachedSimilarities(contentId: string): ContentSimilarity[] | null {
    const cached = this.cache.similarities[contentId];
    if (cached && cached.expiresAt > new Date()) {
