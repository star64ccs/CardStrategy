...
describe('AndroidBiometricService', () => {
  ...
  describe('初始化測試', () => {
    test('應該成功創建單例實例', () => {
      const instance1 = AndroidBiometricService.getInstance();
      const instance2 = AndroidBiometricService.getInstance();
    ...
    test('應該在非 Android 平台拋出錯誤', async () => {
      ...
      // 創建新實例會觸發初始化錯誤
      const newService = new (AndroidBiometricService as any)();
      await new Promise(resolve => setTimeout(resolve, 100));
  ...
  describe('生物識別認證測試', () => {
    ...
    test('應該處理設備不支持的情況', async () => {
      ...
      const originalLoadLibrary = mockService.loadAndroidBiometricLibrary;
    ...
    test('應該處理服務未初始化的情況', async () => {
      // 創建未初始化的服務
      const uninitializedService = new (AndroidBiometricService as any)();
  ...
  describe('簽名創建測試', () => {
    test('應該成功創建簽名', async () => {
      const promptMessage = '請進行簽名認證';
      const payload = 'test-payload-data';

      const result = await service.createSignature(promptMessage, payload);
    ...
    test('應該處理簽名創建失敗', async () => {
      ...
      const result = await service.createSignature('test', 'payload');
  ...
  describe('密鑰管理測試', () => {
    test('應該成功使密鑰失效', async () => {
      const result = await service.invalidateKeys();
    ...
    test('應該成功重新初始化密鑰', async () => {
      const result = await service.reinitializeKeys();
  ...
  describe('錯誤處理測試', () => {
    test('應該處理初始化錯誤', async () => {
      // 模擬初始化失敗
      const mockService = new (AndroidBiometricService as any)();
  ...
  describe('Android 特定功能測試', () => {
    test('應該支持 Android 特定的生物識別類型', async () => {
      const capabilities = await service.detectCapabilities();
      const androidTypes = capabilities.filter(
        cap =>
