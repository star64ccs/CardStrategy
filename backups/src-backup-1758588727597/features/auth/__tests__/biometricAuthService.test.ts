...
describe('BiometricAuthService', () => {
  ...
  describe('updateBiometricSettings', () => {
    ...
    it('應該合併現有設置', async () => {
      const currentSettings = await biometricAuthService.getBiometricSettings();
      const updates = { maxRetryAttempts: 10 };
  ...
  describe('錯誤處理', () => {
    it('應該處理初始化失敗', async () => {
      // 創建新實例來測試初始化失敗情況
      const service = new (BiometricAuthService as any)();
    ...
    it('應該正確映射錯誤代碼', () => {
      const service = biometricAuthService as any;
  ...
  describe('集成測試', () => {
    it('應該完成完整的生物識別流程', async () => {
      ...
      // 2. 創建密鑰
      const keyCreated = await biometricAuthService.createBiometricKeys();
      ...
      // 7. 刪除密鑰
      const keyDeleted = await biometricAuthService.deleteBiometricKeys();
