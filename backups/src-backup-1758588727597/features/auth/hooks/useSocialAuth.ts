...
export const useSocialAuth = (
  ...
  // 從 Redux 獲取狀態
  const isSocialLoggingIn = useSelector(selectIsSocialLoggingIn);
  const socialLoginError = useSelector(selectSocialLoginError);
  const socialAuthResponse = useSelector(selectSocialAuthResponse);
  const linkedAccounts = useSelector(selectLinkedAccounts);
  const isLoadingAccounts = useSelector(selectIsLoadingAccounts);
  const accountsError = useSelector(selectAccountsError);
  const socialLoginUrls = useSelector(selectSocialLoginUrls);
  const isLoadingUrls = useSelector(selectIsLoadingUrls);
  const urlsError = useSelector(selectUrlsError);
  const configuredProviders = useSelector(selectConfiguredProviders);
  const isCheckingConfig = useSelector(selectIsCheckingConfig);
  ...
  const login = useCallback(
    async (
      ...
      try {
        const response = await dispatch(socialLogin(credentials)).unwrap();
  ...
  // 加載已鏈接帳戶
  const loadLinkedAccounts = useCallback(async (): Promise<
    SocialAccountLink[]
  > => {
    try {
      const accounts = await dispatch(getLinkedSocialAccounts()).unwrap();
  ...
  // 檢查帳戶是否已鏈接
  const checkAccountLinked = useCallback(
  ...
  // 檢查配置
  const checkConfig = useCallback(async (): Promise<SocialProvider[]> => {
    try {
      const providers = await dispatch(checkSocialLoginConfig()).unwrap();
  ...
  // 清除帳戶錯誤
  const clearAccountsErrorInternal = useCallback(() => {
    dispatch(clearAccountsError());
  }, [dispatch]);

  // 清除 URL 錯誤
  const clearUrlsErrorInternal = useCallback(() => {
    dispatch(clearUrlsError());
  }, [dispatch]);

  // 清除配置錯誤
  const clearConfigErrorInternal = useCallback(() => {
  ...
  // 獲取提供商名稱
  const getProviderName = useCallback((provider: SocialProvider): string => {
