...
describe('ThemeService', () => {
  ...
  describe('主題自定義', () => {
    ...
    it('應該自定義主題', async () => {
      ...
      const themes = themeService.getAvailableThemes();
      const customTheme = themes.find(t => t.id === 'light_custom');
  ...
  describe('主題導入/導出', () => {
    ...
    it('應該導出主題', async () => {
      const themeData = await themeService.exportTheme('light');
    ...
    it('應該導入主題', async () => {
      ...
      const themes = themeService.getAvailableThemes();
      const importedTheme = themes.find(t => t.id === 'imported');
    ...
    it('應該在導入缺少必要字段的主題時拋出錯誤', async () => {
      const invalidTheme = JSON.stringify({ id: 'invalid' });
  ...
  describe('工具方法', () => {
    ...
    it('應該獲取主題間距', () => {
      const spacing = themeService.getThemeSpacing();
      expect(spacing.md).toBe(16);
      expect(spacing.lg).toBe(24);
    });

    it('應該獲取主題字體', () => {
      const typography = themeService.getThemeTypography();
      expect(typography.fontSize.md).toBe(16);
      expect(typography.fontSize.lg).toBe(18);
    });

    it('應該獲取主題圓角', () => {
      const borderRadius = themeService.getThemeBorderRadius();
      expect(borderRadius.md).toBe(8);
      expect(borderRadius.lg).toBe(12);
    });

    it('應該獲取主題陰影', () => {
      const shadows = themeService.getThemeShadows();
