...
class I18nService implements I18nManager, I18nTools {
  ...
  async detectLanguage(): Promise<string> {
    try {
      ...
      const languageCode = this.getLanguageFromLocale(
  ...
  formatRelativeTime(date: Date): string {
    ...
    const rtf = this.formatters.relativeTime.get(language)!;
  ...
  formatPlural(count: number, singular: string, plural: string): string {
    const language = this.state.currentLanguage;
    const rtf = new Intl.PluralRules(language);
    const rule = rtf.select(count);
    return rule === 'one' ? singular : plural;
  }

  // 統計和監控
  getStats(): TranslationStats {
    const languages = Object.keys(this.state.availableLanguages);
    ...
    languages.forEach(lang => {
      const missingForLang = this.missingTranslations.filter(
        t => t.language === lang
      ).length;
      const totalForLang = 1000; // 估算總鍵數
      const translatedForLang = totalForLang - missingForLang;
  ...
  // 工具方法
  isRTL(): boolean {
    const language = this.state.availableLanguages[this.state.currentLanguage];
