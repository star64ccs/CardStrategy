...
describe('CurrencyService', () => {
  ...
  describe('Singleton Pattern', () => {
    it('應該返回相同的實例', () => {
      const instance1 = currencyService;
      const instance2 = currencyService;
  ...
  describe('貨幣管理', () => {
    ...
    it('應該成功更改貨幣', async () => {
      const result = await currencyService.changeCurrency('USD');
    ...
    it('應該拒絕不支持的貨幣', async () => {
      const result = await currencyService.changeCurrency('INVALID');
  ...
  describe('匯率管理', () => {
    ...
    it('應該獲取相同貨幣的匯率', async () => {
      ...
      const result = await currencyService.getExchangeRate(request);
      expect(result.success).toBe(true);
  ...
  describe('格式化功能', () => {
    ...
    it('應該正確格式化貨幣', () => {
      const formatted = currencyService.formatCurrency(1234.56, 'TWD');
      // 檢查格式化結果是字符串且包含數字
    ...
    it('應該解析貨幣字符串', () => {
      const parsed = currencyService.parseCurrency('1234.56', 'TWD');
      expect(parsed).toBe(1234.56);
    });

    it('應該獲取貨幣符號', () => {
      const symbol = currencyService.getCurrencySymbol('TWD');
      expect(symbol).toBe('NT$');
    });

    it('應該獲取貨幣名稱', () => {
      const name = currencyService.getCurrencyName('TWD');
  ...
  describe('工具功能', () => {
    ...
    it('應該計算轉換手續費', () => {
      const fee = currencyService.calculateConversionFee(1000, 1.5);
      expect(fee).toBeGreaterThan(0);
    });

    it('應該應用加價', () => {
      const result = currencyService.applyMarkup(100, 0.1);
      expect(result).toBe(10);
    });

    it('應該四捨五入貨幣', () => {
      const rounded = currencyService.roundCurrency(123.456, 'TWD', 'round');
      expect(rounded).toBe(123);
    });

    it('應該清理貨幣輸入', () => {
      const cleaned = currencyService.sanitizeCurrencyInput('NT$1,234.56');
      expect(cleaned).toBe('1,234.56');
    });

    it('應該比較貨幣', () => {
      const result = currencyService.compareCurrencies('TWD', 'USD');
      expect(typeof result).toBe('number');
    });

    it('應該估算轉換', () => {
      const estimated = currencyService.estimateConversion(100, 'USD', 'TWD');
  ...
  describe('邊界條件', () => {
    ...
    it('應該處理極大金額', () => {
      ...
      const formatted = currencyService.formatCurrency(largeAmount, 'TWD');
      expect(formatted).toBeDefined();
    });

    it('應該處理極小金額', () => {
      const smallAmount = Number.MIN_VALUE;
      const formatted = currencyService.formatCurrency(smallAmount, 'TWD');
      expect(formatted).toBeDefined();
    });

    it('應該處理零金額', () => {
      const formatted = currencyService.formatCurrency(0, 'TWD');
      expect(formatted).toBeDefined();
    });

    it('應該處理空字符串輸入', () => {
      const cleaned = currencyService.sanitizeCurrencyInput('');
      expect(cleaned).toBe('');
    });
