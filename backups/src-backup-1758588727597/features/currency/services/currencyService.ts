...
class CurrencyService implements CurrencyManager, CurrencyTools {
  ...
  async changeCurrency(currency: string): Promise<CurrencyUpdateResponse> {
    try {
      ...
      const oldCurrency = this.state.currentCurrency;
  ...
  async getExchangeRate(
    ...
    try {
      ...
      try {
        ...
        // 使用備用匯率
        const fallbackKey = `${request.fromCurrency}/${request.toCurrency}`;
        const fallbackRate = this.config.fallbackRates[fallbackKey];
  ...
  async updateExchangeRates(): Promise<void> {
    try {
      ...
      const currencies = Object.keys(this.state.availableCurrencies);
  ...
  getStats(): CurrencyStats {
    const totalConversions = this.state.conversionHistory.length;
    const mostUsedCurrency = this.getMostUsedCurrency();
    const averageConversionAmount = this.calculateAverageConversionAmount();
    const lastConversionDate =
      totalConversions > 0
        ? this.state.conversionHistory[totalConversions - 1].timestamp
        : null;
    const conversionAccuracy = this.calculateConversionAccuracy();
  ...
  formatCurrency(
    ...
    const targetCurrency = currency || this.state.currentCurrency;
    const currencyInfo = this.state.availableCurrencies[targetCurrency];

    if (!currencyInfo) {
      return `${amount}`;
    }

    const formatter = this.createCurrencyFormatter(targetCurrency, options);
  ...
  parseCurrency(value: string, currency?: string): number {
    ...
    // 移除貨幣符號和空格
    const cleanedValue = this.sanitizeCurrencyInput(value);

    // 根據地區解析數字
    const locale = this.getCurrencyLocale(targetCurrency);
  ...
  getCurrencySymbol(currency: string): string {
    const currencyInfo = this.state.availableCurrencies[currency];
  ...
  roundCurrency(
    ...
    const factor = 10 ** currencyInfo.decimalPlaces;
  ...
  compareCurrencies(currency1: string, currency2: string): number {
    const info1 = this.state.availableCurrencies[currency1];
    const info2 = this.state.availableCurrencies[currency2];
  ...
  private async loadState(): Promise<void> {
    try {
      const stored = await AsyncStorage.getItem('currency_state');
      if (stored) {
  ...
  private async fetchExchangeRateFromAPI(
    ...
    const url = `${this.config.apiEndpoints.exchangeRate}${fromCurrency}`;
  ...
  private calculateAverageConversionAmount(): number {
    ...
    const total = this.state.conversionHistory.reduce(
  ...
  private calculateConversionAccuracy(): number {
    ...
    const totalRequests = this.apiCallCount;
    const successfulRequests = this.state.conversionHistory.length;
