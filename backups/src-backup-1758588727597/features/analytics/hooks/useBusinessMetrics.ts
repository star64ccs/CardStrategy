...
export const useBusinessMetrics = () => {
  ...
  const exportDataState = useSelector(selectExportData);
  ...
  // 清除導出錯誤
  const clearExportErrorHandler = useCallback(() => {
  ...
  // 更新實時指標
  const updateRealTimeMetricsHandler = useCallback(
    (metrics: unknown) => {
  ...
  // 添加事件監聽器
  const addEventListenerHandler = useCallback(
  ...
  // 移除事件監聽器
  const removeEventListenerHandler = useCallback(
    (eventType: string, listener: (data: unknown) => void) => {
  ...
  const isHealthy = useMemo((): boolean => {
    ...
    const health = (analysis as any)?.summary?.overallHealth;
