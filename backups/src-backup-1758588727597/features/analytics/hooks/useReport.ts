...
export const useReport = () => {
  ...
  const completedExports = useSelector(selectCompletedExports);
  const templateCount = useSelector(selectTemplateCount);
  const reportCount = useSelector(selectReportCount);
  const exportCount = useSelector(selectExportCount);
  ...
  const createNewTemplate = useCallback(
    async (request: CreateTemplateRequest) => {
      try {
        const template = await dispatch(createTemplate(request)).unwrap();
        return template;
      } catch (error) {
  ...
  const getTemplateById = useCallback(
    async (templateId: string) => {
      try {
        const template = await dispatch(getTemplate(templateId)).unwrap();
        return template;
      } catch (error) {
  ...
  const createNewReport = useCallback(
    async (request: CreateReportRequest) => {
      try {
        const report = await dispatch(createReport(request)).unwrap();
        return report;
      } catch (error) {
  ...
  const getReportById = useCallback(
    async (reportId: string) => {
      try {
        const report = await dispatch(getReport(reportId)).unwrap();
        return report;
      } catch (error) {
  ...
  const selectReportById = useCallback(
  ...
  const exportReportById = useCallback(
    async (reportId: string, request: ExportReportRequest) => {
      try {
        const exportInstance = await dispatch(
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      const analyticsData = await dispatch(getAnalytics()).unwrap();
  ...
  // 快速創建方法
  const quickCreateBusinessReport = useCallback(
    async (name: string) => {
      const businessTemplates = getTemplatesByCategory(
  ...
  const quickCreateFinancialReport = useCallback(
    async (name: string) => {
      const financialTemplates = getTemplatesByCategory(
      ...
      const template = financialTemplates[0];
      return createNewReport({
        templateId: template.id,
        name,
      });
  ...
  const quickExportToPDF = useCallback(
    async (reportId: string) => {
  ...
  const quickExportToExcel = useCallback(
    async (reportId: string) => {
      return exportReportById(reportId, {
