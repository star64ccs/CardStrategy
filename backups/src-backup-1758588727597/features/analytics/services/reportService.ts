...
export class ReportService {
  ...
  private async processExport(
    ...
    try {
      const exportInstance = this.exports.get(exportId);
  ...
  private calculatePopularTemplates(
    ...
    reports.forEach(report => {
      const count = templateUsage.get(report.templateId) || 0;
  ...
  private calculateDeliveryStats(reports: ReportInstance[]): unknown {
    const totalDeliveries = reports.length;
    const successfulDeliveries = reports.filter(
      r => r.status === ReportStatus.DELIVERED
    ).length;
    const failedDeliveries = reports.filter(
  ...
  private calculatePerformanceMetrics(reports: ReportInstance[]): unknown {
    ...
    const successfulOperations = reports.filter(
      r => r.status !== ReportStatus.FAILED
    ).length;
