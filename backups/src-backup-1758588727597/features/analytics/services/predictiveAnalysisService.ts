...
export class PredictiveAnalysisService {
  ...
  public async getPredictiveAnalysis(
    ...
    try {
      ...
      const filteredPredictions = this.filterPredictions(filter);

      const accuracy = this.calculateAverageAccuracy(filteredPredictions);
  ...
  public async generateReport(
    ...
    const bestPrediction = modelPredictions.reduce(
      (best, current) => (current.accuracy > best.accuracy ? current : best),
      modelPredictions[0]
    );

    const worstPrediction = modelPredictions.reduce(
      (worst, current) => (current.accuracy < worst.accuracy ? current : worst),
      modelPredictions[0]
    );

    const insights = await this.generateInsights(modelId, dateRange);
  ...
  public async getRealTimeMetrics(): Promise<{
    ...
    const totalPredictions = Array.from(this.predictions.values()).length;
