...
const mockConvertToJSON = require('../utils/dataConverters').convertToJSON;
const mockConvertToCSV = require('../utils/dataConverters').convertToCSV;
const mockConvertToExcel = require('../utils/dataConverters').convertToExcel;
const mockConvertToPDF = require('../utils/dataConverters').convertToPDF;
...
describe('PredictiveAnalysisService', () => {
  ...
  describe('單例模式', () => {
    test('應該返回相同的實例', () => {
      const instance1 = PredictiveAnalysisService.getInstance();
      const instance2 = PredictiveAnalysisService.getInstance();
  ...
  describe('預測分析', () => {
    ...
    test('應該獲取預測分析數據', async () => {
      const result = await service.getPredictiveAnalysis();
    ...
    test('應該使用過濾器獲取預測分析', async () => {
      ...
      const result = await service.getPredictiveAnalysis(filter);
      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
    });
    ...
    test('應該在未初始化時拋出錯誤', async () => {
      // 重置單例實例以創建未初始化的服務
      (PredictiveAnalysisService as any).instance = undefined;
      const newService = PredictiveAnalysisService.getInstance();
  ...
  describe('預測生成', () => {
    ...
    test('應該在模型不存在時拋出錯誤', async () => {
      const inputFeatures = { feature1: 100 };
      await expect(
  ...
  describe('警報管理', () => {
    ...
    test('應該刪除警報', async () => {
      ...
      const result = await service.deleteAlert(alert.id);
      expect(result).toBe(true);
    });
  ...
  describe('邊界條件測試', () => {
    ...
    test('應該處理無效的配置', async () => {
      ...
      const updatedConfig = await service.updateConfig(invalidConfig);
      expect(updatedConfig).toBeDefined();
    });
  });
});
