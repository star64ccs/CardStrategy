...
describe('BusinessMetricsService', () => {
  ...
  describe('單例模式', () => {
    test('應該返回相同的實例', () => {
      const instance1 = BusinessMetricsService.getInstance();
      const instance2 = BusinessMetricsService.getInstance();
  ...
  describe('業務指標分析', () => {
    ...
    test('應該獲取業務指標分析數據', async () => {
      const analysis = await service.getBusinessMetrics();
    ...
    test('應該使用過濾器獲取業務指標分析', async () => {
      ...
      const analysis = await service.getBusinessMetrics(filter);
    ...
    test('應該在未初始化時拋出錯誤', async () => {
      // 創建新的服務實例，不初始化
      const newService = BusinessMetricsService.getInstance();
  ...
  describe('警報管理', () => {
    ...
    test('應該更新警報', () => {
      ...
      const alert = service.createAlert(alertData);
      const updatedAlert = service.updateAlert(alert.id, {
        acknowledged: true,
      });
    ...
    test('應該處理不存在的警報', async () => {
      const alert = await service.getAlert('non-existent-id');
