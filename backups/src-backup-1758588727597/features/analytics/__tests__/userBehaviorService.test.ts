...
// Mock 數據轉換函數
const mockConvertToJSON = jest.fn(data => JSON.stringify(data));
const mockConvertToCSV = jest.fn(data => 'csv,data,format');
const mockConvertToExcel = jest.fn(data => 'excel,data,format');
const mockConvertToPDF = jest.fn(data => 'pdf,data,format');
...
describe('UserBehaviorService', () => {
  ...
  describe('單例模式', () => {
    it('應該返回相同的實例', () => {
      const instance1 = UserBehaviorService.getInstance();
      const instance2 = UserBehaviorService.getInstance();
  ...
  describe('事件追蹤', () => {
    ...
    it('應該在禁用時不追蹤事件', () => {
      ...
      const initialEventCount = (service as any).events.length;
      service.trackEvent(event);
  ...
  describe('行為分析', () => {
    ...
    it('應該獲取行為分析數據', async () => {
      const analysis = await service.getBehaviorAnalysis();
    ...
    it('應該使用過濾器獲取行為分析', async () => {
      ...
      const analysis = await service.getBehaviorAnalysis(filter);
  ...
  describe('數據導出', () => {
    ...
    it('應該導出 JSON 格式數據', async () => {
      ...
      const result = await service.exportData(analysis, options);
  ...
  describe('警報管理', () => {
    ...
    it('應該更新警報', async () => {
      ...
      const updatedAlert = await service.getAlert(alert.id);
    ...
    it('應該刪除警報', async () => {
      ...
      // 驗證警報已被刪除
      const deletedAlert = await service.getAlert(alert.id);
    ...
    it('應該處理不存在的警報', async () => {
      const nonExistentAlert = await service.getAlert('non-existent-id');
  ...
  describe('用戶特定數據', () => {
    ...
    it('應該獲取用戶模式', async () => {
      const patterns = await service.getUserPatterns('user123');
    ...
    it('應該獲取用戶指標', async () => {
      const metrics = await service.getUserMetrics('user123');
  ...
  describe('邊界條件測試', () => {
    ...
    it('應該處理無效的過濾器', async () => {
      ...
      const analysis = await service.getBehaviorAnalysis(invalidFilter);
