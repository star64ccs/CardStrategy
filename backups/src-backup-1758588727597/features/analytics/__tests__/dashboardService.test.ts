...
describe('DashboardService', () => {
  ...
  describe('初始化', () => {
    ...
    test('應該在未初始化時拋出錯誤', async () => {
      ...
      (DashboardService as any).instance = undefined;
      const uninitializedService = DashboardService.getInstance();
  ...
  describe('儀表板管理', () => {
    ...
    test('應該獲取默認儀表板', async () => {
      const dashboard = await service.getDashboard('default-dashboard');
    ...
    test('應該更新儀表板', async () => {
      ...
      const updatedDashboard = await service.updateDashboard(dashboard.id, {
        name: '更新名稱',
        description: '更新描述',
      });
    ...
    test('應該刪除儀表板', async () => {
      ...
      const deletedDashboard = await service.getDashboard(dashboard.id);
    ...
    test('應該過濾儀表板列表', async () => {
      ...
      const result = await service.getDashboards({ search: '測試儀表板1' });
  ...
  describe('數據管理', () => {
    ...
    test('應該獲取儀表板數據', async () => {
      const data = await service.getDashboardData('default-dashboard');
  ...
  describe('導出功能', () => {
    ...
    test('應該導出儀表板為 PDF', async () => {
      const exportResult = await service.exportDashboard(
        'default-dashboard',
        'pdf'
      );
  ...
  describe('警報管理', () => {
    ...
    test('應該更新警報', async () => {
      ...
      const updatedAlert = await service.updateAlert(alert.id, {
        isActive: false,
      });
    ...
    test('應該獲取特定儀表板的警報', async () => {
      const alerts = await service.getAlerts('default-dashboard');
      expect(Array.isArray(alerts)).toBe(true);
    });
  });
  ...
  describe('模板管理', () => {
    ...
    test('應該按分類獲取模板', async () => {
      const businessTemplates = await service.getTemplates('business');
  ...
  describe('分析功能', () => {
    ...
    test('應該獲取儀表板分析', async () => {
      const analytics = await service.getAnalytics('default-dashboard');
      expect(analytics).toBeDefined();
    });

    test('應該更新儀表板分析', async () => {
      const analytics = await service.updateAnalytics('default-dashboard', {
        views: 100,
        uniqueViews: 80,
      });
    ...
    test('應該獲取性能指標', async () => {
      const metrics = await service.getPerformanceMetrics('default-dashboard');
  ...
  describe('數據驗證', () => {
    ...
    test('應該驗證組件數據結構', async () => {
      const dashboard = await service.getDashboard('default-dashboard');
      const widgets = dashboard?.layouts[0]?.widgets || [];
