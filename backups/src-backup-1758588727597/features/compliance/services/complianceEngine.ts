...
export class ComplianceEngine {
  ...
  public detectJurisdiction(userLocation: Location): Jurisdiction {
    try {
      const jurisdiction = this.findJurisdictionByLocation(userLocation);
  ...
  public applyRegulations(jurisdiction: Jurisdiction): ComplianceRules {
    try {
      ...
      const featureRestrictions = this.getFeatureRestrictions(
        applicableRegulations
      );
      const dataRequirements = this.getDataRequirements(applicableRegulations);
      const auditRequirements = this.getAuditRequirements(
        applicableRegulations
      );
  ...
  public checkCompliance(data: unknown, operation: string): ComplianceResult {
    try {
      ...
      // 檢查數據最小化
      const dataMinimizationCheck = this.checkDataMinimization(
        data,
        operation
      );
      ...
      // 檢查同意要求
      const consentCheck = this.checkConsentRequirements(data, operation);
      ...
      // 檢查年齡驗證
      const ageVerificationCheck = this.checkAgeVerification(data, operation);
  ...
  public generateComplianceReport(
    ...
    try {
      const targetJurisdiction = jurisdiction || this.getDefaultJurisdiction();
      ...
      // 統計違規
      const violations = this.extractViolationsFromEvents(periodEvents);

      // 計算統計數據
      const totalChecks = periodEvents.filter(e =>
        e.action.includes('compliance_check')
      ).length;
      const compliantChecks = periodEvents.filter(
  ...
  private requiresConsent(operation: string): boolean {
    const consentRequiredOperations = [
      'data_collection',
