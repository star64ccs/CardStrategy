  ...
  public async checkForUpdates(): Promise<RegulationUpdate[]> {
    try {
      ...
      for (const source of this.updateSources) {
        try {
          const sourceUpdates = await this.checkSourceForUpdates(source);
  ...
  private async processUpdates(updates: RegulationUpdate[]): Promise<void> {
    try {
      for (const update of updates) {
        // 評估合規影響
        const impact = await this.assessComplianceImpact(update);
...
// 導出單例實例
export const regulationUpdateMonitor = RegulationUpdateMonitor.getInstance();
