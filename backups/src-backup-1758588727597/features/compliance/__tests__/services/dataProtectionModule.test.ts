...
describe('DataProtectionModule', () => {
  ...
  describe('單例模式測試', () => {
    it('應該返回相同的實例', () => {
      const instance1 = DataProtectionModule.getInstance();
      const instance2 = DataProtectionModule.getInstance();
  ...
  describe('初始化測試', () => {
    it('應該成功初始化模組', async () => {
      const result = await dataProtectionModule.initialize();
    ...
    it('應該使用自定義配置初始化', async () => {
      ...
      const result = await dataProtectionModule.initialize(customConfig);
      expect(result).toBe(true);
    });
  });
  ...
  describe('數據分類測試', () => {
    it('應該分類個人數據', () => {
      ...
      const classification = dataProtectionModule.classifyData(data);
    ...
    it('應該處理空數據', () => {
      const classification = dataProtectionModule.classifyData(null);
      expect(classification.category).toBe('public');
    });
  });
  ...
  describe('保留政策測試', () => {
    it('應該應用個人數據保留政策', () => {
      ...
      const policy = dataProtectionModule.applyRetentionPolicy(data);
  ...
  describe('數據最小化測試', () => {
    it('應該最小化個人數據', () => {
      ...
      const minimizedData = dataProtectionModule.minimizeData(
        data,
        'account_management'
      );
    ...
    it('應該確定適當的匿名化級別', () => {
      ...
      const sensitiveMinimized = dataProtectionModule.minimizeData(
        sensitiveData,
        'analytics'
      );
      const personalMinimized = dataProtectionModule.minimizeData(
        personalData,
        'analytics'
      );
  ...
  describe('目的驗證測試', () => {
    it('應該驗證有效目的', () => {
      ...
      const isValid = dataProtectionModule.validatePurpose(
        data,
        'account_management'
      );
  ...
  describe('數據主體請求測試', () => {
    it('應該處理訪問請求', () => {
      ...
      const result = dataProtectionModule.processDataSubjectRequest(request);
  ...
  describe('數據導出測試', () => {
    it('應該導出用戶數據', () => {
      ...
      const exportedData = dataProtectionModule.exportUserData(userId);
  ...
  describe('數據刪除測試', () => {
    it('應該刪除用戶數據', () => {
      const userId = 'user_123';
      const deletionResult = dataProtectionModule.deleteUserData(userId);
  ...
  describe('重置測試', () => {
    it('應該重置模組狀態', async () => {
      ...
      // 驗證重置後可以重新初始化
      const initResult = await dataProtectionModule.initialize();
  ...
  describe('邊界條件測試', () => {
    ...
    it('應該處理空數據最小化', () => {
      const minimizedData = dataProtectionModule.minimizeData(null, 'test');
  ...
  describe('性能測試', () => {
    it('應該快速處理大量分類', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const data = { name: `User ${i}`, email: `user${i}@example.com` };
        dataProtectionModule.classifyData(data);
      }
  ...
  describe('功能測試', () => {
    it('應該處理複雜的數據場景', () => {
      ...
      const classification = dataProtectionModule.classifyData(complexData);
