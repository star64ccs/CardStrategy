...
describe('ComplianceEngine', () => {
  ...
  describe('單例模式測試', () => {
    it('應該返回相同的實例', () => {
      const instance1 = ComplianceEngine.getInstance();
      const instance2 = ComplianceEngine.getInstance();
  ...
  describe('初始化測試', () => {
    it('應該成功初始化引擎', async () => {
      const result = await complianceEngine.initialize();
    ...
    it('應該使用自定義配置初始化', async () => {
      ...
      const result = await complianceEngine.initialize(customConfig);
      expect(result).toBe(true);
    });
  });
  ...
  describe('管轄區檢測測試', () => {
    it('應該檢測台灣管轄區', () => {
      ...
      const jurisdiction = complianceEngine.detectJurisdiction(location);
  ...
  describe('法規應用測試', () => {
    it('應該應用台灣法規', () => {
      const jurisdiction = complianceEngine.detectJurisdiction({
        country: 'TW',
      });
      const complianceRules = complianceEngine.applyRegulations(jurisdiction);
    ...
    it('應該包含隱私法規要求', () => {
      ...
      const privacyRegulations = complianceRules.applicableRegulations.filter(
  ...
  describe('合規性檢查測試', () => {
    it('應該通過合規性檢查', () => {
      ...
      const result = complianceEngine.checkCompliance(data, 'data_collection');
    ...
    it('應該檢測年齡驗證違規', () => {
      ...
      const result = complianceEngine.checkCompliance(data, 'gaming_access');
      expect(result.isCompliant).toBe(false);
      expect(
    ...
    it('應該為非敏感操作通過檢查', () => {
      ...
      const result = complianceEngine.checkCompliance(data, 'basic_profile');
      expect(result.isCompliant).toBe(true);
      expect(result.score).toBe(100);
    });
  });
  ...
  describe('同意驗證測試', () => {
    it('應該驗證有效同意', () => {
      ...
      const result = complianceEngine.validateConsent(consent);
  ...
  describe('合規報告生成測試', () => {
    it('應該生成合規報告', () => {
      ...
      const report = complianceEngine.generateComplianceReport(jurisdiction);
    ...
    it('應該包含正確的統計信息', () => {
      const report = complianceEngine.generateComplianceReport();
  ...
  describe('配置管理測試', () => {
    it('應該更新配置', () => {
      ...
      // 驗證配置已更新（通過檢查行為變化）
      const data = { name: 'Test', consent: true };
      const result = complianceEngine.checkCompliance(data, 'data_collection');
  ...
  describe('重置測試', () => {
    it('應該重置引擎狀態', async () => {
      ...
      // 驗證重置後可以重新初始化
      const initResult = await complianceEngine.initialize();
  ...
  describe('邊界條件測試', () => {
    it('應該處理空數據', () => {
      const result = complianceEngine.checkCompliance(null, 'data_collection');
      expect(result.isCompliant).toBe(true);
      expect(result.score).toBe(100);
    });
    ...
    it('應該處理空操作', () => {
      ...
      const result = complianceEngine.checkCompliance(data, '');
      expect(result.isCompliant).toBe(true);
      expect(result.score).toBe(100);
    });
  ...
  describe('性能測試', () => {
    it('應該快速處理大量檢查', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const data = { name: `User ${i}`, consent: true };
  ...
  describe('功能測試', () => {
    ...
    it('應該處理長期運行的場景', () => {
      const jurisdictions = ['TW', 'MO', 'EU', 'US', 'CA'];

      jurisdictions.forEach(country => {
        const location: Location = { country };
        const jurisdiction = complianceEngine.detectJurisdiction(location);
        const rules = complianceEngine.applyRegulations(jurisdiction);
