...
describe('ECommerceComplianceModule', () => {
  ...
  describe('單例模式測試', () => {
    test('應該返回相同的實例', () => {
      const instance1 = ECommerceComplianceModule.getInstance();
      const instance2 = ECommerceComplianceModule.getInstance();
  ...
  describe('初始化測試', () => {
    test('應該成功初始化', async () => {
      const result = await eCommerceComplianceModule.initialize();
    ...
    test('應該使用自定義配置初始化', async () => {
      ...
      const result = await eCommerceComplianceModule.initialize(customConfig);
      expect(result).toBe(true);
    });
  });
  ...
  describe('交易合規檢查測試', () => {
    ...
    test('應該包含年齡驗證檢查', () => {
      ...
      const ageCheck = result.complianceChecks.find(
    ...
    test('應該包含位置驗證檢查', () => {
      ...
      const locationCheck = result.complianceChecks.find(
    ...
    test('應該包含支付驗證檢查', () => {
      ...
      const paymentCheck = result.complianceChecks.find(
  ...
  describe('消費者權利檢查測試', () => {
    test('應該檢查消費者權利', () => {
      const result = eCommerceComplianceModule.checkConsumerRights(
        'user_123',
        'tw'
      );
  ...
  describe('合規報告生成測試', () => {
    test('應該生成合規報告', () => {
      ...
      const report = eCommerceComplianceModule.generateComplianceReport();
    ...
    test('應該生成指定期間的合規報告', () => {
      ...
      const report = eCommerceComplianceModule.generateComplianceReport(period);
  ...
  describe('功能測試', () => {
    test('應該處理複雜的電子商務場景', () => {
      ...
      // 檢查消費者權利
      const rightsResult = eCommerceComplianceModule.checkConsumerRights(
        'user_complex',
        'tw'
      );
      ...
      // 驗證至少有一個產品列表是合規的
      const compliantListings = listingResults.filter(
