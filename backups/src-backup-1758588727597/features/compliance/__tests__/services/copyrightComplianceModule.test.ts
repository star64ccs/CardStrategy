...
describe('CopyrightComplianceModule', () => {
  ...
  describe('單例模式', () => {
    it('應該返回相同的實例', () => {
      const instance1 = CopyrightComplianceModule.getInstance();
      const instance2 = CopyrightComplianceModule.getInstance();
  ...
  describe('初始化', () => {
    it('應該成功初始化模組', async () => {
      const result = await copyrightComplianceModule.initialize();
    ...
    it('應該使用自定義配置初始化', async () => {
      ...
      const result = await copyrightComplianceModule.initialize(customConfig);
      expect(result).toBe(true);
    });
  });
  ...
  describe('DMCA處理', () => {
    it('應該處理DMCA請求', () => {
      ...
      const result = copyrightComplianceModule.processDMCARequest(dmcaRequest);
  ...
  describe('授權管理', () => {
    it('應該創建新授權', () => {
      ...
      const result = copyrightComplianceModule.manageLicensing(license);
