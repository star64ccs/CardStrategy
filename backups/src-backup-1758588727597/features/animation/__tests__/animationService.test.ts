...
describe('AnimationService', () => {
  ...
  describe('動畫管理', () => {
    ...
    it('應該創建動畫', () => {
      const animationId = animationService.createAnimation({
        type: 'timing',
        duration: 300,
        easing: 'ease-in-out',
        from: { opacity: 0 },
    ...
    it('應該創建預設動畫', () => {
      const presetId = animationService.createPreset({
        name: 'Test Preset',
  ...
  describe('預設動畫', () => {
    ...
    it('應該創建淡入動畫', () => {
      const animation = animationService.createFadeAnimation(0, 1, 300);
    ...
    it('應該創建滑動動畫', () => {
      const animation = animationService.createSlideAnimation('left', 100);
      expect(animation).toBeDefined();
      expect(animation.type).toBe('timing');
    ...
    it('應該創建縮放動畫', () => {
      const animation = animationService.createScaleAnimation(0, 1, 300);
      expect(animation).toBeDefined();
  ...
  describe('動畫工具', () => {
    ...
    it('應該組合動畫', () => {
      const animation1 = animationService.createFadeAnimation(0, 1, 200);
      const animation2 = animationService.createSlideAnimation('left', 100);

      const combined = animationService.createSequence([
        animation1,
        animation2,
      ]);
    ...
    it('應該插值動畫值', () => {
      const interpolated = animationService.interpolate(0.5, [0, 1], [0, 100]);
      expect(interpolated).toBe(50);
    });

    it('應該插值顏色', () => {
      const interpolated = animationService.interpolateColor(
        0.5,
        [0, 1],
        ['#FF0000', '#0000FF']
      );
  ...
  describe('邊界條件', () => {
    ...
    it('應該處理空的插值範圍', () => {
      const result = animationService.interpolate(0.5, [], []);
