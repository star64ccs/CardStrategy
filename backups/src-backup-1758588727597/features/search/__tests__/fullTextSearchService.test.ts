...
describe('FullTextSearchService', () => {
  ...
  describe('單例模式', () => {
    it('應該返回相同的實例', () => {
      const instance1 = FullTextSearchService.getInstance();
      const instance2 = FullTextSearchService.getInstance();
  ...
  describe('搜索功能', () => {
    ...
    it('應該應用排序', async () => {
      ...
      for (let i = 1; i < response.results.length; i++) {
        const prevPrice = response.results[i - 1].price || 0;
        const currentPrice = response.results[i].price || 0;
  ...
  describe('搜索索引管理', () => {
    ...
    it('應該獲取搜索索引列表', async () => {
      const indexes = await searchService.getSearchIndexes();

      expect(indexes).toBeInstanceOf(Array);
      expect(indexes.length).toBeGreaterThan(0);

      const cardsIndex = indexes.find(index => index.id === 'cards');
    ...
    it('應該更新搜索索引', async () => {
      const result = await searchService.updateSearchIndex('cards');
    ...
    it.skip('應該處理索引更新失敗', async () => {
      // 模擬更新失敗
      const mockUpdateIndex = jest.spyOn(
      ...
      const result = await searchService.updateSearchIndex('invalid_index');
      expect(result).toBe(false);
  ...
  describe('配置管理', () => {
    ...
    it('應該更新搜索配置', () => {
      ...
      const updatedConfig = searchService.getConfig();
  ...
  describe('錯誤處理', () => {
    it('應該處理搜索錯誤', async () => {
      ...
      // 模擬搜索過程中發生錯誤
      const mockExecuteSearch = jest.spyOn(
        searchService as any,
        'executeSearch'
      );
