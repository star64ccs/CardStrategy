...
export class IntelligentSearchService {
  ...
  async search(
    ...
    try {
      ...
      // 應用智能功能
      const results = await this.applyIntelligentFeatures(
  ...
  private async applyIntelligentFeatures(
    ...
    for (const result of baseResults) {
      // 計算各種分數
      const relevanceScore = this.calculateRelevanceScore(result, query.query);
  ...
  private async calculatePersonalizationScore(
    ...
    const preferences = await this.getUserPreferences(query.userId);
  ...
  async getSuggestions(
    ...
    if (context?.sessionId) {
      const history = this.searchHistory.get(context.sessionId) || [];
  ...
  async analyzeQuery(query: string): Promise<QueryAnalysis> {
    ...
    // 確定搜索意圖
    const intent = this.determineSearchIntent(query, entities);

    // 生成建議和糾正
    const suggestions = this.generateQuerySuggestions(query);
    const corrections = this.generateQueryCorrections(query);
