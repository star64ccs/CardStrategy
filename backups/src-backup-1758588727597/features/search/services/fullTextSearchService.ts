...
export class FullTextSearchService {
  ...
  async search(query: SearchQuery): Promise<SearchResponse> {
    ...
    try {
      ...
      // 應用排序
      const sortedResults = this.applySorting(filteredResults, query.sortBy);
      ...
      // 生成高亮
      const resultsWithHighlights = this.generateHighlights(
        paginatedResults,
        query.query
      );
  ...
  private async executeSearch(query: SearchQuery): Promise<SearchResult[]> {
    ...
    const allResults = this.searchIndex.get('cards') || [];
  ...
  private applyPagination(
    ...
    const currentLimit = limit || this.config.defaultLimit;
  ...
  private generateHighlights(
    ...
    return results.map(result => {
      ...
      // 標題高亮
      if (result.title) {
        const titleHighlight = this.createHighlight(
          result.title,
          searchTerms,
          'title'
        );
      ...
      if (result.description) {
        const descHighlight = this.createHighlight(
          result.description,
          searchTerms,
          'description'
        );
  ...
  private recordSearchHistory(
    ...
    const userId = 'anonymous'; // 實際應用中應該從認證系統獲取
  ...
  async getSearchStats(): Promise<SearchStats> {
    ...
    const allSearchTimes = Array.from(this.searchHistory.values())
      .flat()
    ...
    const userBehavior = this.getUserSearchBehavior();
