...
export class BackgroundSyncManager {
  ...
  addTask(task: Omit<SyncTask, 'id' | 'retryCount' | 'createdAt'>): string {
    ...
    logger.info('[Background Sync] 添加同步任務:', { id, type: task.type });
    return id;
  }
  ...
  private async processTask(task: SyncTask): Promise<void> {
    try {
      logger.info('[Background Sync] 處理任務:', {
        id: task.id,
        type: task.type,
      });
        ...
        if (response.status === 409) {
          ...
          if (resolution.resolved) {
            ...
            if (retryResponse.ok) {
              ...
              logger.info('[Background Sync] 衝突解決成功，任務完成:', {
                id: task.id,
          ...
          this.completeTask(task);
          logger.info('[Background Sync] 任務完成:', { id: task.id });
        }
      } else {
  ...
  private handleTaskError(task: SyncTask, error: any): void {
    ...
    if (task.retryCount >= task.maxRetries) {
      ...
      this.status.pendingTasks--;
      logger.error('[Background Sync] 任務失敗，已達最大重試次數:', {
        id: task.id,
        error: task.error,
      });
    } else {
      ...
      logger.warn('[Background Sync] 任務重試:', {
        id: task.id,
  ...
  private async resolveConflict(
    ...
    logger.info('[Background Sync] 檢測到衝突:', { id: task.id, conflictInfo });
    ...
    try {
      ...
      logger.error('[Background Sync] 衝突解決失敗:', { id: task.id, error });
      return {
        resolved: false,
        finalValue: task.body,
        strategy: 'client-wins',
  ...
  private calculateConflictSeverity(
    ...
    const conflictingFields = commonKeys.filter(
      key => JSON.stringify(clientData[key]) !== JSON.stringify(serverData[key])
    );
  ...
  private resolveCustom(
    ...
    if (customResolvers[resolverKey]) {
      try {
        ...
        logger.error('[Background Sync] 自定義衝突解決器失敗:', {
          resolverKey,
          error,
        });
      }
  ...
  private startAutoSync(): void {
    ...
    logger.info('[Background Sync] 自動同步已啟動，間隔:', {
  ...
  private loadTasks(): void {
    try {
      ...
      if (saved) {
        ...
        this.updateStatus();
        logger.info('[Background Sync] 已加載任務:', {
  ...
  cleanupExpiredTasks(maxAge: number = 7 * 24 * 60 * 60 * 1000): number {
    ...
    if (cleanedCount > 0) {
      this.updateStatus();
      this.saveTasks();
      logger.info('[Background Sync] 清理過期任務:', { count: cleanedCount });
    }
