...
export class CodeQualityChecker {
  ...
  checkComponentOptimization(componentName: string, props: any): void {
    ...
    if (!componentName.includes('React.memo')) {
      this.addIssue(
        'optimization',
        `組件 ${componentName} 建議使用 React.memo 優化`
      );
    }
    ...
    if (propCount > 10) {
      this.addIssue(
        'performance',
        `組件 ${componentName} props 過多 (${propCount} 個)，建議拆分`
      );
    }
  }
  ...
  checkServicePerformance(
    ...
    if (executionTime > 1000) {
      this.addIssue(
        'performance',
        `服務 ${serviceName}.${methodName} 執行時間過長 (${executionTime}ms)`
      );
    }
  }
  ...
  checkMemoryUsage(componentName: string, memoryUsage: number): void {
    if (memoryUsage > 50 * 1024 * 1024) {
      ...
      this.addIssue(
        'memory',
        `組件 ${componentName} 內存使用過高 (${Math.round(memoryUsage / 1024 / 1024)}MB)`
      );
    }
  }
  ...
  checkNetworkRequests(url: string, responseTime: number): void {
    if (responseTime > 5000) {
      this.addIssue(
        'network',
        `網絡請求 ${url} 響應時間過長 (${responseTime}ms)`
      );
    }
  }
