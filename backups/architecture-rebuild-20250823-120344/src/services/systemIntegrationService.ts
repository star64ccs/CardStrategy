...
export class SystemIntegrationService {
  ...
  // 執行卡牌辨識
  private async executeCardRecognition(
    imageData: string,
    cardInfo: any
  ): Promise<any> {
    const startTime = Date.now();
  ...
  // 執行防偽檢測
  private async executeAntiCounterfeit(
    imageData: string,
    cardInfo: any
  ): Promise<any> {
    const startTime = Date.now();
    try {
      const result = await antiCounterfeitService.analyzeCounterfeit(
        imageData,
        {
          aiModel: {
            modelVersion: 'v2.0',
  ...
  // 生成整合結果
  private generateIntegratedResult(
    results: any,
    startTime: number
  ...
  private generateRecommendations(results: any): string[] {
    ...
    if (
      results.antiCounterfeit.success &&
      results.antiCounterfeit.isCounterfeit
    ) {
    ...
    if (
      results.pricePrediction.success &&
      results.pricePrediction.confidence < 0.7
    ) {
  ...
  private assessOverallRisk(results: any): string {
    ...
    if (
      results.pricePrediction.success &&
      results.pricePrediction.confidence < 0.6
    ) {
      risks.push('價格預測不確定性');
    }
