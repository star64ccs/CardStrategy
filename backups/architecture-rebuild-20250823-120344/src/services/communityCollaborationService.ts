...
class CommunityCollaborationService {
  ...
  async applyForPartnership(
    request: Omit<
      CommunityPartner,
      'id' | 'status' | 'contributionCount' | 'lastContribution'
    >
      ...
      const validationResult = validateInput(
        z.object({
          ...
          memberCount: z
            .number()
            .min(
              this.config.minMemberCount,
              `成員數量至少${this.config.minMemberCount}人`
            ),
  ...
  async updateProjectProgress(
    ...
    try {
      const response = await apiService.patch<CollaborationProject>(
        `${API_ENDPOINTS.COMMUNITY.PROJECTS}/${projectId}/progress` ||
          `/community/projects/${projectId}/progress`,
...
export const communityCollaborationService =
  new CommunityCollaborationService();
