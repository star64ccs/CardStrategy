...
class AdvancedPredictionService {
  ...
  // 新增：增強的數據預處理
  async enhancedDataPreprocessing(
    cardId: number,
    timeframe: Timeframe
  ): Promise<{
    processedData: any;
  ...
  private async enhanceDataFeatures(data: any[]): Promise<any[]> {
    return data.map((item, index) => {
      ...
      if (index > 0) {
        ...
        enhanced.priceChangePercent =
          (enhanced.priceChange / data[index - 1].price) * 100;
      ...
      if (index >= 7) {
        const weekAverage =
          data.slice(index - 7, index).reduce((sum, d) => sum + d.price, 0) / 7;
        enhanced.weekAverage = weekAverage;
        enhanced.priceVsWeekAverage =
          ((item.price - weekAverage) / weekAverage) * 100;
      }
      ...
      if (index >= 30) {
        ...
        const monthVariance =
          monthPrices.reduce((a, b) => a + Math.pow(b - monthMean, 2), 0) /
          monthPrices.length;
  ...
  private calculateDataQualityScore(
    ...
    const completenessScore =
      (data.length / (data.length + missingData.length)) * 50;

    return Math.max(
      0,
      Math.min(100, 100 - outlierPenalty - missingPenalty + completenessScore)
    );
  }
  ...
  private interpolateVolume(data: any[], targetDate: string): number {
    ...
    const nearbyData = data
      .filter(
        item =>
          Math.abs(new Date(item.date).getTime() - targetTime) <
          7 * 24 * 60 * 60 * 1000
      )
    ...
    return (
      nearbyData.reduce((sum, item) => sum + item.volume, 0) / nearbyData.length
    );
  }

  // 新增：增強的模型集成
  async enhancedEnsemblePrediction(
    cardId: number,
    timeframe: Timeframe
  ): Promise<{
    predictions: any[];
    ...
    try {
      ...
      for (const model of models.models.filter(m => m.status === 'active')) {
        try {
          const prediction = await this.getAdvancedPrediction(
            cardId,
            model.type,
            timeframe
          );
      ...
      const consensus =
        predictions.reduce((sum, p) => sum + p.prediction * p.weight, 0) /
        totalWeight;
  ...
  private calculateEnsembleMetrics(predictions: any[], consensus: number): any {
    const prices = predictions.map(p => p.prediction);
    const variance =
      prices.reduce((a, b) => a + Math.pow(b - consensus, 2), 0) /
      prices.length;
