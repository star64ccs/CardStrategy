...
describe('防偽判斷功能測試', () => {
  ...
  describe('真偽驗證算法測試', () => {
    it('應該正確識別真卡', async () => {
      ...
      const result = await enhancedAIService.enhancedAuthenticityVerification(
        mockImageData,
        {
    ...
    it('應該正確識別假卡', async () => {
      ...
      const result =
        await enhancedAIService.enhancedAuthenticityVerification(mockImageData);
  ...
  describe('全息圖分析測試', () => {
    it('應該正確分析全息圖特徵', async () => {
      ...
      mockAuthenticity.authenticityService.analyzeHologram.mockResolvedValue(
        mockHologramResult
      );
  ...
  describe('印刷質量分析測試', () => {
    it('應該正確分析印刷質量', async () => {
      ...
      const mockAuthenticity = require('../../../services/authenticityService');
      mockAuthenticity.authenticityService.analyzePrinting.mockResolvedValue(
        mockPrintingResult
      );
  ...
  describe('材料分析測試', () => {
    it('應該正確分析卡片材料', async () => {
      ...
      const mockAuthenticity = require('../../../services/authenticityService');
      mockAuthenticity.authenticityService.analyzeMaterial.mockResolvedValue(
        mockMaterialResult
      );
