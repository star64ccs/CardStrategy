...
const FakeCardTrainingScreen: React.FC = () => {
  ...
  const [selectedModel, setSelectedModel] = useState<TrainingResult | null>(
    null
  );
  ...
  const startTraining = async () => {
    try {
      setIsLoading(true);
      const response =
        await fakeCardTrainingService.startTraining(trainingConfig);
      if (response.success) {
  ...
  const evaluateModel = async (modelId: string) => {
    try {
      ...
      if (response.success) {
        Alert.alert(
          '評估完成',
          `模型準確率: ${(response.data.evaluationMetrics.overallAccuracy * 100).toFixed(2)}%`
        );
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    if (activeTraining) {
      const interval = setInterval(async () => {
        try {
          const response =
            await fakeCardTrainingService.getTrainingProgress(activeTraining);
      ...
      {trainingStats && (
        <Card style={styles.statsCard}>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {trainingStats.deployedModels}
              </Text>
              <Text style={styles.statLabel}>已部署</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {(trainingStats.averageAccuracy * 100).toFixed(1)}%
              </Text>
          ...
          <View style={styles.formRow}>
            ...
            <Input
              ...
              onChangeText={value =>
                setTrainingConfig({
                  ...trainingConfig,
                  trainingEpochs: parseInt(value) || 100,
                })
              }
          ...
          <View style={styles.formRow}>
            ...
            <Input
              ...
              onChangeText={value =>
                setTrainingConfig({
                  ...trainingConfig,
                  learningRate: parseFloat(value) || 0.001,
                })
              }
              keyboardType='numeric'
              style={styles.numberInput}
          ...
          <View style={styles.formRow}>
            ...
            <Input
              ...
              onChangeText={value =>
                setTrainingConfig({
                  ...trainingConfig,
                  validationSplit: parseFloat(value) || 0.2,
                })
              }
              keyboardType='numeric'
              style={styles.numberInput}
            ...
            >
              <View
                style={[
                  ...
                  trainingConfig.dataAugmentation && styles.checkboxChecked,
                ]}
              />
            </TouchableOpacity>
          ...
          <View style={styles.checkboxRow}>
            <TouchableOpacity
              ...
              onPress={() =>
                setTrainingConfig({
                  ...trainingConfig,
                  useTransferLearning: !trainingConfig.useTransferLearning,
                })
              }
            >
              <View
                style={[
                  styles.checkboxInner,
                  trainingConfig.useTransferLearning && styles.checkboxChecked,
                ]}
              />
            </TouchableOpacity>
      ...
      <Card style={styles.dataCard}>
        ...
        <View style={styles.dataStats}>
          ...
          <Text style={styles.dataText}>
            假卡數據:{' '}
            {
              trainingData.filter(item => item.fakeType === 'counterfeit')
                .length
            }
          </Text>
          <Text style={styles.dataText}>
            重印數據:{' '}
            {trainingData.filter(item => item.fakeType === 'reprint').length}
          </Text>
          <Text style={styles.dataText}>
            自製數據:{' '}
            {trainingData.filter(item => item.fakeType === 'custom').length}
          </Text>
        </View>
      </Card>
