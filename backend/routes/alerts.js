...
router.get('/', protect, authorize('admin'), [
  query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('限制必須在 1-100 之間'),
  query('type').optional().isString().withMessage('警報類型必須是字符串'),
  query('severity').optional().isIn(['info', 'warning', 'critical']).withMessage('嚴重程度無效')
], async (req, res) => {
  try {
 ...
 * @access  Private (Admin)
 */
router.get('/history', protect, authorize('admin'), [
  query('limit').optional().isInt({ min: 1, max: 1000 }).withMessage('限制必須在 1-1000 之間'),
  query('type').optional().isString().withMessage('警報類型必須是字符串'),
  query('severity').optional().isIn(['info', 'warning', 'critical']).withMessage('嚴重程度無效'),
 ...
 * @access  Private (Admin)
 */
router.post('/trigger', protect, authorize('admin'), [
  body('type').isString().notEmpty().withMessage('警報類型是必需的'),
  body('message').isString().notEmpty().withMessage('警報消息是必需的'),
  body('severity').optional().isIn(['info', 'warning', 'critical']).withMessage('嚴重程度無效'),
  ...
  try {
    ...
    const { type, message, severity = 'warning', value, threshold } = req.body;

    const alert = await alertService.triggerManualAlert(type, message, severity, {
 ...
 * @access  Private (Admin)
 */
router.put('/thresholds', protect, authorize('admin'), [
  body('cpu').optional().isFloat({ min: 0, max: 100 }).withMessage('CPU 閾值必須在 0-100 之間'),
  body('memory').optional().isFloat({ min: 0, max: 100 }).withMessage('記憶體閾值必須在 0-100 之間'),
  body('disk').optional().isFloat({ min: 0, max: 100 }).withMessage('磁碟閾值必須在 0-100 之間'),
  body('responseTime').optional().isFloat({ min: 0 }).withMessage('響應時間閾值必須大於 0'),
  body('errorRate').optional().isFloat({ min: 0, max: 100 }).withMessage('錯誤率閾值必須在 0-100 之間'),
  body('databaseConnections').optional().isFloat({ min: 0, max: 100 }).withMessage('數據庫連接閾值必須在 0-100 之間')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    if (req.body.memory !== undefined) thresholds.memory = parseFloat(req.body.memory);
    if (req.body.disk !== undefined) thresholds.disk = parseFloat(req.body.disk);
    if (req.body.responseTime !== undefined) thresholds.responseTime = parseFloat(req.body.responseTime);
    if (req.body.errorRate !== undefined) thresholds.errorRate = parseFloat(req.body.errorRate);
    if (req.body.databaseConnections !== undefined) thresholds.databaseConnections = parseFloat(req.body.databaseConnections);
...
router.post('/test', protect, authorize('admin'), [
  body('channel').isIn(['email', 'slack', 'webhook', 'all']).withMessage('通知渠道無效')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
