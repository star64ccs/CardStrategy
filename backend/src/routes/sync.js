...
router.post('/incremental', authenticateToken, async (req, res) => {
  try {
    ...
    logger.info(
      `同步完成 - 處理: ${results.processed}, 衝突: ${results.conflicts}, 錯誤: ${results.errors}`
    );
...
async function getServerChanges(userId, lastSyncTime) {
  ...
  try {
    // 獲取卡片變更
    const cardChanges = await Card.findAll({
      where: {
        userId,
        updatedAt: {
          [Op.gt]: syncTime,
    ...
    // 獲取收藏變更
    const collectionChanges = await Collection.findAll({
      where: {
        userId,
        updatedAt: {
          [Op.gt]: syncTime,
    ...
    // 獲取註釋變更
    const annotationChanges = await Annotation.findAll({
      where: {
        userId,
        updatedAt: {
          [Op.gt]: syncTime,
