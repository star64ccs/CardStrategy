...
router.post(
  ...
  [
    ...
    body('timeframe')
      ...
      .withMessage('時間框架必須是1d、7d、30d、90d、180d或365d'),
    body('modelType')
      .optional()
      .isIn(['linear', 'polynomial', 'exponential', 'arima', 'lstm', 'ensemble'])
      .withMessage('模型類型必須是linear、polynomial、exponential、arima、lstm或ensemble'),
  ],
  async (req, res) => {
    try {
      ...
      logger.info(
        `價格預測: 用戶 ${req.user.username} 預測卡牌 ${cardId}, 模型 ${modelType}, 時間框架 ${timeframe}`
      );
