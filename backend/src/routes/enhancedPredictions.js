...
router.post(
  ...
  [
    ...
    body('modelType')
      .optional()
      .isIn(['enhancedLSTM', 'transformer', 'technicalEnsemble', 'dynamicEnsemble'])
      .withMessage('模型類型無效'),
  ],
  async (req, res) => {
    try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `開始批量增強預測: ${cardIds.length} 張卡牌, 模型 ${modelType}, 時間框架 ${timeframe}`
      );
  ...
  async (req, res) => {
    try {
      ...
      if (analysis.macd.macd > analysis.macd.signal)
        signals.push({ indicator: 'MACD', signal: '買入', strength: '中' });
      if (analysis.macd.macd < analysis.macd.signal)
        signals.push({ indicator: 'MACD', signal: '賣出', strength: '中' });

      const bbPosition =
        (analysis.currentPrice - analysis.bollingerBands.lower) /
        (analysis.bollingerBands.upper - analysis.bollingerBands.lower);
      if (bbPosition < 0.2)
        signals.push({ indicator: '布林帶', signal: '接近下軌', strength: '弱' });
      if (bbPosition > 0.8)
        signals.push({ indicator: '布林帶', signal: '接近上軌', strength: '弱' });
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            buySignals: signals.filter(s => s.signal.includes('買') || s.signal.includes('超賣'))
              .length,
            sellSignals: signals.filter(s => s.signal.includes('賣') || s.signal.includes('超買'))
              .length,
  ...
  try {
    ...
    const stats = await PredictionModel.findAll({
      attributes: [
        'modelType',
        [
          PredictionModel.sequelize.fn('COUNT', PredictionModel.sequelize.col('id')),
          'totalPredictions',
        ],
        [
          PredictionModel.sequelize.fn('AVG', PredictionModel.sequelize.col('confidence')),
          'avgConfidence',
        ],
        [
          PredictionModel.sequelize.fn('AVG', PredictionModel.sequelize.col('accuracy')),
          'avgAccuracy',
        ],
        [
          PredictionModel.sequelize.fn(
            'COUNT',
            PredictionModel.sequelize.literal('CASE WHEN accuracy >= 0.8 THEN 1 END')
          ),
          'highAccuracyCount',
        ],
      ],
      ...
      group: ['modelType'],
      order: [
        [PredictionModel.sequelize.fn('AVG', PredictionModel.sequelize.col('accuracy')), 'DESC'],
      ],
    });
