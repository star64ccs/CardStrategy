...
class EnhancedPredictionService {
  ...
  async enhancedLSTMPrediction(historicalData, timeframe) {
    try {
      ...
      const bollingerBands = this.technicalIndicators.calculateBollingerBands(prices);
      ...
      return {
        ...
        factors: {
          ...
          technicalStrength: this.calculateTechnicalStrength({ rsi, macd, bollingerBands })
        }
      };
    } catch (error) {
  ...
  async transformerPrediction(historicalData, timeframe) {
    try {
      ...
      // 注意力機制預測
      const prediction = await this.attentionBasedPrediction(sequence, timeframe);

      const confidence = this.calculateTransformerConfidence(sequence, prediction);
  ...
  async technicalEnsemblePrediction(historicalData, timeframe) {
    try {
      ...
      const indicators = {
        ...
        bollingerBands: this.technicalIndicators.calculateBollingerBands(prices),
      ...
      // 基於技術指標的預測
      const prediction = this.technicalBasedPrediction(prices, indicators, timeframe);
  ...
  async predictCardPrice(cardId, timeframe, modelType = 'dynamicEnsemble') {
    try {
      ...
      // 計算風險等級
      const riskLevel = this.calculateEnhancedRiskLevel(prediction.confidence, prediction.factors.volatility);
      ...
      logger.info(`增強預測完成: 卡牌 ${cardId}, 模型 ${modelType}, 時間框架 ${timeframe}`);
  ...
  extractAdvancedFeatures(prices, volumes, indicators) {
    ...
    if (volumes.some(v => v > 0)) {
      features.volumeChange = (volumes[volumes.length - 1] - volumes[0]) / volumes[0];
    ...
    features.bollingerPosition = (prices[prices.length - 1] - indicators.bollingerBands.lower) /
  ...
  encodeSequence(prices) {
    ...
    for (let i = 0; i < prices.length; i++) {
      const normalized = (prices[i] - Math.min(...prices)) / (Math.max(...prices) - Math.min(...prices));
  ...
  calculateEnhancedConfidence(features, prediction) {
    ...
    const confidence = baseConfidence * dataQuality * (1 - volatility * 0.5) * technicalStrength;
  ...
  async calculateRecentPerformance() {
    ...
    return {
      ...
      dynamicEnsemble: 0.90
    };
  }

  calculateEnsembleTrend(predictions) {
    const upCount = Object.values(predictions).filter(p => p.factors.trend === 'up').length;
    const downCount = Object.values(predictions).filter(p => p.factors.trend === 'down').length;
...
class TechnicalIndicators {
  ...
  calculateMACD(prices, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    ...
    const signalLine = this.calculateEMA([...Array(25).fill(0), macdLine], signalPeriod);
