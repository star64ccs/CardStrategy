    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        processedResults.errors.push({
          source: ['user_upload', 'official_api', 'third_party', 'user_correction', 'web_scraping'][
            index
          ],
  ...
  async getCollectionStats(options = {}) {
    try {
      ...
      const dailyStats = await this.TrainingData.findAll({
        attributes: [
          [
            this.TrainingData.sequelize.fn('DATE', this.TrainingData.sequelize.col('createdAt')),
            'date',
          ],
          [this.TrainingData.sequelize.fn('COUNT', this.TrainingData.sequelize.col('id')), 'count'],
        ],
        ...
        trend:
          parseFloat(growthRate) > 0
            ...
            : parseFloat(growthRate) < 0
              ...
              : 'stable',
        averageDailyGrowth:
          periodDays > 0 ? ((secondHalfCount - firstHalfCount) / periodDays).toFixed(2) : 0,
      };
    } catch (error) {
  ...
  async calculateEfficiencyMetrics(startDate, endDate) {
    try {
      ...
      const highQualityRatio =
        totalRecords > 0 ? ((highQualityCount / totalRecords) * 100).toFixed(2) : 0;
