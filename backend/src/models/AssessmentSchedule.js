...
const createAssessmentScheduleModel = (sequelize) => {
  if (AssessmentSchedule) return AssessmentSchedule;

  AssessmentSchedule = sequelize.define('AssessmentSchedule', {
    id: {
    ...
    dataTypes: {
      type: DataTypes.JSON,
      allowNull: false,
      defaultValue: ['training', 'annotation', 'validation', 'market', 'user_generated'],
    },
    assessmentCriteria: {
      type: DataTypes.JSON,
      allowNull: false,
...
const getAssessmentScheduleModel = () => {
  if (!AssessmentSchedule) {
    try {
      const { getSequelize } = require('../config/database');
      const sequelize = getSequelize();
      if (sequelize) {
        AssessmentSchedule = createAssessmentScheduleModel(sequelize);
      }
    } catch (error) {
      console.error('Error creating AssessmentSchedule model:', error);
      return null;
    }
  }
  return AssessmentSchedule;
