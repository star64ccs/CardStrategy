...
const createAnnotationDataModel = (sequelize) => {
  ...
  AnnotationData = sequelize.define('AnnotationData', {
    id: {
    ...
    annotationType: {
      type: DataTypes.ENUM('card_identification', 'condition_assessment', 'authenticity_verification', 'centering_analysis'),
      allowNull: false
    },
    annotationResult: {
      type: DataTypes.JSON,
...
const getAnnotationDataModel = () => {
  if (!AnnotationData) {
    throw new Error('AnnotationData model not initialized. Call createAnnotationDataModel first.');
  }
  return AnnotationData;
};
