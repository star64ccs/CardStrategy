...
export class CryptoKeyManager implements KeyManager {
  ...
  public async importKey(
    ...
    try {
      // 解析密鑰數據
      const parsedKey = this.parseKeyData(keyData, format);
  ...
  private calculateExpirationDate(algorithm: EncryptionAlgorithm): Date {
    const now = new Date();
    const expirationDays = this.getDefaultExpirationDays(algorithm);
    ...
    try {
      ...
      Object.keys(keyStorage).forEach(storageKey => {
        if (storageKey.startsWith('key_')) {
          try {
            const keyData = JSON.parse(keyStorage[storageKey]);
  ...
  private convertToPem(key: EncryptionKey): string {
    // 模擬 PEM 格式轉換
    const header = `-----BEGIN ${key.type.toUpperCase()} KEY-----`;
    const footer = `-----END ${key.type.toUpperCase()} KEY-----`;
    const keyData = key.keyData.match(/.{1,64}/g)?.join('\n') || key.keyData;
  ...
  private parsePemKey(pemData: string): {
    ...
    const keyData = lines.slice(1, -2).join('');
