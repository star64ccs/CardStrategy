...
export class UserBehaviorService {
  ...
  async getBehaviorAnalysis(
    ...
    const patterns = await this.analyzePatterns(filteredEvents);
    const profiles = await this.generateProfiles(filteredEvents);
    const stats = await this.calculateStats(filteredEvents);
    const insights = await this.generateInsights(filteredEvents, stats);
  ...
  async generateReport(
    ...
    const analysis = await this.getBehaviorAnalysis(filter);
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    ...
    const profile = await this.generateUserProfile(userId, userEvents);
    return profile || null;
  }
  ...
  private async generateUserProfile(
    ...
    const existingProfile = this.profiles.find(p => p.userId === userId);
    if (existingProfile) {
  ...
  private calculateConfidence(events: UserBehaviorEvent[]): number {
    ...
    const sampleSize = Math.min(events.length / 100, 1);
