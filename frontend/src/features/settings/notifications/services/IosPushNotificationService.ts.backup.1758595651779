...
export class IOSPushNotificationService {
  ...
  private async loadIOSPushNotificationLibrary(): Promise<IOSPushNotificationLibrary> {
    ...
    return {
      ...
      getDeviceToken: async () => {
        // 模擬獲取設備令牌
        const token = `ios_device_token_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  public async registerForRemoteNotifications(): Promise<boolean> {
    try {
      ...
      if (success) {
        // 註冊成功後，獲取設備令牌
        const token = await this.pushLib.getDeviceToken();
        if (token) {
          this.deviceToken = {
            token,
            timestamp: new Date(),
  ...
  public async sendNotification(
    ...
    try {
      ...
      // 驗證設備令牌
      const isValid = await this.pushLib.validateDeviceToken(deviceToken);
      if (!isValid) {
        return {
          success: false,
