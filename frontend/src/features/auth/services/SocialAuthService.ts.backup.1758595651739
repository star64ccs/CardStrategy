...
export class SocialAuthService {
  ...
  async socialLogin(
    ...
    try {
      ...
      // 發送社交登錄請求
      const response = await api.post<SocialAuthResponse>(
        '/auth/social/login',
        credentials
      );
  ...
  async linkSocialAccount(
    ...
    try {
      ...
      const response = await api.post<SocialAccountLink>(
        '/auth/social/link',
        credentials
      );
  ...
  async unlinkSocialAccount(provider: SocialProvider): Promise<void> {
    try {
      ...
      const response = await api.delete(`/auth/social/unlink/${provider}`);
  ...
  async getLinkedSocialAccounts(): Promise<SocialAccountLink[]> {
    try {
      ...
      const response = await api.get<SocialAccountLink[]>(
  ...
  async isSocialAccountLinked(provider: SocialProvider): Promise<boolean> {
    try {
      const accounts = await this.getLinkedSocialAccounts();
  ...
  async getSocialUserInfo(
    ...
    try {
      ...
      const response = await api.get<SocialUserInfo>(
  ...
  isProviderConfigured(provider: SocialProvider): boolean {
    ...
    switch (provider) {
      case 'google': {
        const config = providerConfig as typeof this.config.google;
        return !!(config.clientId && config.clientSecret);
      }
      case 'facebook': {
        const config = providerConfig as typeof this.config.facebook;
        return !!(config.appId && config.appSecret);
      }
      case 'apple': {
        const config = providerConfig as typeof this.config.apple;
        return !!(
          config.clientId &&
          config.teamId &&
          config.keyId &&
      ...
      case 'twitter': {
        const config = providerConfig as typeof this.config.twitter;
        return !!(config.consumerKey && config.consumerSecret);
      }
      case 'github': {
        const config = providerConfig as typeof this.config.github;
        return !!(config.clientId && config.clientSecret);
      }
      case 'discord': {
        const config = providerConfig as typeof this.config.discord;
        return !!(config.clientId && config.clientSecret);
      }
      case 'line': {
        const config = providerConfig as typeof this.config.line;
        return !!(config.channelId && config.channelSecret);
      }
      case 'kakao': {
        const config = providerConfig as typeof this.config.kakao;
        return !!(config.clientId && config.clientSecret);
      }
      default:
        return false;
