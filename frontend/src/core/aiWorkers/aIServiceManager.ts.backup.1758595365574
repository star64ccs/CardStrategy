...
export class AIServiceManager {
  ...
  private selectProvider(request: AIRequest): string {
    ...
    // 根據優先級和成本選擇提供商
    const availableProviders = Array.from(this.activeProviders.keys());

    // 優先使用本地部署（免費）
    const localProviders = availableProviders.filter(
    ...
    return cloudProviders.reduce((best, current) => {
      const bestProvider = AI_PROVIDERS[best];
      const currentProvider = AI_PROVIDERS[current];
  ...
  public async callAI(request: AIRequest): Promise<AIResponse> {
    ...
    try {
      ...
      // 選擇提供商
      const providerKey = this.selectProvider(request);
      const provider = AI_PROVIDERS[providerKey];
      ...
      // 調用具體的AI服務
      const response = await this.callProvider(providerKey, request);
  ...
  private async callBaidu(
    ...
    const model = request.model || 'ernie-bot-turbo';
  ...
  private async callAlibaba(
    ...
    const response = await fetch(AI_PROVIDERS.alibaba.endpoint, {
      method: 'POST',
      headers: {
  ...
  private async callZhipu(
    ...
    const model = request.model || 'glm-3-turbo';
    ...
    const response = await fetch(AI_PROVIDERS.zhipu.endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${apiKey}`,
  ...
  private async callAzure(
    ...
    const model = request.model || 'gpt-35-turbo';
  ...
  private calculateCost(
    ...
    const inputCost = (tokens.input / 1000) * provider.cost.input;
    const outputCost = (tokens.output / 1000) * provider.cost.output;
  ...
  private updateStats(
    ...
    const providerStats = this.stats.providerUsage[providerKey];
