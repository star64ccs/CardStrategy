...
const designSystemSlice = createSlice({
  ...
  reducers: {
    ...
    registerComponent: (
      ...
      const category = determineComponentCategory(name);
      if (category) {
    ...
    updateComponent: (
      ...
      const { name, config } = action.payload;
      const existingConfig = getComponentFromState(state, name);
      if (existingConfig) {
        const category = determineComponentCategory(name);
        if (category) {
...
export const selectThemes = (state: { designSystem: DesignSystemState }) =>
...
export const selectComponents = (state: { designSystem: DesignSystemState }) =>
  state.designSystem.components;

export const selectTokens = (state: { designSystem: DesignSystemState }) =>
...
export const selectIsLoading = (state: { designSystem: DesignSystemState }) =>
  state.designSystem.isLoading;

export const selectError = (state: { designSystem: DesignSystemState }) =>
...
// 按類別選擇令牌
export const selectTokensByCategory =
