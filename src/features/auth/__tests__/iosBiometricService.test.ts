...
describe('IOSBiometricService', () => {
  ...
  describe('getInstance', () => {
    it('應該返回單例實例', () => {
      const _instance1 = IOSBiometricService.getInstance();
      const _instance2 = IOSBiometricService.getInstance();
  ...
  describe('authenticate', () => {
    it('應該成功執行 iOS 生物識別認證', async () => {
      ...
      const _result = await iosBiometricService.authenticate(request);
  ...
  describe('安全信息管理', () => {
    it('應該提供安全信息', () => {
      const _securityInfo = iosBiometricService.getSecurityInfo();
