...
describe('PWAService', () => {
  ...
  describe('初始化測試', () => {
    ...
    test('應該處理非 Web 平台', async () => {
      const _iosService = new (PWAService as any)();
    ...
    test('應該處理 Service Worker 不支持的情況', async () => {
      const _noSwService = new (PWAService as any)();
  ...
  describe('安裝管理測試', () => {
    ...
    test('應該獲取安裝狀態', () => {
      const _status = service.getInstallStatus();
    ...
    test('應該處理安裝 PWA', async () => {
      ...
      const _beforeInstallHandler = mockWindow.addEventListener.mock.calls.find(
  ...
  describe('網絡監控測試', () => {
    ...
    test('應該處理網絡狀態變化', () => {
      const _onlineHandler = mockWindow.addEventListener.mock.calls.find(
        call => call[0] === 'online'
      )?.[1];

      const _offlineHandler = mockWindow.addEventListener.mock.calls.find(
        call => call[0] === 'offline'
      )?.[1];
  ...
  describe('單例模式測試', () => {
    test('應該返回相同的實例', () => {
      const _instance1 = PWAService.getInstance();
      const _instance2 = PWAService.getInstance();
