...
describe('ConsumerProtectionModule', () => {
  ...
  describe('單例模式測試', () => {
    test('應該返回相同的實例', () => {
      const _instance1 = ConsumerProtectionModule.getInstance();
      const _instance2 = ConsumerProtectionModule.getInstance();
  ...
  describe('初始化測試', () => {
    ...
    test('應該使用自定義配置初始化', async () => {
      ...
      const _result = await consumerProtectionModule.initialize(customConfig);
      expect(result).toBe(true);
    });
  });
  ...
  describe('不公平條款檢測測試', () => {
    ...
    test('應該檢測排除責任條款', () => {
      const _contractText = '本協議排除責任，商家不承擔任何損失';
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        contractText,
        'global'
      );
    ...
    test('應該檢測限制賠償條款', () => {
      const _contractText = '賠償金額限制賠償為最低金額';
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        contractText,
        'global'
      );
    ...
    test('應該檢測懲罰條款', () => {
      const _contractText = '違約將支付懲罰條款規定的賠償';
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        contractText,
        'global'
      );
    ...
    test('應該處理無不公平條款的合同', () => {
      const _contractText = '這是一個公平合理的合同條款';
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        contractText,
        'global'
      );
  ...
  describe('邊界條件測試', () => {
    ...
    test('應該處理非常長的合同文本', () => {
      const _longText = '這是一個非常長的合同文本'.repeat(1000);
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        longText,
        'global'
      );
    ...
    test('應該處理包含數字的合同文本', () => {
      const _numericText = '賠償金額限制為1000元，不得超過合同金額的50%';
      const _detectedTerms = consumerProtectionModule.detectUnfairTerms(
        numericText,
        'global'
      );
  ...
  describe('功能測試', () => {
    ...
    test('應該處理不同地區的消費者權利', () => {
      const _jurisdictions = ['global', 'taiwan', 'macau', 'china', 'eu', 'us'];

      jurisdictions.forEach(jurisdiction => {
        const _rights = consumerProtectionModule.getConsumerRights(jurisdiction);
