...
describe('RegulationUpdateMonitor', () => {
  ...
  describe('getInstance', () => {
    it('應該返回單例實例', () => {
      const _instance1 = RegulationUpdateMonitor.getInstance();
      const _instance2 = RegulationUpdateMonitor.getInstance();
  ...
  describe('startMonitoring', () => {
    ...
    it('應該立即執行一次檢查', async () => {
      const _checkSpy = jest.spyOn(monitor, 'checkForUpdates');
  ...
  describe('manualCheck', () => {
    ...
    it('應該返回更新結果', async () => {
      const _updates = await monitor.manualCheck();
  ...
  describe('GDPR 更新處理', () => {
    ...
    it('應該檢測到 GDPR 更新', async () => {
      const _updates = await monitor.checkForUpdates();
      const _gdprUpdates = updates.filter(u => u.regulation === 'GDPR');
    ...
    it('應該正確識別 GDPR 受影響模組', async () => {
      const _updates = await monitor.checkForUpdates();
      const _gdprUpdate = updates.find(u => u.regulation === 'GDPR');
  ...
  describe('CCPA 更新處理', () => {
    ...
    it('應該檢測到 CCPA 更新', async () => {
      const _updates = await monitor.checkForUpdates();
      const _ccpaUpdates = updates.filter(u => u.regulation === 'CCPA');
    ...
    it('應該正確識別 CCPA 受影響模組', async () => {
      const _updates = await monitor.checkForUpdates();
      const _ccpaUpdate = updates.find(u => u.regulation === 'CCPA');
  ...
  describe('合規影響評估', () => {
    ...
    it('應該正確評估 GDPR 影響', async () => {
      const _impact = await (monitor as any).assessComplianceImpact(mockUpdate);
  ...
  describe('實施計劃生成', () => {
    ...
    it('應該生成完整的實施計劃', async () => {
      const _plan = await (monitor as any).generateImplementationPlan(
        mockUpdate,
        mockImpact
      );
