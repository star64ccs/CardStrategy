...
describe('AccessibilityService', () => {
  ...
  describe('Singleton Pattern', () => {
    it('應該返回相同的實例', () => {
      const _instance1 = AccessibilityService.getInstance();
      const _instance2 = AccessibilityService.getInstance();
  ...
  describe('無障礙模式管理', () => {
    ...
    it('應該獲取當前配置文件', () => {
      const _profile = accessibilityService.getCurrentProfile();
      expect(profile).toBeDefined();
      expect(profile.id).toBeDefined();
    });

    it('應該切換配置文件', async () => {
      const _profiles = accessibilityService.getAvailableProfiles();
      if (profiles.length > 1) {
        const _newProfileId = profiles[1].id;
  ...
  describe('配置管理', () => {
    ...
    it('應該獲取當前配置', () => {
      const _config = accessibilityService.getConfig();
    ...
    it('應該更新配置', async () => {
      ...
      const _updatedConfig = accessibilityService.getConfig();
  ...
  describe('工具功能', () => {
    ...
    it('應該計算顏色對比度', () => {
      const _contrast = accessibilityService.calculateContrastRatio(
        '#000000',
        '#FFFFFF'
      );
    ...
    it('應該檢查顏色對比度是否足夠', () => {
      const _isSufficient = accessibilityService.isHighContrast(
        '#000000',
        '#FFFFFF'
      );
    ...
    it('應該檢查功能是否啟用', () => {
      const _isEnabled = accessibilityService.isFeatureEnabled('highContrast');
