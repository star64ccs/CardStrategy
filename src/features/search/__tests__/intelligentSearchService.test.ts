...
describe('IntelligentSearchService', () => {
  ...
  describe('Singleton Pattern', () => {
    it('應該返回相同的實例', () => {
      const _instance1 = IntelligentSearchService.getInstance();
      const _instance2 = IntelligentSearchService.getInstance();
  ...
  describe('初始化', () => {
    ...
    it('應該在初始化失敗時返回 false', async () => {
      ...
      const _result = await newService.initialize();
  ...
  describe('智能搜索', () => {
    ...
    it('應該執行基本智能搜索', async () => {
      ...
      const _response = await service.search(query);
  ...
  describe('搜索建議', () => {
    ...
    it('應該獲取搜索建議', async () => {
      const _suggestions = await service.getSuggestions('char');
    ...
    it('應該包含熱門搜索建議', async () => {
      const _suggestions = await service.getSuggestions('char');
      const _popularSuggestions = suggestions.filter(s => s.type === 'popular');
  ...
  describe('搜索歷史', () => {
    ...
    it('應該保存搜索歷史', async () => {
      ...
      const _query = 'Charizard';
      const _results = ['1', '2'];

      await service.saveSearchHistory(userId, query, results);

      const _history = await service.getSearchHistory(userId);
  ...
  describe('熱門搜索', () => {
    ...
    it('應該獲取所有熱門搜索', async () => {
      const _popularSearches = await service.getPopularSearches();
    ...
    it('應該按類別獲取熱門搜索', async () => {
      const _popularSearches = await service.getPopularSearches('Pokemon');
  ...
  describe('相關搜索', () => {
    ...
    it('應該獲取相關搜索', async () => {
      const _relatedSearches = await service.getRelatedSearches('Charizard');
  ...
  describe('查詢分析', () => {
    ...
    it('應該分析查詢', async () => {
      const _analysis = await service.analyzeQuery('Pokemon Charizard price');
    ...
    it('應該識別實體', async () => {
      const _analysis = await service.analyzeQuery('Pokemon Charizard');

      expect(analysis.entities.length).toBeGreaterThan(0);
      const _cardEntity = analysis.entities.find(e => e.type === 'card_name');
      expect(cardEntity).toBeDefined();
      expect(cardEntity?.text).toBe('Charizard');
    });

    it('應該確定搜索意圖', async () => {
      const _analysis = await service.analyzeQuery('Charizard price');
  ...
  describe('用戶偏好', () => {
    ...
    it('應該更新用戶偏好', async () => {
      ...
      const _savedPreferences = await service.getUserPreferences(userId);
    ...
    it('應該獲取用戶偏好', async () => {
      ...
      const _preferences = await service.getUserPreferences(userId);
  ...
  describe('搜索統計', () => {
    ...
    it('應該獲取搜索統計', async () => {
      const _stats = await service.getSearchStats();
  ...
  describe('自動完成', () => {
    ...
    it('應該獲取自動完成選項', async () => {
      const _autoComplete = await service.getAutoComplete('char');
    ...
    it('應該支持上下文', async () => {
      ...
      const _autoComplete = await service.getAutoComplete('char', context);
      expect(Array.isArray(autoComplete)).toBe(true);
    });
  });
  ...
  describe('邊界情況', () => {
    ...
    it('應該處理空結果', async () => {
      ...
      const _response = await newService.search(query);
