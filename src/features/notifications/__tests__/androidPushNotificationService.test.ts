...
describe('AndroidPushNotificationService', () => {
  ...
  describe('推送通知發送測試', () => {
    ...
    test('應該成功批量發送推送通知', async () => {
      ...
      const _results = await service.sendBulkNotifications(
        deviceTokens,
        mockPayload
      );
    ...
    test('應該處理推送通知發送失敗', async () => {
      ...
      const _result = await service.sendNotification('test-token', mockPayload);
    ...
    test('應該處理配置未設置的情況', async () => {
      const _unconfiguredService = new (AndroidPushNotificationService as any)();
      await unconfiguredService.initializeAndroidPushNotificationLibrary();

      const _result = await unconfiguredService.sendNotification(
        'test-token',
        mockPayload
      );
  ...
  describe('主題訂閱測試', () => {
    ...
    test('應該處理主題訂閱失敗', async () => {
      ...
      const _result = await service.subscribeToTopic('test-topic');
  ...
  describe('統計和監控測試', () => {
    ...
    test('應該成功驗證設備令牌', async () => {
      ...
      const _validResult = await service.validateDeviceToken(validToken);
      const _invalidResult = await service.validateDeviceToken(invalidToken);
  ...
  describe('錯誤處理測試', () => {
    ...
    test('應該處理推送通知發送異常', async () => {
      ...
      const _result = await service.sendNotification('test-token', testPayload);
  ...
  describe('Android 特定功能測試', () => {
    ...
    test('應該支持 Android 特定的推送載荷', async () => {
      ...
      const _result = await service.sendNotification(
        'test-token',
  ...
  describe('並發測試', () => {
    ...
    test('應該支持並發主題訂閱', async () => {
      const _topics = ['topic1', 'topic2', 'topic3'];

      const _subscribePromises = topics.map(topic =>
        service.subscribeToTopic(topic)
      );
      const _subscribeResults = await Promise.all(subscribePromises);
