...
describe('IOSPushNotificationService', () => {
  ...
  describe('getInstance', () => {
    it('應該返回單例實例', () => {
      const _instance1 = IOSPushNotificationService.getInstance();
      const _instance2 = IOSPushNotificationService.getInstance();
  ...
  describe('requestPermissions', () => {
    ...
    it('應該處理權限請求失敗', async () => {
      ...
      const _result = await newService.requestPermissions();
  ...
  describe('registerForRemoteNotifications', () => {
    ...
    it('應該處理註冊失敗', async () => {
      ...
      const _result = await newService.registerForRemoteNotifications();
  ...
  describe('sendNotification', () => {
    ...
    it('應該處理無效設備令牌', async () => {
      const _invalidToken = 'invalid_token';
      const payload: IOSPushNotificationPayload = {
        aps: {
          alert: {
            body: '測試內容',
  ...
  describe('sendBulkNotifications', () => {
    ...
    it('應該成功批量發送推送通知', async () => {
      ...
      const _results = await iosPushService.sendBulkNotifications(
        deviceTokens,
        payload
      );
    ...
    it('應該處理批量發送失敗', async () => {
      ...
      const _deviceTokens = ['token1', 'token2'];
      const payload: IOSPushNotificationPayload = {
        aps: {
          alert: {
            body: '測試內容',
      ...
      const _results = await newService.sendBulkNotifications(
        deviceTokens,
        payload
      );
  ...
  describe('sendLocalNotification', () => {
    it('應該成功發送本地推送通知', async () => {
      ...
      const _result = await iosPushService.sendLocalNotification(
        title,
        body,
        options
      );
  ...
  describe('getDeliveryStats', () => {
    it('應該返回投遞統計信息', async () => {
      const _stats = await iosPushService.getDeliveryStats();
  ...
  describe('validateDeviceToken', () => {
    it('應該正確驗證有效的設備令牌', async () => {
      const _validToken = 'ios_device_token_test_123456789';
      const _result = await iosPushService.validateDeviceToken(validToken);
    ...
    it('應該正確驗證無效的設備令牌', async () => {
      const _invalidToken = 'invalid_token';
      const _result = await iosPushService.validateDeviceToken(invalidToken);
  ...
  describe('錯誤處理', () => {
    it('應該正確處理服務未初始化的情況', async () => {
      ...
      const _permissionsResult = await newService.requestPermissions();
      expect(permissionsResult).toBe(false);

      const _registerResult = await newService.registerForRemoteNotifications();
      expect(registerResult).toBe(false);

      const _unregisterResult =
        await newService.unregisterForRemoteNotifications();
      expect(unregisterResult).toBe(false);
    });

    it('應該正確處理未配置的情況', async () => {
      const _deviceToken = 'test_token';
      const payload: IOSPushNotificationPayload = {
        aps: {
          alert: {
            body: '測試內容',
  ...
  describe('性能測試', () => {
    ...
    it('應該能夠快速批量發送推送通知', async () => {
      ...
      const _deviceTokens = Array.from(
        { length: 10 },
