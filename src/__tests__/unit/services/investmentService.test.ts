...
describe('InvestmentService', () => {
  ...
  describe('getInvestment', () => {
    ...
    it('應該處理 API 錯誤', async () => {
      ...
      await expect(
        investmentService.getInvestment(mockInvestmentId)
      ).rejects.toThrow('投資不存在');
    });
  });
  ...
  describe('deletePriceAlert', () => {
    ...
    it('應該成功刪除價格警報', async () => {
      ...
      expect(mockApiService.delete).toHaveBeenCalledWith(
        `/investments/price-alerts/${mockAlertId}`
      );
    });
  ...
  describe('exportInvestmentReport', () => {
    ...
    it('應該成功導出 JSON 報告', async () => {
      ...
      expect(mockApiService.get).toHaveBeenCalledWith(
        '/investments/export?format=json'
      );
    });

    it('應該處理無效的格式', async () => {
      await expect(
        investmentService.exportInvestmentReport('xml' as any)
      ).rejects.toThrow();
    });
