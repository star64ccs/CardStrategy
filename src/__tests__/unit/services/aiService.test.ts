...
describe('AIService', () => {
  ...
  describe('getCardAnalysis', () => {
    ...
    it('應該處理 API 錯誤', async () => {
      ...
      await expect(aiService.getCardAnalysis(mockCardId)).rejects.toThrow('API 錯誤');
  ...
  describe('getPricePrediction', () => {
    ...
    it('應該成功獲取價格預測', async () => {
      ...
      expect(mockApiService.post).toHaveBeenCalledWith(
        expect.any(String),
        { cardId: mockCardId, timeframe: '7d' }
      );
    });

    it('應該處理無效的時間框架', async () => {
      await expect(aiService.getPricePrediction(mockCardId, 'invalid' as any)).rejects.toThrow();
    });

    it('應該處理 API 錯誤', async () => {
      mockApiService.post.mockRejectedValue(new Error('預測服務錯誤'));

      await expect(aiService.getPricePrediction(mockCardId, '7d')).rejects.toThrow('預測服務錯誤');
      expect(mockLogger.error).toHaveBeenCalled();
    });
  });
  ...
  describe('getInvestmentRecommendation', () => {
    ...
    it('應該處理無效的卡牌 ID', async () => {
      ...
      await expect(aiService.getInvestmentRecommendation(invalidCardId)).rejects.toThrow();
    });

    it('應該處理 API 錯誤', async () => {
      mockApiService.post.mockRejectedValue(new Error('建議服務錯誤'));

      await expect(aiService.getInvestmentRecommendation(mockCardId)).rejects.toThrow('建議服務錯誤');
      expect(mockLogger.error).toHaveBeenCalled();
    });
  });
  ...
  describe('sendChatMessage', () => {
    ...
    it('應該成功發送聊天消息', async () => {
      ...
      const result = await aiService.sendChatMessage(mockMessage, mockSessionId);

      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockChatMessage);
      expect(mockApiService.post).toHaveBeenCalledWith(
        expect.any(String),
        { message: mockMessage, sessionId: mockSessionId }
      );
    });
    ...
    it('應該處理無效的會話 ID', async () => {
      ...
      await expect(aiService.sendChatMessage(mockMessage, invalidSessionId)).rejects.toThrow();
    });

    it('應該處理 API 錯誤', async () => {
      mockApiService.post.mockRejectedValue(new Error('聊天服務錯誤'));

      await expect(aiService.sendChatMessage(mockMessage)).rejects.toThrow('聊天服務錯誤');
      expect(mockLogger.error).toHaveBeenCalled();
    });
  });
