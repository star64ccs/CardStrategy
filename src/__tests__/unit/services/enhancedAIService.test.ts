...
describe('EnhancedAIService', () => {
  ...
  describe('enhancedRecognizeCard', () => {
    ...
    it('應該處理 API 錯誤', async () => {
      ...
      await expect(enhancedAIService.enhancedRecognizeCard(mockImageData)).rejects.toThrow('API 錯誤');
  ...
  describe('enhancedAnalyzeCondition', () => {
    ...
    it('應該成功分析卡片條件', async () => {
      ...
      const result = await enhancedAIService.enhancedAnalyzeCondition(mockCardId, mockImageData);
  ...
  describe('enhancedVerifyAuthenticity', () => {
    ...
    it('應該成功驗證真偽', async () => {
      ...
      const result = await enhancedAIService.enhancedVerifyAuthenticity(mockCardId, mockImageData);
  ...
  describe('enhancedPricePrediction', () => {
    ...
    it('應該成功預測價格', async () => {
      ...
      const result = await enhancedAIService.enhancedPricePrediction(mockCardId, ['7d', '30d', '90d']);
      ...
      expect(mockApiService.post).toHaveBeenCalledWith(
        expect.any(String),
        { cardId: mockCardId, timeframes: ['7d', '30d', '90d'] }
      );
    });
  });
  ...
  describe('comprehensiveAnalysis', () => {
    ...
    it('應該執行綜合分析', async () => {
      ...
      const result = await enhancedAIService.comprehensiveAnalysis(mockImageData);
      ...
      expect(result.conditionAnalysis.data.analysis.overallGrade).toBe('Near Mint');
      expect(result.authenticityVerification.data.authenticity.riskLevel).toBe('low');
      expect(result.pricePrediction.data.predictions[0].predictedPrice).toBe(150.5);
    });
    ...
    it('應該處理識別失敗的情況', async () => {
      ...
      await expect(enhancedAIService.comprehensiveAnalysis(mockImageData)).rejects.toThrow('卡片識別失敗，無法進行後續分析');
    });
  });
  ...
  describe('getConfig', () => {
    it('應該返回當前配置', () => {
      ...
      expect(config.authenticityVerification.useBlockchainVerification).toBe(true);
  ...
  describe('updateConfig', () => {
    it('應該更新配置', () => {
      ...
      expect(updatedConfig.authenticityVerification.useBlockchainVerification).toBe(true); // 保持原值
    });
  });
});
