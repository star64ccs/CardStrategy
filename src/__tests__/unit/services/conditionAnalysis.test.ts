...
describe('模擬鑑定功能測試', () => {
  ...
  describe('卡片狀況評級測試', () => {
    it('應該正確評級完美狀況卡片', async () => {
      const _mockImageData = 'data:image/jpeg;base64,mint_condition...';
      ...
      const _result = await enhancedAIService.enhancedConditionAnalysis(
        mockImageData,
        {
    ...
    it('應該正確評級受損卡片', async () => {
      const _mockImageData = 'data:image/jpeg;base64,damaged_card...';
      const _mockConditionResult = {
        success: true,
        data: {
          analysis: {
      ...
      const _result =
        await enhancedAIService.enhancedConditionAnalysis(mockImageData);
  ...
  describe('損傷評估測試', () => {
    it('應該正確評估卡片損傷', async () => {
      const _mockImageData = 'data:image/jpeg;base64,damage_assessment...';
      const _mockDamageResult = {
      ...
      const _mockCondition = require('../../../services/conditionService');
      mockCondition.conditionService.assessDamage.mockResolvedValue(
        mockDamageResult
      );

      const _result = await conditionService.assessDamage(mockImageData);
  ...
  describe('保存狀態分析測試', () => {
    it('應該正確分析保存狀態', async () => {
      const _mockImageData = 'data:image/jpeg;base64,preservation_state...';
      const _mockPreservationResult = {
      ...
      const _mockCondition = require('../../../services/conditionService');
      mockCondition.conditionService.analyzePreservation.mockResolvedValue(
        mockPreservationResult
      );

      const _result = await conditionService.analyzePreservation(mockImageData);
  ...
  describe('紫外線檢查測試', () => {
    it('應該正確進行紫外線檢查', async () => {
      const _mockImageData = 'data:image/jpeg;base64,uv_inspection...';
      const _mockUVResult = {
        uvResponse: 'normal',
        score: 9.8,
        findings: {
      ...
      const _mockCondition = require('../../../services/conditionService');
      mockCondition.conditionService.uvInspection.mockResolvedValue(
        mockUVResult
      );

      const _result = await conditionService.uvInspection(mockImageData);
  ...
  describe('顯微鏡分析測試', () => {
    it('應該正確進行顯微鏡分析', async () => {
      const _mockImageData = 'data:image/jpeg;base64,microscopic_analysis...';
      const _mockMicroscopicResult = {
      ...
      const _mockCondition = require('../../../services/conditionService');
      mockCondition.conditionService.microscopicAnalysis.mockResolvedValue(
        mockMicroscopicResult
      );

      const _result = await conditionService.microscopicAnalysis(mockImageData);
