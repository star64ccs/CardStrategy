...
export const _useTouch = (
  ...
  const _handleTouchStart = useCallback(
    (event: React.TouchEvent) => {
      ...
      const _touches = Array.from(event.touches).map(touch => ({
  ...
  const _handleTouchMove = useCallback(
    (event: React.TouchEvent) => {
      ...
      const _point = getTouchPoint(event);
      const _touches = Array.from(event.touches);

      if (touches.length === 2) {
        // 多點觸控手勢
        const _touch1 = touches[0];
        const _touch2 = touches[1];
        const _point1 = { x: touch1.clientX, y: touch1.clientY };
        const _point2 = { x: touch2.clientX, y: touch2.clientY };

        const _distance = calculateDistance(point1, point2);
        const _angle = calculateAngle(point1, point2);
  ...
  const _handleKeyDown = useCallback((event: React.KeyboardEvent) => {
  ...
  const _handleKeyUp = useCallback((event: React.KeyboardEvent) => {
    if (event.key === 'Enter' || event.key === ' ') {
      setIsPressed(false);
      setIsFocused(false);
    }
