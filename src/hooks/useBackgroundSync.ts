...
export interface BackgroundSyncActions {
  ...
  // 便捷方法
  addApiTask: (url: string, method: string, data?: any, priority?: 'high' | 'medium' | 'low') => string;
  addDataTask: (url: string, data: any, priority?: 'high' | 'medium' | 'low') => string;
  addFileTask: (url: string, file: File, priority?: 'high' | 'medium' | 'low') => string;
  addNotificationTask: (url: string, notification: any, priority?: 'high' | 'medium' | 'low') => string;
  ...
  testConflictResolution: (clientData: any, serverData: any, strategy: ConflictResolutionStrategy) => Promise<ConflictResolution>;
}
...
export const useBackgroundSync = (): BackgroundSyncState & BackgroundSyncActions => {
  const [state, setState] = useState<BackgroundSyncState>({
    ...
    isLoading: false,
    error: null,
    conflictResolutionConfig: backgroundSyncManager.getConflictResolutionConfig()
  });
  ...
  // 添加任務
  const addTask = useCallback((task: Omit<SyncTask, 'id' | 'retryCount' | 'createdAt'>): string => {
    try {
  ...
  // 批量添加任務
  const addTasks = useCallback((tasks: Omit<SyncTask, 'id' | 'retryCount' | 'createdAt'>[]): string[] => {
    try {
  ...
  // 移除任務
  const removeTask = useCallback((id: string): boolean => {
    try {
  ...
  // 清理過期任務
  const cleanupExpiredTasks = useCallback((maxAge?: number): number => {
    try {
  ...
  // 衝突解決功能
  const setConflictResolutionStrategy = useCallback((taskId: string, strategy: ConflictResolutionStrategy) => {
    try {
      const success = backgroundSyncManager.setConflictResolutionStrategy(taskId, strategy);
      updateState();
      return success;
    } catch (error) {
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '設置衝突解決策略失敗'
      }));
      return false;
    }
  }, [updateState]);

  const addCustomResolver = useCallback((key: string, resolver: (client: any, server: any) => any) => {
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '添加自定義解析器失敗'
      }));
    }
  }, [updateState]);

  const removeCustomResolver = useCallback((key: string) => {
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '移除自定義解析器失敗'
      }));
      return false;
    }
  }, [updateState]);

  const updateConflictResolutionConfig = useCallback((config: Partial<ConflictResolutionConfig>) => {
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '更新衝突解決配置失敗'
      }));
    }
  }, [updateState]);

  const testConflictResolution = useCallback(async (clientData: any, serverData: any, strategy: ConflictResolutionStrategy) => {
    try {
      const result = await backgroundSyncManager.testConflictResolution(clientData, serverData, strategy);
      return result;
    } catch (error) {
      setState(prev => ({
        ...prev,
