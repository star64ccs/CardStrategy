...
export const CardSchema = BaseEntitySchema.extend({
  name: z
    .string()
    .min(1, '卡牌名稱不能為空')
    .max(100, '卡牌名稱不能超過 100 個字元'),
  setName: z
    .string()
    .min(1, '系列名稱不能為空')
    .max(50, '系列名稱不能超過 50 個字元'),
...
export const SecuritySettingsSchema = z.object({
  sessionTimeout: z
    .number()
    .int()
    .min(300, '會話超時時間至少為 5 分鐘')
    .max(86400, '會話超時時間不能超過 24 小時'),
...
export const CardFiltersSchema = z.object({
  rarity: z
    .array(z.enum(['common', 'uncommon', 'rare', 'mythic', 'special', 'promo']))
    .optional(),
  type: z
    .array(
      z.enum([
