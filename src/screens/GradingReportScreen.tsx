...
import { simulatedGradingService, GradingReport } from '../services/simulatedGradingService';
...
export const GradingReportScreen: React.FC<GradingReportScreenProps> = ({ navigation, route }) => {
  const { colors } = useTheme();
  const { t } = useTranslation();

  const [report, setReport] = useState<GradingReport | null>(route.params.report || null);
  ...
  const loadGradingReport = async (gradingNumber: string) => {
    ...
    try {
      const response = await simulatedGradingService.getGradingReport(gradingNumber);
  ...
  const getAgencyColor = (agency: string) => {
    switch (agency) {
      case 'PSA': return '#2196F3';
      case 'BGS': return '#FF5722';
      case 'CGC': return '#9C27B0';
  ...
  const renderSubGrades = () => {
    ...
    return (
      <Card style={styles.subGradesCard}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>子評分</Text>
        <View style={styles.subGradesGrid}>
          {subGradeItems.map((item, index) => (
            <View key={index} style={styles.subGradeItem}>
              <Text style={[styles.subGradeLabel, { color: colors.textSecondary }]}>
                {item.label}
              </Text>
              <View style={[styles.subGradeBadge, { backgroundColor: item.color }]}>
  ...
  const renderMarketValue = () => {
    ...
    return (
      <Card style={styles.marketValueCard}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>市場價值評估</Text>
        <View style={styles.marketValueContent}>
          <View style={styles.estimatedValue}>
            <Text style={[styles.valueLabel, { color: colors.textSecondary }]}>預估價值</Text>
            ...
            <Text style={[styles.valueRange, { color: colors.textSecondary }]}>
              {formatCurrency(marketValue.range.min)} - {formatCurrency(marketValue.range.max)}
            </Text>
          </View>
          <View style={styles.valueFactors}>
            <Text style={[styles.factorsTitle, { color: colors.textSecondary }]}>影響因素:</Text>
  ...
  const renderDetails = () => {
    ...
    return (
      <Card style={styles.detailsCard}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>鑑定詳情</Text>
  ...
  const renderRecommendations = () => {
    ...
    return (
      <Card style={styles.recommendationsCard}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>改善建議</Text>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: colors.text }]}>載入鑑定報告中...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.cardInfoCard}>
          <View style={styles.cardInfoHeader}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>卡牌信息</Text>
            <View style={[styles.agencyBadge, { backgroundColor: getAgencyColor(report.agency) }]}>
          ...
          <View style={styles.cardInfoContent}>
            {report.cardInfo.imageUrl && (
              <Image source={{ uri: report.cardInfo.imageUrl }} style={styles.cardImage} />
            )}
        ...
        {report.gradingResult.details?.comprehensiveAnalysis && (
          <Card style={styles.comprehensiveCard}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>綜合鑑定結果</Text>
            <Text style={[styles.comprehensiveDescription, { color: colors.textSecondary }]}>
              系統自動參考多個專業鑑定機構的評分準則
            </Text>

            <View style={styles.comprehensiveGrades}>
              <View style={styles.comprehensiveGradeItem}>
                <Text style={[styles.comprehensiveGradeLabel, { color: colors.textSecondary }]}>PSA</Text>
                <View style={[styles.comprehensiveGradeBadge, { backgroundColor: getAgencyColor('PSA') }]}>
                  <Text style={styles.comprehensiveGradeText}>
                    {report.gradingResult.details.comprehensiveAnalysis.psaGrade}
                  </Text>
                </View>
              </View>

              <View style={styles.comprehensiveGradeItem}>
                <Text style={[styles.comprehensiveGradeLabel, { color: colors.textSecondary }]}>BGS</Text>
                <View style={[styles.comprehensiveGradeBadge, { backgroundColor: getAgencyColor('BGS') }]}>
                  <Text style={styles.comprehensiveGradeText}>
                    {report.gradingResult.details.comprehensiveAnalysis.bgsGrade}
                  </Text>
                </View>
              </View>

              <View style={styles.comprehensiveGradeItem}>
                <Text style={[styles.comprehensiveGradeLabel, { color: colors.textSecondary }]}>CGC</Text>
                <View style={[styles.comprehensiveGradeBadge, { backgroundColor: getAgencyColor('CGC') }]}>
                  <Text style={styles.comprehensiveGradeText}>
                    {report.gradingResult.details.comprehensiveAnalysis.cgcGrade}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.selectionReason}>
              <Text style={[styles.selectionReasonText, { color: colors.textSecondary }]}>
                {report.gradingResult.details.comprehensiveAnalysis.selectionReason}
              </Text>
            </View>
          </Card>
        )}
        ...
        <Card style={styles.reportInfoCard}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>報告信息</Text>
          <View style={styles.reportInfoList}>
            <View style={styles.reportInfoItem}>
              <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>信心度:</Text>
              <Text style={[styles.infoValue, { color: colors.text }]}>
                {(report.gradingResult.confidence * 100).toFixed(0)}%
              </Text>
            </View>
            <View style={styles.reportInfoItem}>
              <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>查看次數:</Text>
              <Text style={[styles.infoValue, { color: colors.text }]}>
                {report.viewCount}
              </Text>
            </View>
            <View style={styles.reportInfoItem}>
              <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>創建時間:</Text>
              <Text style={[styles.infoValue, { color: colors.text }]}>
                {new Date(report.createdAt).toLocaleDateString('zh-TW')}
              </Text>
            </View>
            <View style={styles.reportInfoItem}>
              <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>有效期至:</Text>
              <Text style={[styles.infoValue, { color: colors.text }]}>
