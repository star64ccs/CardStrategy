...
const FeedbackManagementScreen: React.FC = () => {
    ...
    async (page = 1, refresh = false) => {
      try {
        ...
        if (refresh) {
          ...
          setFeedbacks((prev) =>
            page === 1 ? data.feedbacks : [...prev, ...data.feedbacks]
          );
        }
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([loadFeedbacks(1, true), loadStats(), loadSuggestions()]);
    setRefreshing(false);
    ...
    <Card key={feedback.id} style={styles.feedbackCard}>
      <View style={styles.feedbackHeader}>
        <View style={styles.feedbackTitle}>
          <Text
            style={[styles.feedbackTitleText, { color: theme.colors.text }]}
          >
            {feedback.title}
          </Text>
        ...
        <Text
          style={[styles.feedbackMeta, { color: theme.colors.textSecondary }]}
        >
      ...
      <Text
        style={[styles.feedbackDescription, { color: theme.colors.text }]}
        numberOfLines={2}
      >
      ...
      <View style={styles.feedbackFooter}>
        <Text
          style={[styles.feedbackUser, { color: theme.colors.textSecondary }]}
        >
          由 {feedback.user.username} 提交
        </Text>
        <Text
          style={[styles.feedbackDate, { color: theme.colors.textSecondary }]}
        >
  ...
  const renderStatsCharts = () => {
    ...
    const pieData = Object.entries(stats.feedbackTypeDistribution).map(
      ([key, value], index) => ({
        name: key,
        population: value,
    ...
    <View style={styles.suggestionsContainer}>
      ...
      {suggestions.map((suggestion, index) => (
        <Card key={index} style={styles.suggestionCard}>
          ...
          <Text
            style={[styles.suggestionAction, { color: theme.colors.primary }]}
          >
          ...
          <ScrollView style={styles.detailContent}>
            ...
            <View style={styles.detailActions}>
              <Button
                title="更新狀態"
                onPress={() => {
                  Alert.prompt('更新狀態', '請選擇新狀態', [
                    { text: '取消', style: 'cancel' },
                    {
                      text: '待處理',
                      onPress: () =>
                        handleStatusUpdate(selectedFeedback.id, 'pending'),
                    },
                    {
                      text: '審核中',
                      onPress: () =>
                        handleStatusUpdate(selectedFeedback.id, 'under_review'),
                    },
                    {
                      text: '處理中',
                      onPress: () =>
                    ...
                    },
                    {
                      text: '已關閉',
                      onPress: () =>
                        handleStatusUpdate(selectedFeedback.id, 'closed'),
                    },
                  ]);
                }}
                style={styles.actionButton}
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          responseType === type ? 'white' : theme.colors.text,
                      },
                    ]}
                  >
                    {type}
