...
const DataQualityAssessmentScreen: React.FC = () => {
  ...
  const [currentTab, setCurrentTab] = useState<
    'overview' | 'schedules' | 'assessments'
  >('overview');
  const [showCreateScheduleModal, setShowCreateScheduleModal] = useState(false);
  const [showExecuteAssessmentModal, setShowExecuteAssessmentModal] =
    useState(false);
  const [showAssessmentDetailModal, setShowAssessmentDetailModal] =
    useState(false);
  const [selectedAssessment, setSelectedAssessment] =
    useState<DataQualityAssessment | null>(null);
  ...
  const loadSchedules = async () => {
    try {
      const schedulesData = await dataQualityService.getAssessmentSchedules({
        limit: 10,
      });
  ...
  const loadAssessments = async () => {
    try {
      const assessmentsData = await dataQualityService.getAssessments({
        limit: 10,
      });
  ...
  const handleViewAssessmentDetail = async (assessmentId: number) => {
    try {
      const assessment =
        await dataQualityService.getAssessmentById(assessmentId);
        ...
        <Card style={[styles.chartCard, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.legend}>
            <View style={styles.legendItem}>
              <View
                style={[styles.legendColor, { backgroundColor: '#36A2EB' }]}
              />
            ...
            <View style={styles.legendItem}>
              <View
                style={[styles.legendColor, { backgroundColor: '#4BC0C0' }]}
              />
              <Text style={[styles.legendText, { color: colors.text }]}>
                完成
              </Text>
        ...
        {schedules.map((schedule) => (
          ...
          >
            ...
            <View style={styles.scheduleDetails}>
              <Text
                style={[styles.scheduleDetail, { color: colors.textSecondary }]}
              >
              ...
              <Text
                style={[styles.scheduleDetail, { color: colors.textSecondary }]}
              >
                總運行: {schedule.totalRuns} | 成功: {schedule.successfulRuns} |
                失敗: {schedule.failedRuns}
              </Text>
            ...
            <View style={styles.scheduleActions}>
              ...
              <Button
                ...
                style={[
                  styles.actionButton,
                  {
                    backgroundColor: schedule.isActive ? '#F44336' : '#4CAF50',
                  },
                ]}
              />
            </View>
    ...
    return (
      <ScrollView style={styles.container}>
        ...
        {assessments.map((assessment) => (
          <Card
            key={assessment.id}
            style={[styles.assessmentCard, { backgroundColor: colors.card }]}
          >
        ...
        >
          <Text
            style={[
              styles.tabText,
              { color: colors.text },
              currentTab === 'schedules' && styles.activeTabText,
            ]}
          >
            計劃
          </Text>
        ...
        >
          <Text
            style={[
              styles.tabText,
              { color: colors.text },
              currentTab === 'assessments' && styles.activeTabText,
            ]}
          >
            記錄
          </Text>
      ...
      >
        {selectedAssessment && (
          <ScrollView style={styles.modalContent}>
            ...
            {selectedAssessment.results?.recommendations &&
              selectedAssessment.results.recommendations.length > 0 && (
                <View style={styles.recommendationsSection}>
                  <Text style={[styles.modalTitle, { color: colors.text }]}>
                    改進建議
                  </Text>
                  {selectedAssessment.results.recommendations.map(
                    (rec, index) => (
                      <Card
                        key={index}
                        style={[
