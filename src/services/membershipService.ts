...
const UpgradeMembershipRequestSchema = z.object({
  tier: z.enum(['trial', 'premium', 'vip'], { errorMap: () => ({ message: '無效的會員等級' }) })
});
...
class MembershipService {
  ...
  async getStatus(): Promise<MembershipStatus> {
    try {
      ...
      const validationResult = validateApiResponse(MembershipStatusSchema, response.data);
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '會員狀態數據驗證失敗');
      }
  ...
  async upgrade(tier: string): Promise<MembershipStatus> {
    try {
      const validationResult = validateInput(UpgradeMembershipRequestSchema, { tier });
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '升級參數驗證失敗');
      }
      const response = await apiService.post<MembershipStatus>('/membership/upgrade', {
        tier: validationResult.data!.tier
      });
      const responseValidation = validateApiResponse(MembershipStatusSchema, response.data);
      if (!responseValidation.isValid) {
        throw new Error(responseValidation.errorMessage || '會員狀態數據驗證失敗');
      }
  ...
  async checkFeatureUsage(feature: string): Promise<{ usage: FeatureUsage }> {
    try {
      ...
      if (!responseValidation.isValid) {
        throw new Error(responseValidation.errorMessage || '功能使用量數據驗證失敗');
      }
      return responseValidation.data!;
    } catch (error) {
      throw error;
  ...
  async getBenefits(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '會員權益數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async getLimits(): Promise<{
    ...
    try {
      ...
      const validationResult = validateApiResponse(MembershipLimitsSchema, response.data);
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '功能限制數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async getAvailableFeatures(): Promise<{
    ...
    try {
      ...
      const validationResult = validateApiResponse(MembershipFeaturesSchema, response.data);
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '可用功能數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async isFeatureAvailable(feature: string): Promise<boolean> {
    try {
      ...
      if (!responseValidation.isValid) {
        throw new Error(responseValidation.errorMessage || '功能可用性數據驗證失敗');
      }
  ...
  async getTrialStatus(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '試用狀態數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async getPaymentHistory(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '付款歷史數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async getSubscriptionDetails(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '訂閱詳情數據驗證失敗');
      }
      return validationResult.data!;
    } catch (error) {
      throw error;
  ...
  async updatePaymentMethod(paymentMethod: any): Promise<MembershipStatus> {
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '付款方式數據驗證失敗');
      }
  ...
  async downloadInvoice(invoiceId: string): Promise<MembershipStatus> {
    try {
      ...
      if (!responseValidation.isValid) {
        throw new Error(responseValidation.errorMessage || '下載鏈接數據驗證失敗');
      }
      return responseValidation.data!;
    } catch (error) {
      throw error;
