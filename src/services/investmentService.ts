...
class InvestmentService {
  ...
  async getInvestment(investmentId: string): Promise<Investment> {
    try {
      const validationResult = validateInput(
        z.object({ investmentId: z.string().uuid('無效的投資 ID') }),
        { investmentId }
      );
      if (!validationResult.isValid) {
        throw new Error(validationResult.errorMessage || '投資 ID 驗證失敗');
      }
      const response = await apiService.get<Investment>(
  ...
  async addInvestment(data: {
    ...
    try {
      ...
      const response = await apiService.post<Investment>(
        '/investments',
  ...
  async updateInvestment(
    ...
    try {
      ...
      const response = await apiService.put<Investment>(
        `/investments/${investmentId}`,
        data
      );
      const responseValidation = validateApiResponse(
  ...
  async getPortfolio(): Promise<Portfolio> {
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(
          validationResult.errorMessage || '投資組合數據驗證失敗'
        );
      }
      return validationResult.data!;
    } catch (error) {
  ...
  async getInvestmentAdvice(cardId: string): Promise<InvestmentAdvice> {
    try {
      ...
      const response = await apiService.get<InvestmentAdvice>(
        `/investments/advice/${cardId}`
      );
      const responseValidation = validateApiResponse(
        z.object({
          cardId: z.string(),
      ...
      if (!responseValidation.isValid) {
        throw new Error(
          responseValidation.errorMessage || '投資建議數據驗證失敗'
        );
      }
      return responseValidation.data!;
    } catch (error) {
  ...
  async getInvestmentStatistics(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(
          validationResult.errorMessage || '投資統計數據驗證失敗'
        );
      }
      return validationResult.data!;
    } catch (error) {
  ...
  async setPriceAlert(
    ...
    try {
      const validationResult = validateInput(PriceAlertRequestSchema, {
        cardId,
        targetPrice,
        type,
      });
      if (!validationResult.isValid) {
        throw new Error(
          validationResult.errorMessage || '價格警報數據驗證失敗'
        );
      }
      ...
      if (!responseValidation.isValid) {
        throw new Error(
          responseValidation.errorMessage || '價格警報數據驗證失敗'
        );
      }
      return responseValidation.data!;
    } catch (error) {
  ...
  async deletePriceAlert(alertId: string): Promise<void> {
    try {
      const validationResult = validateInput(
        z.object({ alertId: z.string().uuid('無效的警報 ID') }),
        { alertId }
      );
      if (!validationResult.isValid) {
  ...
  async getInvestmentHistory(filters?: any): Promise<{
    ...
    try {
      ...
      const response = await apiService.get('/investments/history', {
        params: filters,
      });
      const responseValidation = validateApiResponse(
        z.object({
      ...
      if (!responseValidation.isValid) {
        throw new Error(
          responseValidation.errorMessage || '投資歷史數據驗證失敗'
        );
      }
      return responseValidation.data!;
    } catch (error) {
  ...
  // 導出投資報告
  async exportInvestmentReport(
    format: 'pdf' | 'csv' | 'json' = 'pdf'
  ): Promise<string> {
    try {
      const validationResult = validateInput(
        z.object({ format: z.enum(['pdf', 'csv', 'json']) }),
        { format }
      );
      if (!validationResult.isValid) {
      ...
      const response = await apiService.get(
        `/investments/export?format=${format}`
      );
  ...
  async getMarketTrends(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(
          validationResult.errorMessage || '市場趨勢數據驗證失敗'
        );
      }
      return validationResult.data!;
    } catch (error) {
  ...
  async getRiskAnalysis(): Promise<{
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(
          validationResult.errorMessage || '風險分析數據驗證失敗'
        );
      }
      return validationResult.data!;
    } catch (error) {
