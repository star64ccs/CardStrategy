...
export class SearchPerformanceOptimizationService {
  ...
  public async optimizeSearch(
    ...
    try {
      ...
      // 計算性能提升
      const _performanceImprovement = this.calculatePerformanceImprovement(
        query,
        optimizedQuery
      );
      const _searchAccuracy = this.calculateSearchAccuracy(optimizedQuery);
  ...
  private generateOptimizedQuery(
    ...
    const _allTerms = [originalQuery, ...spellCorrections, ...synonyms];
