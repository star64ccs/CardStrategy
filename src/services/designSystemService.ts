...
export class DesignSystemServiceClass implements DesignSystemService {
  ...
  setTheme(theme: ThemeType): void {
    ...
    const _oldTheme = this.currentTheme;
  ...
  updateComponent(name: string, config: Partial<ComponentConfig>): void {
    const _existingConfig = this.getComponent(name);
    if (existingConfig) {
  ...
  updateToken(name: string, value: string | number): void {
    const _token = this.getToken(name);
    if (token) {
      const _oldValue = token.value;
      token.value = value;
      this.emit({
        type: 'tokenUpdate',
        tokenName: name,
  ...
  updateAccessibilityConfig(config: Partial<AccessibilityConfig>): void {
    const _oldConfig = { ...this.accessibility };
  ...
  checkContrastRatio(color1: string, color2: string): number {
    const _luminance1 = this.getLuminance(color1);
    const _luminance2 = this.getLuminance(color2);
    const _lighter = Math.max(luminance1, luminance2);
    const _darker = Math.min(luminance1, luminance2);
  ...
  isAccessible(
    ...
    const _ratio = this.checkContrastRatio(foreground, background);
    const _requiredRatio =
      size === 'large'
  ...
  private loadThemePreference(): void {
    if (typeof window !== 'undefined') {
      const _savedTheme = localStorage.getItem(
        'cardstrategy-theme'
      ) as ThemeType;
  ...
  private getLuminance(color: string): number {
    ...
    const _r = parseInt(hex.substr(0, 2), 16) / 255;
    const _g = parseInt(hex.substr(2, 2), 16) / 255;
    const _b = parseInt(hex.substr(4, 2), 16) / 255;

    const _rsRGB = r <= 0.03928 ? r / 12.92 : ((r + 0.055) / 1.055) ** 2.4;
    const _gsRGB = g <= 0.03928 ? g / 12.92 : ((g + 0.055) / 1.055) ** 2.4;
    const _bsRGB = b <= 0.03928 ? b / 12.92 : ((b + 0.055) / 1.055) ** 2.4;

    return 0.2126 * rsRGB + 0.7152 * gsRGB + 0.0722 * bsRGB;
  }
}

export const _designSystemService = new DesignSystemServiceClass();
