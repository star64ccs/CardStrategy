...
class CardService {
  ...
  async getCards(params: CardSearchParams = {}): Promise<ApiResponse<CardsResponse['data']>> {
    try {
      LoggingUtils.logApiCall('getCards', params);

      const response = await apiService.get<CardsResponse['data']>(API_CONFIG.CARDS.LIST, { params });
  ...
  async getCardDetail(id: string): Promise<ApiResponse<CardDetailResponse['data']>> {
    try {
      ValidationUtils.validateUUID(id, '卡片 ID');
      LoggingUtils.logApiCall('getCardDetail', { id });

      const response = await apiService.get<CardDetailResponse['data']>(API_CONFIG.CARDS.DETAIL(id));
  ...
  async searchCards(query: string, params: CardSearchParams = {}): Promise<ApiResponse<CardsResponse['data']>> {
    try {
      ...
      const response = await apiService.get<CardsResponse['data']>(API_CONFIG.CARDS.SEARCH, {
        params: { query, ...params }
      });

      LoggingUtils.logApiCall('searchCards', { query, ...params }, response.data);
      return response;
    } catch (error) {
  ...
  async getRecommendations(userId?: string): Promise<ApiResponse<CardsResponse['data']>> {
    try {
      const params = userId ? { userId } : {};
      LoggingUtils.logApiCall('getRecommendations', params);

      const response = await apiService.get<CardsResponse['data']>(API_CONFIG.CARDS.RECOMMENDATIONS, { params });
  ...
  async recognizeCard(imageUri: string): Promise<ApiResponse<CardRecognitionResult['data']>> {
    try {
      ...
      const response = await apiService.post<CardRecognitionResult['data']>(API_CONFIG.CARDS.RECOGNIZE, {
  ...
  async verifyCard(id: string): Promise<ApiResponse<CardVerificationResult['data']>> {
    try {
      ValidationUtils.validateUUID(id, '卡片 ID');
      LoggingUtils.logApiCall('verifyCard', { id });

      const response = await apiService.post<CardVerificationResult['data']>(API_CONFIG.CARDS.VERIFY(id), { cardId: id });

      LoggingUtils.logApiCall('verifyCard', { id }, response.data);
      return response;
    } catch (error) {
      LoggingUtils.logApiError('verifyCard', error, { id });
      throw error;
    }
  }

  async analyzeCardCondition(id: string): Promise<ApiResponse<CardConditionAnalysisResult['data']>> {
    try {
      ValidationUtils.validateUUID(id, '卡片 ID');
      LoggingUtils.logApiCall('analyzeCardCondition', { id });

      const response = await apiService.post<CardConditionAnalysisResult['data']>(API_CONFIG.CARDS.ANALYZE_CONDITION(id), { cardId: id });

      LoggingUtils.logApiCall('analyzeCardCondition', { id }, response.data);
      return response;
    } catch (error) {
      LoggingUtils.logApiError('analyzeCardCondition', error, { id });
      throw error;
    }
  }

  async recognizeCardsBatch(imageUris: string[]): Promise<ApiResponse<CardRecognitionResult['data'][]>> {
    try {
      ...
      LoggingUtils.logApiCall('recognizeCardsBatch', { count: imageUris.length });
      ...
      LoggingUtils.logBatchOperation('recognizeCardsBatch', imageUris.length, successCount, failedCount, duration);
      ...
      LoggingUtils.logApiError('recognizeCardsBatch', error, { count: imageUris.length });
      throw error;
    }
  }
  ...
  private async convertImageToBase64(imageUri: string): Promise<string> {
    try {
      LoggingUtils.logFileOperation('convertImageToBase64', 'process', imageUri);
      ...
      if (imageUri.startsWith('file://')) {
        ...
        LoggingUtils.logFileOperation('convertImageToBase64', 'process', imageUri, dataUrl.length);
        return dataUrl;
      }
      ...
      if (imageUri.startsWith('http://') || imageUri.startsWith('https://')) {
        ...
        LoggingUtils.logFileOperation('convertImageToBase64', 'process', imageUri, base64.length);
        return base64;
      }

      // 檢查是否已經是 base64 格式
      if (imageUri.startsWith('data:')) {
        LoggingUtils.logFileOperation('convertImageToBase64', 'process', imageUri, imageUri.length);
        return imageUri;
      }
