...
export interface ValidationService {
  ...
  validatePrice(price: unknown): boolean;
  validatePrivacyPreferences(preferences: unknown): {
    isValid: boolean;
    errors: string[];
  };
  validateFeedback(feedback: unknown): { isValid: boolean; errors: string[] };
  validateDataRightsRequest(request: unknown): {
    isValid: boolean;
    errors: string[];
  };
...
class ValidationServiceImpl implements ValidationService {
  ...
  validatePhoneNumber(phone: string): boolean {
    try {
      ...
      const _trimmedPhone = phone.trim();
      if (trimmedPhone === '') {
        return false;
      }
  ...
  validateCardNumber(cardNumber: string): boolean {
    try {
      ...
      const _cleanNumber = cardNumber.replace(/\s/g, '');
  ...
  validateCardName(cardName: string): boolean {
    try {
      ...
      const _trimmedName = cardName.trim();
  ...
  validateAgeVerification(data: unknown): {
    ...
    try {
      ...
      if (!data.birthDate || !this.validateDate(data.birthDate)) {
        ...
        const _birthDate = new Date(data.birthDate);
...
export const _validationService = new ValidationServiceImpl();
