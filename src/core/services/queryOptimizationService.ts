...
export class QueryOptimizationService {
  ...
  public analyzeQuery(queryMetrics: QueryMetrics): QueryAnalysis {
    ...
    // 計算優化分數
    const _optimizationScore = this.calculateOptimizationScore(
      sql,
      executionTime
    );
    ...
    // 預估改善效果
    const _estimatedImprovement = this.estimateImprovement(
      sql,
      executionTime,
      recommendations
    );
    ...
    // 評估風險等級
    const _riskLevel = this.assessRiskLevel(sql, recommendations);
  ...
  public optimizeQuery(sql: string): OptimizationResult {
    ...
    const _riskAssessment = this.assessOptimizationRisk(
      originalQuery,
      optimizedQuery,
      changes
    );
  ...
  public generatePerformanceReport(): PerformanceReport {
    ...
    const _totalQueries = this.queryHistory.length;
    if (totalQueries === 0) {
      return {
        totalQueries: 0,
    ...
    const _averageExecutionTime =
      this.queryHistory.reduce((sum, q) => sum + q.executionTime, 0) /
      totalQueries;
    const _slowQueries = this.queryHistory.filter(
      q => q.executionTime > 1000
    ).length;
    const _verySlowQueries = this.queryHistory.filter(
    ...
    // 生成整體建議
    const _recommendations = this.generateOverallRecommendations(
      queryAnalyses,
      queryPatterns
    );
  ...
  private setupOptimizationRules(): void {
    ...
    this.optimizationRules.set('optimizeLimit', (sql: string) => {
      ...
      if (pattern.test(sql)) {
        const _offset = parseInt(sql.match(/OFFSET\s+(\d+)/i)?.[1] || '0');
        if (offset > 1000) {
          return {
            optimized: true,
  ...
  private analyzeQueryPatterns(): QueryPattern[] {
    const _patterns = new Map<string, QueryPattern>();

    for (const query of this.queryHistory) {
      const _pattern = this.extractQueryPattern(query.sql);
      ...
      const _patternData = patterns.get(pattern)!;
  ...
  private generateOverallRecommendations(
    ...
    const _slowQueries = analyses.filter(a => a.executionTime > 1000).length;
    ...
    const _highComplexityQueries = analyses.filter(
      a => a.complexity === 'HIGH'
    ).length;
    ...
    const _frequentPatterns = patterns.filter(p => p.frequency > 10);
    ...
    const _optimizationOpportunities = analyses.filter(
  ...
  private calculateOverallScore(
    ...
    // 基於優化分數
    const _averageOptimizationScore =
