...
export class AnalyticsService {
  ...
  async trackEvent(event: AnalyticsEvent): Promise<ApiResponse> {
    try {
      const _platforms = event.platforms || this.defaultPlatforms;
      const _availablePlatforms = this.getAvailablePlatforms();
      const _validPlatforms = platforms.filter(
  ...
  async identifyUser(user: AnalyticsUser): Promise<ApiResponse> {
    try {
      const _platforms = user.platforms || this.defaultPlatforms;
      const _availablePlatforms = this.getAvailablePlatforms();
      const _validPlatforms = platforms.filter(
        platform => availablePlatforms[platform]
      );
  ...
  async trackUserSignUp(
    ...
    const _validPlatforms = platforms || this.defaultPlatforms;
    const _availablePlatforms = this.getAvailablePlatforms();
    const _filteredPlatforms = validPlatforms.filter(
      platform => availablePlatforms[platform]
    );
