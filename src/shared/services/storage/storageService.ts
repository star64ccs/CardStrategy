...
export class StorageService {
  ...
  async uploadFile(
    ...
    try {
      ...
      const _selectedProvider = this.selectProvider(provider);
      ...
      if (selectedProvider === 's3') {
        const _result = await s3Service.uploadFile(
          key,
          file,
        ...
        const _result = await cloudflareService.uploadFile(
          key,
          file,
  ...
  async listFiles(
    ...
    try {
      ...
      if (selectedProvider === 's3') {
        const _result = await s3Service.listFiles(options as S3ListOptions);
        if (result.success && result.data) {
          return {
            success: true,
        ...
        const _result = await cloudflareService.listFiles(
  ...
  async copyFile(
    ...
    try {
      ...
      if (selectedProvider === 's3') {
        const _result = await s3Service.copyFile(sourceKey, destinationKey);
        if (result.success && result.data) {
          return {
            success: true,
        ...
        const _result = await cloudflareService.copyFile(
          sourceKey,
          destinationKey
        );
        if (result.success && result.data) {
  ...
  async getFileMetadata(
    ...
    try {
      ...
      if (selectedProvider === 's3') {
        const _result = await s3Service.getFileMetadata(key);
        if (result.success && result.data) {
          return {
            success: true,
            data: this.convertS3FileToStorageFile(result.data),
        ...
        const _result = await cloudflareService.getFileMetadata(key);
        if (result.success && result.data) {
          return {
            success: true,
            data: this.convertCloudflareFileToStorageFile(result.data),
  ...
  async getServiceStats(): Promise<ApiResponse> {
    try {
      const _s3Stats = await s3Service.getServiceStats();
      const _cloudflareStats = await cloudflareService.getServiceStats();
...
export const _storageService = StorageService.getInstance();
