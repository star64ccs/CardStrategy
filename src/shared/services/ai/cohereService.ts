...
import { SearchParams, PaginationParams } from '../../../core/types';
...
export class CohereService {
  ...
  async embedTexts(
    ...
    try {
      ...
      const _response = await api.post<CohereEmbedResponse>(
        `${this.baseUrl}/embed`,
        {
          texts,
          model,
  ...
  async generateText(
    ...
    try {
      ...
      if (
        ...
        const _mockText = `模擬生成的文本：${prompt.substring(0, 20)}... (溫度: ${temperature})`;
        return {
          success: true,
          data: mockText,
          message: '開發環境模擬結果',
      ...
      const _response = await api.post<CohereGenerateResponse>(
  ...
  async classifyText(
    ...
    try {
      ...
      const _response = await api.post<CohereClassifyResponse>(
  ...
  async summarizeText(
    ...
    try {
      ...
      if (
        ...
        const _mockSummary = `模擬摘要：${text.substring(0, Math.min(50, text.length))}...`;
        return {
          success: true,
          data: mockSummary,
          message: '開發環境模擬結果',
      ...
      const _response = await api.post<CohereSummarizeResponse>(
  ...
  async batchProcess(
    ...
    try {
      ...
      if (
        ...
        const _mockResults = texts.map((text, index) => {
          switch (operation) {
            case 'embed':
              return {
                id: `mock_embedding_${index}`,
