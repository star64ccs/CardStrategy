...
export class RBACService {
  ...
  getUserPermissions(userId: string): Permission[] {
    ...
    const _permissionIds = new Set<string>();
  ...
  private async setupRoleHierarchy(): Promise<void> {
    ...
    const _userRole = Array.from(this.roles.values()).find(
      r => r.name === 'user'
    );
    const _guestRole = Array.from(this.roles.values()).find(
      r => r.name === 'guest'
    );
  ...
  private getInheritedPermissions(roleId: string): Permission[] {
    const permissions: Permission[] = [];
    const _childRoleIds = this.roleHierarchy.get(roleId) || [];

    for (const childRoleId of childRoleIds) {
      const _childRole = this.roles.get(childRoleId);
      if (childRole) {
