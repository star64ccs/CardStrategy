...
export class OAuthService {
  ...
  getAuthorizationUrl(provider: string, state?: string): ApiResponse<string> {
    try {
      const _oauthProvider = this.config.providers[provider];
      if (!oauthProvider) {
        return {
          success: false,
      ...
      const _authUrl = `${oauthProvider.authorizationUrl}?${params.toString()}`;
  ...
  async exchangeCodeForToken(
    ...
    try {
      ...
      const _response = await api.post(oauthProvider.tokenUrl, tokenData, {
        headers: {
  ...
  async getUserInfo(
    ...
    try {
      ...
      const _response = await api.get(oauthProvider.userInfoUrl, { headers });
      ...
      const _userData = response.data as any;
...
export const _oauthService = new OAuthService();
