...
export const LazyLoadExample: React.FC = () => {
  ...
  const _dataHook = useLazyDataSimple(mockDataLoader, {
    strategy: LazyLoadStrategy.MANUAL,
    priority: LazyLoadPriority.NORMAL,
  });
              ...
              <div>
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderRadius: '4px',
                    cursor: componentHook.isLoading ? 'not-allowed' : 'pointer',
                    marginBottom: '10px',
                  }}
                >
                  {componentHook.isLoading ? '加載中...' : '手動加載組件'}
                </button>

                {componentHook.isLoading && <CustomLoadingComponent />}
                {componentHook.hasError && componentHook.state.error && (
                  <CustomErrorComponent
                    error={componentHook.state.error}
                    retry={componentHook.retry}
                  />
                )}
                {componentHook.isLoaded && componentHook.state.component && (
                  <div>
                    <h4>Hook 加載的組件：</h4>
                    <componentHook.state.component
                      title='Hook 加載的組件'
