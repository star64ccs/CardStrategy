...
import { mobileOptimizer, getDeviceInfo, getResponsiveValue } from '@/utils/mobileOptimizer';
...
const MobileDashboard: React.FC<MobileDashboardProps> = ({ userId }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [health, metrics, analytics, suggestions, settings] = await Promise.all([
    ...
    <View style={styles.tabContent}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.deviceInfoGrid}>
          <View style={styles.deviceInfoCard}>
            ...
            <Text style={styles.deviceInfoValue}>{deviceInfo?.platform || 'N/A'}</Text>
          </View>
          <View style={styles.deviceInfoCard}>
            <Text style={styles.deviceInfoLabel}>版本</Text>
            <Text style={styles.deviceInfoValue}>{deviceInfo?.version || 'N/A'}</Text>
          </View>
          <View style={styles.deviceInfoCard}>
            <Text style={styles.deviceInfoLabel}>設備類型</Text>
            <Text style={styles.deviceInfoValue}>
              {deviceInfo?.isTablet ? '平板' : deviceInfo?.isMobile ? '手機' : '桌面'}
            </Text>
          </View>
          <View style={styles.deviceInfoCard}>
      ...
      {mobileHealth && (
        <View style={styles.section}>
          ...
          <View style={[styles.healthCard, styles[`health${mobileHealth.status}`]]}>
            <View style={styles.healthHeader}>
              <MaterialIcons
                name={mobileHealth.status === 'healthy' ? 'check-circle' : 'warning'}
                size={24}
                color={mobileHealth.status === 'healthy' ? '#4CAF50' : '#FF9800'}
              />
        ...
        optimizationSuggestions.map((suggestion, index) => (
          <View key={index} style={styles.suggestionCard}>
            <View style={styles.suggestionHeader}>
              <MaterialIcons
                name={suggestion.type === 'performance' ? 'speed' :
                  suggestion.type === 'battery' ? 'battery-full' :
                    suggestion.type === 'storage' ? 'storage' : 'wifi'}
                size={20}
                color="#2196F3"
              />
              <Text style={styles.suggestionTitle}>{suggestion.title}</Text>
              <View style={[styles.priorityBadge, styles[`priority${suggestion.priority}`]]}>
      ...
      {notificationSettings && (
        <View style={styles.notificationSettings}>
          <View style={styles.settingItem}>
            ...
            <TouchableOpacity
              style={[styles.toggle, notificationSettings.priceAlerts && styles.toggleActive]}
              onPress={() => toggleNotificationSetting('priceAlerts')}
            >
              <View style={[styles.toggleThumb, notificationSettings.priceAlerts && styles.toggleThumbActive]} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.settingItem}>
            ...
            <TouchableOpacity
              style={[styles.toggle, notificationSettings.marketUpdates && styles.toggleActive]}
              onPress={() => toggleNotificationSetting('marketUpdates')}
            >
              <View style={[styles.toggleThumb, notificationSettings.marketUpdates && styles.toggleThumbActive]} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.settingItem}>
            ...
            <TouchableOpacity
              style={[styles.toggle, notificationSettings.portfolioChanges && styles.toggleActive]}
              onPress={() => toggleNotificationSetting('portfolioChanges')}
            >
              <View style={[styles.toggleThumb, notificationSettings.portfolioChanges && styles.toggleThumbActive]} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.settingItem}>
            ...
            <TouchableOpacity
              style={[styles.toggle, notificationSettings.systemNotifications && styles.toggleActive]}
              onPress={() => toggleNotificationSetting('systemNotifications')}
            >
              <View style={[styles.toggleThumb, notificationSettings.systemNotifications && styles.toggleThumbActive]} />
            </TouchableOpacity>
          </View>
        </View>
      )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.quietHoursCard}>
            <View style={styles.settingItem}>
              ...
              <TouchableOpacity
                style={[styles.toggle, notificationSettings.quietHours.enabled && styles.toggleActive]}
                onPress={() => toggleNotificationSetting('quietHours.enabled')}
              >
                <View style={[styles.toggleThumb, notificationSettings.quietHours.enabled && styles.toggleThumbActive]} />
              </TouchableOpacity>
            </View>
          </View>
        </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.analyticsGrid}>
              ...
              <View style={styles.analyticsCard}>
                ...
                <Text style={styles.analyticsValue}>
                  {Math.round(analyticsReport.usage?.averageSessionTime || 0)}分鐘
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.analyticsGrid}>
              <View style={styles.analyticsCard}>
                ...
                <Text style={styles.analyticsValue}>
                  {analyticsReport.performance?.averageLoadTime?.toFixed(2) || '0'}ms
                </Text>
              </View>
              <View style={styles.analyticsCard}>
                <Text style={styles.analyticsLabel}>崩潰率</Text>
                <Text style={styles.analyticsValue}>
                  {(analyticsReport.performance?.crashRate * 100 || 0).toFixed(2)}%
                </Text>
              </View>
              <View style={styles.analyticsCard}>
                <Text style={styles.analyticsLabel}>內存使用</Text>
                <Text style={styles.analyticsValue}>
                  {analyticsReport.performance?.memoryUsage?.toFixed(1) || '0'}MB
                </Text>
              </View>
            </View>
          </View>
          ...
          {analyticsReport.usage?.mostUsedFeatures && (
            <View style={styles.section}>
              ...
              <View style={styles.featuresList}>
                {analyticsReport.usage.mostUsedFeatures.map((feature: string, index: number) => (
...
const responsivePadding = getResponsiveValue({ xs: 12, sm: 16, md: 20, lg: 24, xl: 28 });
