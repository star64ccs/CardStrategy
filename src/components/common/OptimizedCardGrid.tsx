...
export const OptimizedCardGrid: React.FC<OptimizedCardGridProps> = memo(({
  ...
  // 使用內存監控
  const { getCurrentMemory, getMemoryStats, checkMemoryLeak } = useMemoryMonitor({
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      logger.error('刷新卡片數據失敗', { error: error instanceof Error ? error.message : String(error) });
    } finally {
      setRefreshing(false);
    }
  }, [onRefresh]);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        getItemLayout={size === 'large' ? undefined : (data, index) => ({
