...
import { recommendationService } from '../../features/ai/services/recommendationService';
import type {
...
export const _getRecommendationHistory = createAsyncThunk(
...
export const _getRecommendationStats = createAsyncThunk(
...
export const _getUserProfile = createAsyncThunk(
  ...
  async (userId: string, { rejectWithValue }) => {
    try {
      const _profile = await recommendationService.getUserProfile(userId);
      return profile;
    } catch (error) {
      return rejectWithValue({
...
export const _analyzePortfolio = createAsyncThunk(
...
// Slice
const _recommendationSlice = createSlice({
  name: 'recommendation',
  initialState,
  reducers: {
...
// Selectors
export const _selectCurrentRecommendation = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation;

export const _selectRecommendationHistory = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.recommendationHistory;

export const _selectRecommendationStats = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.recommendationStats;

export const _selectRecommendationLoading = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.loading;

export const _selectRecommendationError = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.error;

export const _selectUserProfile = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.userProfile;

export const _selectRecommendationsByAction = (
  state: { recommendation: RecommendationState },
  action: string
) => {
  const _current = state.recommendation.currentRecommendation;
  if (!current) return [];
...
export const _selectHighPriorityRecommendations = (state: {
  recommendation: RecommendationState;
}) => {
  const _current = state.recommendation.currentRecommendation;
  if (!current) return [];
...
export const _selectRecommendationsByRisk = (
  state: { recommendation: RecommendationState },
  riskLevel: string
) => {
  const _current = state.recommendation.currentRecommendation;
...
export const _selectPortfolioSuggestion = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.portfolioSuggestion;

export const _selectRiskAnalysis = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.riskAnalysis;

export const _selectExpectedReturn = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.expectedReturn;

export const _selectRecommendationReasoning = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.reasoning;

export const _selectRecommendationConfidence = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.confidence || 0;

export const _selectRecommendationValidUntil = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.validUntil;

export const _selectIsRecommendationValid = (state: {
  recommendation: RecommendationState;
}) => {
  const _validUntil = state.recommendation.currentRecommendation?.validUntil;
  if (!validUntil) return false;
  return new Date(validUntil) > new Date();
};

export const _selectRecommendationMetadata = (state: {
  recommendation: RecommendationState;
}) => state.recommendation.currentRecommendation?.metadata;

export const _selectUserExperience = (state: {
  recommendation: RecommendationState;
...
export const _selectUserInvestmentCapacity = (state: {
  recommendation: RecommendationState;
}) => {
  const _profile = state.recommendation.userProfile;
  if (!profile) return 0;
