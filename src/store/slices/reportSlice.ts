...
export const _initializeReportService = createAsyncThunk(
  ...
  async (config?: unknown) => {
    const _service = ReportService.getInstance();
    await service.initialize(config);
    return { success: true };
  }
);

export const _createTemplate = createAsyncThunk(
  'report/createTemplate',
  async (request: CreateTemplateRequest) => {
    const _service = ReportService.getInstance();
    const _response = await service.createTemplate(request);
    if (!response.success) {
...
export const _getTemplate = createAsyncThunk(
  'report/getTemplate',
  async (templateId: string) => {
    const _service = ReportService.getInstance();
    const _response = await service.getTemplate(templateId);
    if (!response.success) {
      throw new Error(response.message);
    }
    return response.data;
...
export const _createReport = createAsyncThunk(
  ...
  async (request: CreateReportRequest) => {
    const _service = ReportService.getInstance();
    const _response = await service.createReport(request);
    if (!response.success) {
      throw new Error(response.message);
    }
    return response.data;
...
export const _getReport = createAsyncThunk(
  'report/getReport',
  async (reportId: string) => {
    const _service = ReportService.getInstance();
    const _response = await service.getReport(reportId);
    if (!response.success) {
      throw new Error(response.message);
    }
    return response.data;
...
export const _exportReport = createAsyncThunk(
  'report/exportReport',
  async ({
    reportId,
    request,
    ...
    const _service = ReportService.getInstance();
    const _response = await service.exportReport(reportId, request);
    if (!response.success) {
      throw new Error(response.message);
    }
    return response.data;
...
export const _getAnalytics = createAsyncThunk(
  ...
  async () => {
    const _service = ReportService.getInstance();
    const _response = await service.getAnalytics();
    if (!response.success) {
      throw new Error(response.message);
    }
    return response.data;
...
// Slice
const _reportSlice = createSlice({
  name: 'report',
  initialState,
  reducers: {
    // 模板相關
    ...
    updateTemplate: (state, action: PayloadAction<ReportTemplate>) => {
      const _index = state.templates.findIndex(t => t.id === action.payload.id);
      if (index !== -1) {
    ...
    updateExport: (state, action: PayloadAction<ReportExport>) => {
      const _index = state.exports.findIndex(e => e.id === action.payload.id);
      if (index !== -1) {
...
export const _selectSelectedTemplateId = (state: { report: ReportState }) =>
  state.report.selectedTemplateId;
export const _selectReports = (state: { report: ReportState }) =>
  state.report.reports;
export const _selectCurrentReport = (state: { report: ReportState }) =>
  state.report.currentReport;
export const _selectSelectedReportId = (state: { report: ReportState }) =>
  state.report.selectedReportId;
export const _selectExports = (state: { report: ReportState }) =>
  state.report.exports;
export const _selectCurrentExport = (state: { report: ReportState }) =>
  state.report.currentExport;
export const _selectAnalytics = (state: { report: ReportState }) =>
  state.report.analytics;
export const _selectFilterOptions = (state: { report: ReportState }) =>
  state.report.filterOptions;
export const _selectPagination = (state: { report: ReportState }) =>
  state.report.pagination;
export const _selectIsLoading = (state: { report: ReportState }) =>
  state.report.isLoading;
export const _selectError = (state: { report: ReportState }) =>
  state.report.error;
...
export const _selectCompletedReports = (state: { report: ReportState }) =>
  state.report.reports.filter(r => r.status === ReportStatus.COMPLETED);

export const _selectCompletedExports = (state: { report: ReportState }) =>
  state.report.exports.filter(e => e.status === ExportStatus.COMPLETED);

export const _selectTemplateCount = (state: { report: ReportState }) =>
  state.report.templates.length;

export const _selectReportCount = (state: { report: ReportState }) =>
  state.report.reports.length;

export const _selectExportCount = (state: { report: ReportState }) =>
