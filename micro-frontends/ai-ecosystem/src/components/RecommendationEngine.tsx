...
const RecommendationEngine: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'investment' | 'collection' | 'trading' | 'market_opportunity'>('all');
      ...
      const mockRecommendations: AIRecommendation[] = [
        {
          ...
          type: 'investment',
          title: '青眼白龍投資機會',
          description: '基於市場分析，建議在當前價格買入青眼白龍，預期短期內有15%的漲幅',
          confidence: 87.5,
          priority: 'high',
          targetCards: ['card1'],
          expectedReturn: 15.5,
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'investment': return '投資建議';
      case 'collection': return '收藏建議';
      case 'trading': return '交易建議';
            ...
            <div className="recommendation-targets">
              ...
              <div className="target-cards">
                {rec.targetCards.map((cardId, index) => (
                  <span key={index} className="target-card">{cardId}</span>
                ))}
              </div>
            </div>
