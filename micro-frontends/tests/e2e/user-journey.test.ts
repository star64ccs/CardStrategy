...
describe('CardStrategy 用戶旅程端到端測試', () => {
  ...
  test('完整用戶註冊和登錄流程', async () => {
    ...
    // 5. 驗證用戶已登錄
    await expect(page.locator('[data-testid="user-profile"]')).toContainText(testUser.username);
  });
  ...
  test('卡片掃描和識別流程', async () => {
    ...
    // 4. 等待 AI 識別
    await expect(page.locator('[data-testid="scanning-status"]')).toContainText('識別中');
    await expect(page.locator('[data-testid="scanning-status"]')).toContainText('識別完成', { timeout: 10000 });
    
    // 5. 驗證識別結果
    await expect(page.locator('[data-testid="card-name"]')).toContainText(testCard.name);
    await expect(page.locator('[data-testid="card-series"]')).toContainText(testCard.series);
  });
  ...
  test('市場分析和價格預測流程', async () => {
    ...
    await expect(page.locator('[data-testid="alert-success"]')).toContainText('提醒已設置');
  });
  ...
  test('投資組合管理流程', async () => {
    ...
    // 4. 驗證卡片已添加
    await expect(page.locator('[data-testid="portfolio-card"]')).toContainText(testCard.name);
  ...
  test('社交功能和分享流程', async () => {
    ...
    // 4. 驗證帖子已發布
    await expect(page.locator('[data-testid="user-post"]')).toContainText('分享我的新卡片收藏！');
    ...
    // 6. 驗證評論已添加
    await expect(page.locator('[data-testid="comment"])).toContainText('很棒的收藏！');
  });
  ...
  test('數據質量評估流程', async () => {
    ...
    await expect(page.locator('[data-testid="quality-check-status"]')).toContainText('檢查中');
    await expect(page.locator('[data-testid="quality-check-status"]')).toContainText('完成', { timeout: 15000 });
  ...
  test('隱私設置和數據管理流程', async () => {
    ...
    await expect(page.locator('[data-testid="settings-saved"]')).toContainText('設置已保存');
  ...
  test('錯誤處理和恢復流程', async () => {
    ...
    // 4. 驗證錯誤處理
    await expect(page.locator('[data-testid="error-message"]')).toContainText('加載失敗');
