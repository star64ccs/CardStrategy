...
class AdvancedPerformanceMetrics {
  private metrics: {
    ...
    networkSnapshots: Array<{ timestamp: number; bytesIn: number; bytesOut: number }>;
    userSessions: Array<{ userId: string; startTime: number; endTime?: number; actions: number }>;
  };
  ...
  getResults() {
    const { responseTimes, errors, totalRequests, startTime, endTime, memorySnapshots, cpuSnapshots, networkSnapshots, userSessions } = this.metrics;
  ...
  private calculateMemoryUsage() {
    if (this.metrics.memorySnapshots.length === 0) return { average: 0, peak: 0, trend: 'stable' };
  ...
  private calculateNetworkUsage() {
    if (this.metrics.networkSnapshots.length === 0) return { totalBytesIn: 0, totalBytesOut: 0, averageBytesPerSecond: 0 };
    
    const totalBytesIn = this.metrics.networkSnapshots.reduce((sum, s) => sum + s.bytesIn, 0);
    const totalBytesOut = this.metrics.networkSnapshots.reduce((sum, s) => sum + s.bytesOut, 0);
    const duration = this.metrics.networkSnapshots[this.metrics.networkSnapshots.length - 1].timestamp - this.metrics.networkSnapshots[0].timestamp;
    const averageBytesPerSecond = (totalBytesIn + totalBytesOut) / (duration / 1000);
    
    return { totalBytesIn, totalBytesOut, averageBytesPerSecond };
  }

  private calculateUserSessionStats() {
    if (this.metrics.userSessions.length === 0) return { totalSessions: 0, averageSessionDuration: 0, averageActionsPerSession: 0 };
    
    const completedSessions = this.metrics.userSessions.filter(s => s.endTime);
    const totalSessions = completedSessions.length;
    const averageSessionDuration = completedSessions.reduce((sum, s) => sum + (s.endTime! - s.startTime), 0) / totalSessions;
    const averageActionsPerSession = completedSessions.reduce((sum, s) => sum + s.actions, 0) / totalSessions;
...
class LoadTestScenario {
  ...
  static generateSpikeLoad() {
    return {
      ...
      spike: { users: ADVANCED_LOAD_CONFIG.spike.spikeUsers, duration: ADVANCED_LOAD_CONFIG.spike.spikeDuration },
      recovery: { users: ADVANCED_LOAD_CONFIG.spike.baseUsers, duration: ADVANCED_LOAD_CONFIG.spike.recoveryDuration }
    };
  }

  static generateBurstLoad() {
    return Array(ADVANCED_LOAD_CONFIG.burst.burstCount).fill(null).map((_, index) => ({
      burst: index + 1,
...
class UserBehaviorSimulator {
  ...
  getRandomBehavior(module: string) {
    const moduleBehaviors = this.behaviors[module as keyof typeof this.behaviors] || this.behaviors.cardManagement;
    ...
    for (const behavior of moduleBehaviors) {
      ...
      if (random <= 0) {
        return {
          ...
          duration: Math.random() * (behavior.duration[1] - behavior.duration[0]) + behavior.duration[0]
        };
      }
    }
...
describe('CardStrategy 高級負載測試', () => {
  ...
  test('漸進式負載測試 - 逐步增加用戶負載', async () => {
    ...
    for (const scenario of scenarios) {
      ...
      const userPromises = userPages.map(async (page, index) => {
        ...
        return this.simulateAdvancedUserWorkload(page, stageMetrics, scenario.duration, userId);
      });
  ...
  test('尖峰負載測試 - 模擬突發流量', async () => {
    ...
    const spikePromise = this.runUserWorkload(spikeUsers, scenario.spike.duration);
  ...
  test('持續負載測試 - 長時間穩定性測試', async () => {
    ...
    expect(results.averageResponseTime).toBeLessThan(config.expectedResponseTime);
    expect(results.errorRate).toBeLessThan(config.expectedErrorRate);
    expect(results.memoryUsage.peak).toBeLessThan(config.memoryThreshold * 1024 * 1024);
    
    console.log('✅ 持續負載測試完成:');
    console.log(`   測試時長: ${(results.duration / 1000 / 60).toFixed(2)} 分鐘`);
    console.log(`   平均響應時間: ${results.averageResponseTime.toFixed(2)}ms`);
    console.log(`   錯誤率: ${(results.errorRate * 100).toFixed(2)}%`);
    console.log(`   峰值內存使用: ${(results.memoryUsage.peak / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   內存趨勢: ${results.memoryUsage.trend}`);
    console.log(`   平均會話時長: ${(results.userSessionStats.averageSessionDuration / 1000).toFixed(2)}秒`);
  });
  ...
  test('突發負載測試 - 模擬間歇性高負載', async () => {
    ...
    console.log(`   錯誤率: ${(results.errorRate * 100).toFixed(2)}%`);
    console.log(`   總請求數: ${results.totalRequests}`);
    console.log(`   網絡使用: ${(results.networkUsage.totalBytesIn / 1024 / 1024).toFixed(2)}MB 輸入, ${(results.networkUsage.totalBytesOut / 1024 / 1024).toFixed(2)}MB 輸出`);
  });
  ...
  test('混合負載測試 - 多種用戶行為組合', async () => {
    ...
    console.log(`   每秒請求數: ${results.requestsPerSecond.toFixed(2)}`);
    console.log(`   平均會話時長: ${(results.userSessionStats.averageSessionDuration / 1000).toFixed(2)}秒`);
    console.log(`   平均每會話操作數: ${results.userSessionStats.averageActionsPerSession.toFixed(2)}`);
  });
  ...
  private async runUserWorkload(users: Page[], duration: number): Promise<void> {
    const userPromises = users.map(async (page, index) => {
      const userId = `user-${index}`;
      return this.simulateAdvancedUserWorkload(page, metrics, duration, userId);
    });
    
    await Promise.all(userPromises);
  }

  private async runTypedUserWorkload(users: Page[], userType: string, duration: number): Promise<void> {
    const userPromises = users.map(async (page, index) => {
      const userId = `user-${userType}-${index}`;
      return this.simulateTypedUserWorkload(page, metrics, duration, userId, userType);
    });
    
    await Promise.all(userPromises);
  }

  private async simulateAdvancedUserWorkload(page: Page, metrics: AdvancedPerformanceMetrics, duration: number, userId: string): Promise<void> {
    ...
    while (Date.now() < endTime) {
      ...
      try {
        ...
        // 執行用戶操作
        await this.executeUserAction(page, randomModule, behavior.action);
  ...
  private async simulateTypedUserWorkload(page: Page, metrics: AdvancedPerformanceMetrics, duration: number, userId: string, userType: string): Promise<void> {
    const startTime = Date.now();
    const endTime = startTime + duration;
    ...
    while (Date.now() < endTime) {
      ...
      try {
        ...
        await this.executeUserAction(page, userType, behavior.action);
  ...
  private async executeUserAction(page: Page, module: string, action: string): Promise<void> {
    switch (module) {
      case 'cardManagement':
        await this.executeCardManagementAction(page, action);
        break;
      case 'marketAnalysis':
        await this.executeMarketAnalysisAction(page, action);
        break;
      case 'aiEcosystem':
        await this.executeAIEcosystemAction(page, action);
        break;
    }
  }

  private async executeCardManagementAction(page: Page, action: string): Promise<void> {
    switch (action) {
      case 'browse_cards':
        await page.click('[data-testid="card-management-nav"]');
        await page.waitForSelector('[data-testid="card-list"]', { timeout: 5000 });
        break;
      case 'search_cards':
        await page.click('[data-testid="card-management-nav"]');
  ...
  private async executeMarketAnalysisAction(page: Page, action: string): Promise<void> {
    switch (action) {
      case 'view_market_data':
        await page.click('[data-testid="market-analysis-nav"]');
        await page.waitForSelector('[data-testid="market-dashboard"]', { timeout: 5000 });
        break;
      case 'analyze_trends':
        await page.click('[data-testid="market-analysis-nav"]');
  ...
  private async executeAIEcosystemAction(page: Page, action: string): Promise<void> {
    switch (action) {
      case 'ai_scan':
