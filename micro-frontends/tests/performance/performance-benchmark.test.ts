...
describe('CardStrategy 性能基準測試', () => {
  ...
  test('核心 Web Vitals 性能測試', async () => {
    ...
    const webVitals = await page.evaluate(() => {
      ...
      return {
        ...
        fid: fidEntries.length > 0 ? fidEntries[0].processingStart - fidEntries[0].startTime : 0,
    ...
    // 驗證性能基準
    expect(webVitals.lcp).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoad.largestContentfulPaint);
    expect(webVitals.fid).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoad.firstInputDelay);
    expect(cls).toBeLessThan(0.1); // CLS 應該小於 0.1
    expect(webVitals.fcp).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoad.firstContentfulPaint);
    expect(webVitals.domContentLoaded).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoad.domContentLoaded);
    expect(webVitals.loadComplete).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoad.loadComplete);
  ...
  test('微前端模組加載性能測試', async () => {
    ...
    for (const module of modules) {
      ...
      try {
        ...
        // 驗證模組加載時間
        const benchmark = PERFORMANCE_BENCHMARKS.moduleLoad[module as keyof typeof PERFORMANCE_BENCHMARKS.moduleLoad];
        expect(moduleLoadTimes[module]).toBeLessThan(benchmark);

        console.log(`${module} 模組加載時間: ${moduleLoadTimes[module].toFixed(2)}ms`);
      } catch (error) {
  ...
  test('API 響應性能測試', async () => {
    ...
    // 執行各種 API 操作
    const apiTests = [
      { action: () => page.click('[data-testid="card-management-nav"]'), name: 'cardOperations' },
      { action: () => page.fill('[data-testid="search-input"]', 'Luffy'), name: 'searchAPI' },
      { action: () => page.click('[data-testid="market-analysis-nav"]'), name: 'marketData' },
      { action: () => page.click('[data-testid="ai-ecosystem-nav"]'), name: 'aiPredictions' },
      { action: () => page.click('[data-testid="portfolio-nav"]'), name: 'portfolioData' }
    ];
    ...
    Object.entries(metrics).forEach(([api, responseTime]) => {
      const benchmark = PERFORMANCE_BENCHMARKS.apiResponse[api as keyof typeof PERFORMANCE_BENCHMARKS.apiResponse] || 1000;
      expect(responseTime as number).toBeLessThan(benchmark);
      console.log(`API ${api} 響應時間: ${(responseTime as number).toFixed(2)}ms`);
    });
  });
  ...
  test('內存使用和垃圾回收測試', async () => {
    ...
    // 驗證內存使用基準
    expect(initialMemory).toBeLessThan(PERFORMANCE_BENCHMARKS.memoryUsage.initialMemory);
    expect(memoryGrowth).toBeLessThan(PERFORMANCE_BENCHMARKS.memoryUsage.maxMemoryGrowth);
  ...
  test('渲染性能和動畫流暢度測試', async () => {
    ...
    await page.evaluate(() => {
      ...
      const measureFrameRate = () => {
        ...
        if (currentTime - lastTime >= 1000) { // 每秒計算一次
    ...
    if (fpsData.length > 0) {
      const averageFPS = fpsData.reduce((a: number, b: number) => a + b, 0) / fpsData.length;
      const minFPS = Math.min(...fpsData);

      // 驗證幀率基準
      expect(averageFPS).toBeGreaterThan(PERFORMANCE_BENCHMARKS.rendering.frameRate * 0.8); // 平均幀率應該大於 48 FPS
      expect(minFPS).toBeGreaterThan(PERFORMANCE_BENCHMARKS.rendering.frameRate * 0.5); // 最低幀率應該大於 30 FPS
